...
struct OnboardingView: View {
    ...
    private var logo: some View {
        RoundedRectangle(cornerRadius: 18, style: .continuous)
            .fill(LinearGradient(colors: [p.accent, Color(hex: 0x1F6FD0)], startPoint: .topLeading, endPoint: .bottomTrailing))
            .frame(width: 60, height: 60)
            .overlay(FishMark(fishColor: .white, eyeColor: p.accent).frame(width: 36, height: 36))
            .shadow(color: p.accent.opacity(0.4), radius: 11, y: 8)
    }
