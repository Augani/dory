...
[ "$CONCURRENCY" -gt 0 ] && [ "$CONCURRENCY" -le 64 ] \
  || die "concurrency must be between 1 and 64"
...
[ "$(stat -f %u "$SSH_AUTH_SOCK")" = "$(id -u)" ] \
  || die "SSH_AUTH_SOCK is not owned by the release user"
if [[ "$DOCKER" == */* ]]; then
  [ -f "$DOCKER" ] && [ ! -L "$DOCKER" ] && [ -x "$DOCKER" ] \
    || die "Docker CLI is not an exact regular executable"
...
[ -f "$BUILDX" ] && [ ! -L "$BUILDX" ] && [ -x "$BUILDX" ] \
  || die "the exact candidate Docker CLI has no regular sibling docker-buildx plugin: $BUILDX"
