...
IMAGE="${DORY_BIND_COHERENCE_IMAGE:-}"
WORKROOT="${DORY_BIND_COHERENCE_WORKROOT:-$HOME/.dory-bind-coherence}"
CONFIRM="${DORY_BIND_COHERENCE_CONFIRM:-}"
...
Options:
  ...
  --image REF        Already-local digest-pinned probe image
  --workroot PATH    Evidence root inside Dory's shared home (default: ~/.dory-bind-coherence)
  --confirm TOKEN    Must be ISOLATED-ENGINE-BIND-FILE-COHERENCE
  -h, --help
...
die() { echo "bind-file-coherence: $*" >&2; exit 2; }
...
printf '%s\n' "$IMAGE" \
  ...
  || die "probe image must be an exact digest reference"
for command in dd python3 seq shasum stat; do
...
start_primary_container() {
  container_id="$(docker_e run -d --pull=never --network none --name "$NAME" --label "$LABEL" \
    --mount "type=bind,src=$SHARE,dst=/work" \
    --mount "type=bind,src=$FILE,dst=/single/value.bin" \
    "$IMAGE" sh -c 'exec tail -f /dev/null')"
  printf '%s\n' "$container_id" | grep -Eq '^[0-9a-f]{64}$' \
    || die "bind probe launch returned an invalid container ID"
}
start_primary_container
docker_e inspect "$container_id" > "$RUN_DIR/primary-container-inspect.json"
python3 - "$RUN_DIR/primary-container-inspect.json" "$IMAGE" "$LABEL" "$SHARE" "$FILE" <<'PY'
import json
import pathlib
import sys
...
if not isinstance(document, list) or len(document) != 1 or not isinstance(document[0], dict):
    raise SystemExit("bind probe inspect is not one exact container")
...
if not isinstance(config, dict) or not isinstance(host, dict) or not isinstance(mounts, list):
    raise SystemExit("bind probe inspect omits runtime authority")
if config.get("Image") != sys.argv[2]:
    raise SystemExit("bind probe did not launch the exact qualified image")
label_key, label_value = sys.argv[3].split("=", 1)
labels = config.get("Labels")
if not isinstance(labels, dict) or labels.get(label_key) != label_value:
    raise SystemExit("bind probe is missing its exact ownership label")
if host.get("NetworkMode") != "none":
    raise SystemExit("bind probe unexpectedly has network access")
expected = {(sys.argv[4], "/work"), (sys.argv[5], "/single/value.bin")}
actual = {
...
if actual != expected or len(mounts) != 2:
    raise SystemExit("bind probe mount graph differs from the exact host sources")
PY

host_inode() { stat -f%i "$FILE"; }
host_size() { stat -f%z "$FILE"; }
host_sha() { shasum -a 256 "$FILE" | awk '{print $1}'; }
guest_size() { docker_e exec "$container_id" stat -c %s /work/value.bin 2>/dev/null | tr -d '\r'; }
guest_sha() { docker_e exec "$container_id" sha256sum /work/value.bin 2>/dev/null | awk '{print $1}'; }
guest_direct_size() { docker_e exec "$container_id" stat -c %s /single/value.bin 2>/dev/null | tr -d '\r'; }
guest_direct_sha() { docker_e exec "$container_id" sha256sum /single/value.bin 2>/dev/null | awk '{print $1}'; }
...
docker_e exec "$container_id" sh -c "printf xyz > /single/value.bin"
...
for cycle in $(seq 1 "$DIRECT_CYCLES"); do
  docker_e run --rm --pull=never --network none \
    ...
    --mount "type=bind,src=$FILE,dst=/single/value.bin" "$IMAGE" \
...
[ -z "$(docker_e ps -aq --filter "label=$LABEL")" ] \
  || die "owned bind probe container survived cleanup"
