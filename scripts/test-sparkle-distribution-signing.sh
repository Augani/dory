...
PATH="$BIN:$PATH" scripts/verify-distribution-signatures.sh "$APP" TESTTEAM01 >/dev/null
if PATH="$BIN:$PATH" DORY_TEST_ADHOC_BASENAME=Autoupdate \
  scripts/verify-distribution-signatures.sh "$APP" TESTTEAM01 >"$TMP/adhoc.out" 2>&1; then
...
ln -s "$APP" "$TMP/Indirect.app"
if PATH="$BIN:$PATH" scripts/sign-sparkle-for-distribution.sh \
  "$TMP/Indirect.app" 'Developer ID Application' >"$TMP/indirect-sign.out" 2>&1; then
  echo "test-sparkle-distribution-signing: signer accepted an indirect app" >&2
  exit 1
fi
if PATH="$BIN:$PATH" scripts/verify-distribution-signatures.sh \
  "$TMP/Indirect.app" TESTTEAM01 >"$TMP/indirect-verify.out" 2>&1; then
  echo "test-sparkle-distribution-signing: verifier accepted an indirect app" >&2
  exit 1
fi
