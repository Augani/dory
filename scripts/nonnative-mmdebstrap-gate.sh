...
[ "$RELEASE_CANDIDATE" -eq 0 ] || [ -n "$SOURCE_COMMIT" ] \
  || die "release candidate evidence requires --source-commit"
...
if not isinstance(payload, list) or len(payload) != 1:
    raise SystemExit("Debian image inspect is not singular")
image = payload[0]
if image.get("Os") != "linux":
    raise SystemExit("Debian fixture is not a Linux image")
if image.get("Architecture") != "amd64":
    raise SystemExit("Debian fixture is not linux/amd64")
PY
...
payload = json.loads(pathlib.Path(sys.argv[1]).read_text(encoding="utf-8"))
if not isinstance(payload, list) or len(payload) != 1:
    raise SystemExit("built image inspect is not singular")
image = payload[0]
if image.get("Os") != "linux":
    raise SystemExit("built fixture is not a Linux image")
if image.get("Architecture") != "amd64":
    raise SystemExit("built fixture is not linux/amd64")
PY
built_image_id="$(docker_e image inspect -f '{{.Id}}' "$TAG")"
