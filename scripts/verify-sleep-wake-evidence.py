...
def direct_path(path: pathlib.Path, label: str, *, directory: bool) -> pathlib.Path:
    logical = pathlib.Path(os.path.abspath(path))
    require(not logical.is_symlink(), f"{label} is indirect")
    require(logical.is_dir() if directory else logical.is_file(), f"{label} is missing")
...
def load_evidence_json(path: pathlib.Path, evidence_root: pathlib.Path, label: str) -> object:
    require(path.is_file() and not path.is_symlink(), f"{label} is missing or indirect")
    require(path.resolve().is_relative_to(evidence_root), f"{label} escapes the evidence root")
    require(path.stat().st_size <= 2 * 1024 * 1024, f"{label} is too large")
    return json.loads(path.read_bytes(), object_pairs_hook=unique_json_object)
...
def main() -> None:
    ...
    args.evidence_root = direct_path(args.evidence_root, "evidence root", directory=True)
    args.manifest = direct_path(args.manifest, "sleep/wake manifest", directory=False)
    args.results = direct_path(args.results, "sleep/wake results", directory=False)
    args.app = direct_path(args.app, "candidate app", directory=True)
    require(args.manifest.parent == args.evidence_root, "sleep/wake manifest is outside the evidence root")
    require(args.results.parent == args.evidence_root, "sleep/wake results are outside the evidence root")
    require(args.manifest.stat().st_size <= 64 * 1024, "sleep/wake manifest is too large")
    require(args.results.stat().st_size <= 4 * 1024 * 1024, "sleep/wake results are too large")
    for path in args.evidence_root.rglob("*"):
        require(not path.is_symlink(), f"sleep/wake evidence contains a symlink: {path}")
    ...
    for key, path in binaries.items():
        if not path.is_file() or path.is_symlink() or not path.resolve().is_relative_to(args.app) \
                or manifest.get(key) != digest(path):
    ...
    with route_results_path.open(encoding="utf-8", newline="") as handle:
        ...
        require(reader.fieldnames == ["round", "phase", "status", "detail"], "route-churn results schema mismatch")
    ...
    if not rows or reader.fieldnames != expected_fields:
    ...
    for cycle in range(1, args.cycles + 1):
        ...
        for suffix, state in status_files.items():
            ...
            try:
                payload = load_evidence_json(path, args.evidence_root, f"cycle {cycle} {suffix}")
...
if __name__ == "__main__":
    try:
        ...
        raise SystemExit(f"sleep/wake evidence error: {error}") from error
