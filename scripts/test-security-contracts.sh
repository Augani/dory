...
grep -F 'create_args+=(--sandbox-expires-at "$expires_epoch")' scripts/dory >/dev/null \
  || fail "sandbox expiry is not sent through typed machine authority"
grep -F 'let expiration = policy.expiresAtUnixSeconds' \
  dory-core-swift/Sources/DorydKit/SandboxTTLReconciler.swift >/dev/null \
  || fail "daemon does not reconcile typed sandbox expiry"
grep -F 'legacyExpirationEnvironmentKey = "DORY_SANDBOX_EXPIRES_AT"' \
  dory-core-swift/Sources/DoryOperations/DoryVirtualMachineSandboxPolicy.swift >/dev/null \
  || fail "sandbox expiry lost legacy read compatibility"
