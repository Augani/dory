...
[ -d "$APP" ] && [ ! -L "$APP" ] && [ "$(basename "$APP")" = Dory.app ] \
  || release_error "input must be a direct Dory.app directory"
APP_LOGICAL="$(python3 -c 'import os,sys; print(os.path.abspath(sys.argv[1]))' "$APP")"
APP="$(cd "$APP" && pwd -P)"
[ "$APP" = "$APP_LOGICAL" ] || release_error "input app has an indirect ancestor"
[ -n "$EXPECTED_ARCHES" ] || release_error "expected architectures are required"
for expected_arch in $EXPECTED_ARCHES; do
  case "$expected_arch" in arm64|x86_64) ;; *) release_error "unsupported expected architecture: $expected_arch" ;; esac
done
