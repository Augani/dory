...
DESKTOP_DEBIAN_UPDATE="${DORY_RELEASE_DESKTOP_DEBIAN_UPDATE:-}"
...
is_digest_reference() {
  printf '%s\n' "$1" | grep -Eq '^[A-Za-z0-9][A-Za-z0-9._:/-]*@sha256:[0-9a-f]{64}$'
}
...
printf '%s\n' "$SOURCE_COMMIT" | grep -Eq '^[0-9a-f]{40}$' \
  || fail "live qualification requires an exact source commit"
case "$LOG_ROOT" in /*) ;; *) fail "release evidence root must be absolute" ;; esac
case "$LOG_ROOT" in /|"$HOME"|"$ROOT") fail "unsafe release evidence root: $LOG_ROOT" ;; esac
[ ! -e "$LOG_ROOT" ] && [ ! -L "$LOG_ROOT" ] \
  || fail "release evidence root already exists or is indirect: $LOG_ROOT"
is_digest_reference "$FIXTURE_IMAGE" \
  || fail "DORY_RELEASE_FIXTURE_IMAGE must be an exact digest reference"
is_digest_reference "$NONNATIVE_BUILD_IMAGE" \
  || fail "DORY_RELEASE_NONNATIVE_BUILD_IMAGE must be an exact digest reference"
...
if [ "$REQUIRED_ARCH" = arm64 ]; then
  ...
  is_digest_reference "$LOCK_IMAGE" \
    || fail "DORY_RELEASE_LOCK_IMAGE must be a digest-pinned image containing python3"
  is_digest_reference "$SSH_CLIENT_IMAGE" \
    || fail "DORY_RELEASE_SSH_CLIENT_IMAGE must be a digest-pinned image containing ssh-add"
  for asset in "$DESKTOP_KERNEL" "$DESKTOP_DEBIAN_ROOTFS" \
    "$DESKTOP_UBUNTU_ROOTFS" "$DESKTOP_KALI_ROOTFS" "$DESKTOP_DEBIAN_UPDATE" \
    "$DESKTOP_UBUNTU_UPDATE" "$DESKTOP_KALI_UPDATE"; do
...
[ -d "$APP" ] && [ ! -L "$APP" ] \
  || fail "candidate app is unavailable or indirect: $APP"
for helper in \
...
if [ "$REQUIRE_NOTARIZED" = 1 ]; then
  ...
  assessment="$(spctl --assess --type execute --verbose=4 "$APP" 2>&1)" \
    || fail "Gatekeeper rejected the candidate app: $assessment"
  grep -q '^source=Notarized Developer ID$' <<< "$assessment" \
    || fail "candidate is not accepted as Notarized Developer ID"
fi
...
[ ! -e "$PLIST" ] && [ ! -L "$PLIST" ] \
  || fail "existing Dory LaunchAgent would be overwritten: $PLIST"
[ ! -e "$STATE" ] && [ ! -L "$STATE" ] \
  || fail "existing Dory state would be touched: $STATE"
[ ! -e "$APP_SUPPORT" ] && [ ! -L "$APP_SUPPORT" ] \
  || fail "existing Dory application state would be touched: $APP_SUPPORT"
[ ! -e "$PREF_PLIST" ] && [ ! -L "$PREF_PLIST" ] \
  || fail "existing Dory preference file would be touched: $PREF_PLIST"
...
for profile in "${PROFILE_PATHS[@]}"; do
  [ ! -L "$profile" ] || fail "shell profile must not be a symlink on the release user: $profile"
...
[ -d "$LOG_ROOT" ] && [ ! -L "$LOG_ROOT" ] \
  || fail "release evidence root changed while it was being prepared"
trap cleanup EXIT
trap 'exit 130' INT
trap 'exit 143' TERM
if [ "$REQUIRED_ARCH" = arm64 ]; then
...
[ "$(stat -f %u "$STATE/dory.sock")" = "$(id -u)" ] \
  || fail "candidate Docker socket is not owned by the release user"
for image in "$FIXTURE_IMAGE" "$NONNATIVE_BUILD_IMAGE"; do
  "$DOCKER_CLI" -H "unix://$STATE/dory.sock" image inspect "$image" >/dev/null 2>&1 \
    || fail "required offline release fixture is missing: $image"
done
...
if [ "$REQUIRED_ARCH" = arm64 ]; then
  ...
  scripts/desktop-linux-live-gate.sh \
    ...
    --kali-rootfs "$DESKTOP_KALI_ROOTFS" \
  ...
  scripts/sandbox-security-gate.sh \
    --dory "$DORY_CLI" \
    --workroot "$LOG_ROOT/sandbox-security"
  ...
  if [ "$RUN_PHYSICAL_SLEEP" = 1 ]; then
    DORY_NETWORK_INTEGRITY_SOURCE_COMMIT="$SOURCE_COMMIT" \
...
if [ "$REQUIRE_PHYSICAL_INTEL" = 1 ]; then
  ...
  READINESS_WORKDIR="$LOG_ROOT/readiness" \
  DORY_SOCK="$STATE/dory.sock" \
  READINESS_STRICT=1 \
...

{
  ...
  echo "required_arch=$REQUIRED_ARCH"
  echo "notarized_required=$REQUIRE_NOTARIZED"
  echo "physical_sleep_required=$RUN_PHYSICAL_SLEEP"
