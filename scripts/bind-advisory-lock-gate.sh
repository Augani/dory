...
case "$WORKROOT" in /*) ;; *) fail "--workroot must be absolute" ;; esac
case "$WORKROOT" in /|"$HOME"|"$ROOT") fail "unsafe --workroot: $WORKROOT" ;; esac
[ ! -L "$WORKROOT" ] || fail "--workroot must not be a symlink"
[ -S "$SOCKET" ] || fail "Dory socket is unavailable: $SOCKET"
[ "$(stat -f %u "$SOCKET")" = "$(id -u)" ] \
  || fail "Dory socket is not owned by the release user"
...
[ -f "$DOCKER" ] && [ ! -L "$DOCKER" ] && [ -x "$DOCKER" ] \
  || fail "resolved Docker CLI is not an exact regular executable: $DOCKER"
printf '%s\n' "$IMAGE" | grep -Eq '^.+@sha256:[0-9a-f]{64}$' \
  || fail "--image must be digest-pinned"
[ -f "$ROOT/scripts/bind-advisory-lock-probe.py" ] \
  && [ ! -L "$ROOT/scripts/bind-advisory-lock-probe.py" ] \
  || fail "bind advisory lock probe is unavailable"
...
BIND_ROOT="$(mktemp -d "$HOME/.dory-bind-lock-gate.XXXXXXXX")"
EVIDENCE="$WORKROOT/$RUN"
mkdir -p "$BIND_ROOT" "$EVIDENCE"
[ -d "$BIND_ROOT" ] && [ ! -L "$BIND_ROOT" ] \
  && [ -d "$WORKROOT" ] && [ ! -L "$WORKROOT" ] \
  && [ -d "$EVIDENCE" ] && [ ! -L "$EVIDENCE" ] \
  || fail "gate workspace changed while preparing fixtures"
