...
Options:
  ...
  --release-candidate Require the full >24-hour release contract
  --help              Show this help
...
if [ "$RELEASE_CANDIDATE" -eq 1 ]; then
  ...
  [ "$DURATION" -gt 86400 ] || die "release candidate duration must exceed 24 hours"
...
image_platform="$(docker_e image inspect -f '{{.Os}}/{{.Architecture}}' "$IMAGE")"
...
[ "$(docker_e image inspect -f '{{.Id}}' "$IMAGE")" = "$image_id" ] \
  || die "long-lived soak changed the exact fixture image identity"
...
require(bool(rows), "managed-machine latency evidence has no samples")
require(all(row.get("status") == "PASS" for row in rows),
        "managed-machine latency evidence contains a failed sample")
values = [int(row["rtt_ms"]) for row in rows]
require(all(value >= 0 for value in values), "managed-machine latency evidence has a negative RTT")
expected_samples = max(1, duration // interval)
require(len(values) >= max(1, math.floor(expected_samples * 0.90)),
...
require(elapsed >= duration, f"managed-machine latency measurement ended early after {elapsed:.3f}s")
...
require(p99 <= 100, f"managed-machine p99 protocol RTT exceeded 100ms: {p99}ms")
require(over_100 <= allowed_over_100,
...
require(len(outbound_rows) == len(rows),
...
require(outbound_failures <= allowed_outbound_failures,
...
require(max_failure_run < 2,
        f"managed-machine outbound TCP had consecutive failures ({max_failure_run})")
require(all(int(row["rtt_ms"]) >= 0 for row in outbound_rows),
...
set -e
trap - EXIT INT TERM
docker_e inspect "$NAME" >/dev/null 2>&1 && die "service fixture survived owned cleanup"
docker_e inspect "$MACHINE_NAME" >/dev/null 2>&1 && die "managed-machine fixture survived owned cleanup"
release_qualifying=false
if [ "$RELEASE_CANDIDATE" -eq 1 ] && [ "$beyond_24_hours" = PASS ]; then
...
{
  echo "heartbeats_sha256=$(shasum -a 256 "$RESULTS" | awk '{print $1}')"
  echo "machine_service_sha256=$(shasum -a 256 "$MACHINE_RESULTS" | awk '{print $1}')"
  echo "machine_outbound_sha256=$(shasum -a 256 "$OUTBOUND_RESULTS" | awk '{print $1}')"
  echo "summary_sha256=$(shasum -a 256 "$WORKDIR/summary.txt" | awk '{print $1}')"
