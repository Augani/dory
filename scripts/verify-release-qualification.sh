...
[ ! -L "$BUILD_DIR" ] || die "build directory is indirect: $BUILD_DIR"
[ ! -L "$QUALIFICATION" ] || die "qualification directory is indirect: $QUALIFICATION"
BUILD_DIR_LOGICAL="$(python3 -c 'import os,sys; print(os.path.abspath(sys.argv[1]))' "$BUILD_DIR")"
QUALIFICATION_LOGICAL="$(python3 -c 'import os,sys; print(os.path.abspath(sys.argv[1]))' "$QUALIFICATION")"
BUILD_DIR="$(cd "$BUILD_DIR" && pwd -P)"
QUALIFICATION="$(cd "$QUALIFICATION" && pwd -P)"
[ "$BUILD_DIR" = "$BUILD_DIR_LOGICAL" ] || die "build directory has an indirect ancestor"
[ "$QUALIFICATION" = "$QUALIFICATION_LOGICAL" ] || die "qualification directory has an indirect ancestor"
...
[ "$(git rev-parse HEAD)" = "$SOURCE_COMMIT" ] \
  || die "checked-out source does not match --source-commit"
git diff --quiet -- scripts .github/scripts \
  || die "tracked qualification verifier differs from the checked-out source commit"
git diff --cached --quiet -- scripts .github/scripts \
  || die "staged qualification verifier differs from the checked-out source commit"
VERIFIER_SHA256="$(shasum -a 256 scripts/verify-release-qualification.sh | awk '{print $1}')"
AUTHORITY_VALIDATOR_SHA256="$(shasum -a 256 scripts/validate-release-qualification.py | awk '{print $1}')"
...
python3 scripts/validate-release-qualification.py \
  "$BUILD_DIR" "$QUALIFICATION" "$VERSION" "$BUILD" "$SOURCE_COMMIT" \
  "$RUN_ID" "$RUN_ATTEMPT" "$PRIMARY_SHA256" \
  || die "durable schema-2 qualification authority is invalid"
...
if not (isinstance(inspect, list) and len(inspect) == 1):
    raise SystemExit('retained image inspect is not singular')
image = inspect[0]
if not (image.get('Os') == 'linux' and image.get('Architecture') == 'arm64'):
    raise SystemExit('retained default image is not linux/arm64')
image_id = image.get('Id')
inspect_size = int(image.get('Size', 0))
images = json.loads(pathlib.Path(images_path).read_text(encoding='utf-8'))
if not (len(images) == 1 and images[0].get('Id') == image_id):
    raise SystemExit('retained fresh image list is not one selected image')
...
if not (len(df_images) == 1 and df_images[0].get('Id') == image_id):
    raise SystemExit('retained fresh system-df is not one selected image')
df_size = int(df_images[0].get('Size', 0))
layers_size = int(system_df.get('LayersSize', 0))
if not (inspect_size > 0 and list_size == df_size):
    raise SystemExit('retained image-list/system-df bytes disagree')
ratio = max(inspect_size, df_size) * 1000 // min(inspect_size, df_size)
if not ratio <= 16000:
    raise SystemExit('retained inspect/storage definitions diverge by more than 16x')
if not df_size <= layers_size <= df_size * 2:
    raise SystemExit('retained layer bytes are not attributable to the one local image')
if not pathlib.Path(uname_path).read_text(encoding='utf-8').strip() in {'aarch64', 'arm64'}:
    raise SystemExit('retained default run is not arm64')
if not properties.get('image_id') == image_id:
    raise SystemExit('retained image ID differs from the manifest')
if not int(properties.get('inspect_size_bytes', -1)) == inspect_size:
    raise SystemExit('qualification evidence check failed')
if not int(properties.get('image_list_size_bytes', -1)) == list_size:
    raise SystemExit('qualification evidence check failed')
if not int(properties.get('system_df_size_bytes', -1)) == df_size:
    raise SystemExit('qualification evidence check failed')
if not int(properties.get('system_df_layers_size_bytes', -1)) == layers_size:
    raise SystemExit('qualification evidence check failed')
if not int(properties.get('inspect_to_storage_ratio_milli', -1)) == ratio:
    raise SystemExit('qualification evidence check failed')
PY
...
if not not (report.get('BuildCache') or []):
    raise SystemExit('retained prune evidence contains build cache')
PY
...
if not (isinstance(payload, list) and len(payload) == 1):
    raise SystemExit('registry image inspect is not singular')
image = payload[0]
if not (image.get('Os') == 'linux' and image.get('Architecture') == 'arm64'):
    raise SystemExit('registry fixture is not linux/arm64')
digest = sys.argv[2].rsplit('@', 1)[1]
if not any((value.endswith('@' + digest) for value in image.get('RepoDigests') or [])):
    raise SystemExit('registry fixture inspect does not contain the qualified digest')
PY
...
properties = dict((line.split('=', 1) for line in manifest.read_text(encoding='utf-8').splitlines() if '=' in line))
digest = properties['image'].rsplit('@', 1)[1]
payload = json.loads(inspect_path.read_text(encoding='utf-8'))
if not (isinstance(payload, list) and len(payload) == 1):
    raise SystemExit('retained Nix image inspect is not singular')
image = payload[0]
if not (image.get('Os') == 'linux' and image.get('Architecture') == 'amd64'):
    raise SystemExit('retained Nix image is not linux/amd64')
if not any((value.endswith('@' + digest) for value in image.get('RepoDigests') or [])):
    raise SystemExit('retained Nix image digest differs from the manifest')
PY
...
payload = json.loads(inspect_path.read_text(encoding='utf-8'))
if not (isinstance(payload, list) and len(payload) == 1):
    raise SystemExit('retained Arch inspect is not singular')
image = payload[0]
if not (image.get('Os') == 'linux' and image.get('Architecture') == 'amd64'):
    raise SystemExit('retained Arch image is not linux/amd64')
if not any((value.endswith('@' + digest) for value in image.get('RepoDigests') or [])):
    raise SystemExit('retained Arch digest differs from the manifest')
PY
...
properties = dict((line.split('=', 1) for line in manifest.read_text().splitlines() if '=' in line))
for (key, path, architecture) in (('base_image', sys.argv[2], 'amd64'), ('native_image', sys.argv[3], 'arm64')):
    digest = properties[key].rsplit('@', 1)[1]
    payload = json.loads(pathlib.Path(path).read_text())
    if not (isinstance(payload, list) and len(payload) == 1):
        raise SystemExit('qualification evidence check failed')
    image = payload[0]
    if not (image.get('Os') == 'linux' and image.get('Architecture') == architecture):
        raise SystemExit('qualification evidence check failed')
    if not any((value.endswith('@' + digest) for value in image.get('RepoDigests') or [])):
        raise SystemExit('qualification evidence check failed')
PY
...
expected = ['initial', 'same-inode-shrink', 'same-inode-grow', 'same-inode-content', 'atomic-replacement-pinned-direct', 'direct-rebind-after-replacement', 'guest-truncate']
if not [row['phase'] for row in rows] == expected:
    raise SystemExit('unexpected bind coherence phases')
for (index, row) in enumerate(rows):
    if not row['host_size'] == row['guest_directory_size']:
        raise SystemExit(f"directory size divergence in {row['phase']}")
    if not row['host_sha256'] == row['guest_directory_sha256']:
        raise SystemExit(f"directory content divergence in {row['phase']}")
    if row['phase'] == 'atomic-replacement-pinned-direct':
        previous = rows[index - 1]
        if not row['guest_direct_size'] == previous['guest_direct_size']:
            raise SystemExit('direct bind did not retain the pre-replacement inode size')
        if not row['guest_direct_sha256'] == previous['guest_direct_sha256']:
            raise SystemExit('direct bind did not retain the pre-replacement inode content')
        if not row['guest_direct_sha256'] != row['host_sha256']:
            raise SystemExit('atomic replacement fixture did not distinguish direct and directory views')
    else:
        if not row['host_size'] == row['guest_direct_size']:
            raise SystemExit(f"direct size divergence in {row['phase']}")
        if not row['host_sha256'] == row['guest_direct_sha256']:
            raise SystemExit(f"direct content divergence in {row['phase']}")
PY
...
grep -qx 'gvproxy_version=v0.8.9-dory2' "$native_ipv6_manifest" \
...
testcontainers_version="$(sed -n 's/^testcontainers_version=//p' "$testcontainers_manifest")"
printf '%s\n' "$testcontainers_version" | grep -Eq '^[0-9]+\.[0-9]+\.[0-9]+' \
  || die "retained Testcontainers evidence has no exact npm version"
testcontainers_ryuk_image="$(sed -n 's/^ryuk_image=//p' "$testcontainers_manifest")"
printf '%s\n' "$testcontainers_ryuk_image" | grep -Eq '^.+@sha256:[0-9a-f]{64}$' \
  || die "retained Testcontainers evidence has no digest-pinned Ryuk image"
...
act_runner_image="$(sed -n 's/^runner_image=//p' "$act_manifest")"
[ -n "$act_runner_image" ] || die "retained act evidence omits its runner image"
...
import math
import sys
(growth_path, competitor_path, long_path, long_machine_path, long_outbound_path, endurance_path, power_manifest_path, power_history_path, power_assertions_path, power_source_path) = sys.argv[1:]
...
for key in ('seed_allocated_bytes', 'grown_logical_bytes', 'grown_allocated_bytes', 'minimum_logical_bytes', 'guest_ext4_bytes', 'minimum_guest_bytes'):
    if not key in growth:
        raise SystemExit(f'growth evidence omits {key}')
if not int(growth['grown_logical_bytes']) >= int(growth['minimum_logical_bytes']):
    raise SystemExit('sparse disk did not reach the required logical capacity')
if not int(growth['grown_allocated_bytes']) < int(growth['grown_logical_bytes']):
    raise SystemExit('grown disk is not sparse')
if not int(growth['grown_allocated_bytes']) < int(growth['seed_allocated_bytes']):
    raise SystemExit('discard did not reclaim the preallocated seed')
if not int(growth['guest_ext4_bytes']) >= int(growth['minimum_guest_bytes']):
    raise SystemExit('guest ext4 capacity stayed below its required floor')
required_competitor_tests = {'published-port-handoff', 'host-port-collision', 'named-signal-delivery', 'container-api-lifecycle', 'forwarded-connection-fds', 'concurrent-proxy-backpressure', 'missing-source-cp', 'restart-churn', 'compose-port-restart', 'compose-v2-lifecycle', 'network-route-conflict', 'network-api-lifecycle', 'standalone-engine-restart', 'named-volume-empty', 'named-volume', 'named-volume-cp', 'volume-api-lifecycle', 'security-opt-label', 'seccomp-profile', 'bind-open-create-0200', 'bind-mount-option-contract', 'healthcheck', 'buildx-named-context', 'buildkit-default-arg', 'image-save-stdout', 'image-hardlink-missing-parent', 'buildkit-large-dockerfile', 'buildkit-concurrent-sessions', 'buildkit-cache-cancellation', 'container-resolver-contract', 'container-dns-search', 'cleanup-restart-persistence'}
...
if not not missing:
    raise SystemExit(f'competitor evidence omits required tests: {sorted(missing)}')
if not all((by_test[name] == 'PASS' for name in required_competitor_tests)):
    raise SystemExit('one or more required competitor tests did not pass')
long = properties(long_path)
if not float(long.get('actual_elapsed_seconds', '0')) > 86400:
    raise SystemExit('same-connection evidence did not actually cross 24 hours')
if not int(long.get('heartbeats', '0')) > 0:
    raise SystemExit('same-connection evidence has no heartbeats')
if not int(long.get('unique_connection_tuples', '0')) == 1:
    raise SystemExit('same-connection evidence changed its TCP tuple')
if not long.get('machine_to_docker_service') == 'PASS':
    raise SystemExit('machine-to-Docker service route did not pass')
if not long.get('machine_service_route') == 'host.docker.internal':
    raise SystemExit('machine-to-Docker service used the wrong route')
if not float(long.get('machine_service_actual_elapsed_seconds', '0')) > 86400:
    raise SystemExit('machine-to-Docker latency evidence did not actually cross 24 hours')
with open(long_machine_path, encoding='utf-8', newline='') as handle:
    machine_rows = list(csv.DictReader(handle, delimiter='\t'))
if not len(machine_rows) == int(long.get('machine_service_samples', '0')):
    raise SystemExit('machine-to-Docker sample count differs from the summary')
if not len(machine_rows) >= 2592:
    raise SystemExit('machine-to-Docker evidence retains fewer than 90% of 25-hour/30-second samples')
if not all((row.get('status') == 'PASS' for row in machine_rows)):
    raise SystemExit('machine-to-Docker latency evidence contains a failed sample')
...
if not machine_p99 <= 100:
    raise SystemExit(f'machine-to-Docker p99 exceeds 100ms: {machine_p99}ms')
if not machine_over_100 <= max(1, math.floor(len(machine_values) * 0.01)):
    raise SystemExit('machine-to-Docker RTT exceeds 100ms too frequently')
run = 0
max_run = 0
...
if not max_run < 3:
    raise SystemExit('machine-to-Docker RTT contains a sustained >=150ms plateau')
if not int(long.get('machine_service_p99_ms', '-1')) == machine_p99:
    raise SystemExit('machine-to-Docker p99 differs from the raw evidence')
if not int(long.get('machine_service_max_ms', '-1')) == max(machine_values):
    raise SystemExit('machine-to-Docker maximum differs from the raw evidence')
if not int(long.get('machine_service_over_100ms_samples', '-1')) == machine_over_100:
    raise SystemExit('machine-to-Docker >100ms count differs from the raw evidence')
if not int(long.get('machine_service_200ms_samples', '-1')) == machine_200:
    raise SystemExit('machine-to-Docker >=200ms count differs from the raw evidence')
if not int(long.get('machine_service_max_sustained_150ms_samples', '-1')) == max_run:
    raise SystemExit('machine-to-Docker sustained-latency run differs from the raw evidence')
if not long.get('machine_outbound_tcp') == 'PASS':
    raise SystemExit('managed-machine outbound TCP qualification did not pass')
with open(long_outbound_path, encoding='utf-8', newline='') as handle:
    outbound_rows = list(csv.DictReader(handle, delimiter='\t'))
if not len(outbound_rows) == int(long.get('machine_outbound_samples', '0')):
    raise SystemExit('managed-machine outbound sample count differs from the summary')
if not len(outbound_rows) == len(machine_rows):
    raise SystemExit('managed-machine outbound sample count differs from the local-route count')
...
if not outbound_failures <= max(1, math.floor(len(outbound_rows) * 0.005)):
    raise SystemExit('managed-machine outbound TCP failure budget exceeded')
failure_run = 0
max_failure_run = 0
for row in outbound_rows:
    failure_run = failure_run + 1 if row.get('status') != 'PASS' else 0
    max_failure_run = max(max_failure_run, failure_run)
if not max_failure_run < 2:
    raise SystemExit('managed-machine outbound TCP has consecutive failures')
if not all((len(row.get('remote_ipv4', '').split('.')) == 4 and all((part.isdigit() and 0 <= int(part) <= 255 for part in row['remote_ipv4'].split('.'))) for row in outbound_rows if row.get('status') == 'PASS')):
    raise SystemExit('managed-machine outbound PASS row lacks a valid IPv4 target')
if not int(long.get('machine_outbound_tcp_successes', '-1')) == outbound_successes:
    raise SystemExit('managed-machine outbound success count differs from raw evidence')
if not int(long.get('machine_outbound_timeout_samples', '-1')) == outbound_failures:
    raise SystemExit('managed-machine outbound failure count differs from raw evidence')
if not int(long.get('machine_outbound_max_consecutive_failures', '-1')) == max_failure_run:
    raise SystemExit('managed-machine outbound failure run differs from raw evidence')
endurance = properties(endurance_path)
if not int(endurance.get('duration_seconds', '0')) >= 28800:
    raise SystemExit('endurance evidence was configured below eight hours')
power_manifest = properties(power_manifest_path)
if not power_manifest == {'status': 'PASS', 'flags': '-is', 'display_sleep_prevented': 'false'}:
    raise SystemExit('power assertion manifest is not the display-neutral -is contract')
with open(power_source_path, encoding='utf-8') as handle:
    if not "Now drawing from 'AC Power'" in handle.read():
        raise SystemExit('qualification did not start on AC power')
with open(power_assertions_path, encoding='utf-8') as handle:
    assertions = handle.read()
if not ('pid ' in assertions and '(caffeinate)' in assertions):
    raise SystemExit('power evidence is not owned by caffeinate')
if not 'PreventUserIdleSystemSleep' in assertions:
    raise SystemExit('idle-sleep assertion is missing')
if not 'PreventSystemSleep' in assertions:
    raise SystemExit('system-sleep assertion is missing')
with open(power_history_path, encoding='utf-8', newline='') as handle:
    power_rows = list(csv.DictReader(handle, delimiter='\t'))
if not len(power_rows) >= 2:
    raise SystemExit('continuous power evidence has fewer than two samples')
timestamps = []
for row in power_rows:
    if not "Now drawing from 'AC Power'" in row['pmset_state']:
        raise SystemExit('qualification lost AC power')
...
if not (timestamps[-1] - timestamps[0]).total_seconds() >= required_power_span:
    raise SystemExit('power assertion history does not span the same-connection qualification')
PY

[ "$(shasum -a 256 scripts/verify-release-qualification.sh | awk '{print $1}')" = \
    "$VERIFIER_SHA256" ] \
  || die "qualification verifier bytes changed during verification"
[ "$(shasum -a 256 scripts/validate-release-qualification.py | awk '{print $1}')" = \
    "$AUTHORITY_VALIDATOR_SHA256" ] \
  || die "qualification authority validator changed during verification"
[ "$(git rev-parse HEAD)" = "$SOURCE_COMMIT" ] \
  || die "checked-out source changed during qualification verification"
git diff --quiet -- scripts .github/scripts \
  || die "tracked qualification verifier changed during verification"
git diff --cached --quiet -- scripts .github/scripts \
  || die "staged qualification verifier changed during verification"
python3 scripts/validate-release-qualification.py \
  "$BUILD_DIR" "$QUALIFICATION" "$VERSION" "$BUILD" "$SOURCE_COMMIT" \
  "$RUN_ID" "$RUN_ATTEMPT" "$PRIMARY_SHA256" \
  || die "durable qualification authority changed during semantic verification"
