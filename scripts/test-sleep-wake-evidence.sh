...
cp -R "$EVIDENCE" "$TMP/indirect-evidence"
rm "$TMP/indirect-evidence/manifest.txt"
ln -s "$EVIDENCE/manifest.txt" "$TMP/indirect-evidence/manifest.txt"
expect_failure "$TMP/indirect-evidence" "a symlinked retained manifest"

cp -R "$EVIDENCE" "$TMP/duplicate-json"
printf '{"id":"dory-sleep-session-20260712T120000Z-123","state":"stopped","state":"running"}\n' \
  > "$TMP/duplicate-json/cycle-2-machine-status-stopped.json"
expect_failure "$TMP/duplicate-json" "duplicate JSON authority keys"
