...
direct_existing() {
  local path="$1" kind="$2" logical physical
  [ -e "$path" ] && [ ! -L "$path" ] || die "required path is unavailable or indirect: $path"
  logical="$(python3 -c 'import os,sys; print(os.path.abspath(sys.argv[1]))' "$path")"
  physical="$(cd "$(dirname "$path")" && pwd -P)/$(basename "$path")"
  [ "$logical" = "$physical" ] || die "required path has an indirect ancestor: $path"
  case "$kind" in
    directory) [ -d "$physical" ] || die "required directory is invalid: $path" ;;
    file) [ -f "$physical" ] || die "required file is invalid: $path" ;;
    *) die "internal path-kind error" ;;
  esac
  printf '%s\n' "$physical"
}
CANDIDATE_APP="$(direct_existing "$CANDIDATE_APP" directory)"
UPDATE_ZIP="$(direct_existing "$UPDATE_ZIP" file)"
APPCAST="$(direct_existing "$APPCAST" file)"
RELEASE_MANIFEST="$(direct_existing "$RELEASE_MANIFEST" file)"
SBOM="$(direct_existing "$SBOM" file)"
SPARKLE_SOURCE="$(direct_existing "$SPARKLE_SOURCE" directory)"
...
if len(rows) != 1:
    raise SystemExit("Package.resolved must contain one Sparkle pin")
...
[ -z "$(git -C "$SPARKLE_SOURCE" status --porcelain --untracked-files=all)" ] \
  || die "Sparkle source checkout is not exact and clean"

case "$WORKROOT" in /*) ;; *) die "workroot must be absolute" ;; esac
WORKROOT_PARENT="$(dirname "$WORKROOT")"
WORKROOT_NAME="$(basename "$WORKROOT")"
[ "$WORKROOT_NAME" = dory-release-live-sparkle ] \
  || die "workroot must use the dedicated dory-release-live-sparkle name"
...
case "$TEMP_AUTHORITY" in /|"$HOME"|"$ROOT"|"$CANDIDATE_APP"|"$SPARKLE_SOURCE") die "unsafe temporary authority root" ;; esac
case "$WORKROOT" in "$TEMP_AUTHORITY"/*) ;; *) die "workroot must be inside runner temporary storage" ;; esac
for input in "$CANDIDATE_APP" "$UPDATE_ZIP" "$APPCAST" "$RELEASE_MANIFEST" "$SBOM" "$SPARKLE_SOURCE"; do
  case "$input/" in "$WORKROOT/"*) die "workroot cannot contain a candidate input" ;; esac
  case "$WORKROOT/" in "$input/"*) die "workroot cannot be inside a candidate input" ;; esac
done
if [ -e "$WORKROOT" ] || [ -L "$WORKROOT" ]; then
  [ -d "$WORKROOT" ] && [ ! -L "$WORKROOT" ] \
    || die "workroot must not be a symlink or non-directory"
  [ "$(stat -f '%u' "$WORKROOT")" = "$(id -u)" ] \
    || die "workroot is not owned by the release user"
else
  mkdir "$WORKROOT"
fi
chmod 700 "$WORKROOT"
...
metadata_commit="$(python3 "$ROOT/scripts/validate-release-metadata.py" \
  "$(dirname "$RELEASE_MANIFEST")" "$VERSION" "$BUILD")" \
  || die "candidate release metadata is invalid"
[ "$metadata_commit" = "$SOURCE_COMMIT" ] || die "candidate metadata source commit mismatch"
...
require(manifest.get("schemaVersion") == 2, "release manifest schema mismatch")
require(manifest.get("version") == version, "release manifest version mismatch")
require(str(manifest.get("build")) == build, "release manifest build mismatch")
require(manifest.get("sourceCommit") == source, "release manifest source mismatch")
require(manifest.get("publicRelease") is True and manifest.get("notarized") is True,
        "release manifest trust state mismatch")
require(manifest.get("variants") == "arm64", "release manifest architecture mismatch")
records = {row.get("name"): row for row in manifest.get("artifacts", [])}
for path in (update, appcast, sbom):
    name = pathlib.Path(path).name
    require(name in records, f"release manifest omits {name}")
    digest = hashlib.sha256(pathlib.Path(path).read_bytes()).hexdigest()
    require(records[name].get("sha256") == digest, f"release manifest digest mismatch: {name}")
    require(records[name].get("bytes") == os.path.getsize(path), f"release manifest size mismatch: {name}")
PY
...
candidate_assessment="$(spctl --assess --type execute --verbose=4 "$CANDIDATE_APP" 2>&1)" \
  || die "Gatekeeper rejected the exact candidate app"
grep -F 'source=Notarized Developer ID' <<< "$candidate_assessment" >/dev/null \
...
[ ! -e "$RUN_ROOT" ] && [ ! -L "$RUN_ROOT" ] || die "run evidence authority already exists"
mkdir "$RUN_ROOT"
chmod 700 "$RUN_ROOT"
mkdir "$EVIDENCE"
...
if item.findtext(f"{{{uri}}}version") != build:
    raise SystemExit("appcast build mismatch")
if item.findtext(f"{{{uri}}}shortVersionString") != version:
...
if len(base64.b64decode(signature, validate=True)) != 64:
    raise SystemExit("appcast signature shape mismatch")
if int(enclosure.attrib["length"]) != os.path.getsize(archive):
    raise SystemExit("appcast archive size mismatch")
...
! launchctl print "$SERVICE" >/dev/null 2>&1 \
  || die "Dory service survived clean-user cleanup"
for process in Dory doryd dory-hv dory-vmm; do
  ! pgrep -u "$(id -u)" -x "$process" >/dev/null 2>&1 \
    || die "$process survived clean-user cleanup"
done
! "$CANDIDATE_DOCKER" context inspect dory >/dev/null 2>&1 \
...

{
  ...
  echo docker_context_removed=PASS
  echo daemon_processes_stopped=PASS
