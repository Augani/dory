...
Options:
  ...
  --confirm TOKEN     Must be ISOLATED-ENGINE-MANAGED-DATA-DRIVE
  --release-candidate Require digest-pinned, preloaded release inputs
...
CONFIRM=""
RELEASE_CANDIDATE=0
die() { echo "managed data-drive gate: $*" >&2; exit 2; }
...
[ "$CONFIRM" = ISOLATED-ENGINE-MANAGED-DATA-DRIVE ] \
  || die "requires --confirm ISOLATED-ENGINE-MANAGED-DATA-DRIVE"
for command in curl grep id python3 shasum stat uname uuidgen; do
  command -v "$command" >/dev/null || die "required command is missing: $command"
done
[ "$(uname -m)" = arm64 ] || { echo "managed data-drive gate: physical Apple silicon is required" >&2; exit 69; }
case "$RUNTIME" in /*) ;; *) die "runtime must be an absolute path" ;; esac
[ -d "$RUNTIME" ] && [ ! -L "$RUNTIME" ] || die "runtime is unavailable or indirect"
...
for runtime_file in dory-engine bin/dory-hv bin/gvproxy bin/dory-dataplane-proxy \
  share/dory/dory-hv-kernel-arm64.lzfse share/dory/dory-engine-rootfs.ext4.lzfse \
  share/dory/dory-agent-linux-arm64; do
  [ -f "$RUNTIME/$runtime_file" ] && [ ! -L "$RUNTIME/$runtime_file" ] \
    && [ -s "$RUNTIME/$runtime_file" ] \
    || die "runtime member is missing or indirect: $runtime_file"
done
[ -x "$RUNTIME/dory-engine" ] && [ -x "$RUNTIME/bin/dory-hv" ] \
  || die "runtime launch helpers are not executable"
...
case "$WORKROOT" in /|"$HOME"|"$RUNTIME"|"$RUNTIME"/*) die "unsafe workroot: $WORKROOT" ;; esac
...
case "$RUNTIME_HOME" in /*) ;; *) die "isolated runtime HOME must be an absolute path" ;; esac
...
[ ! -e "$RUNTIME_HOME" ] && [ ! -L "$RUNTIME_HOME" ] \
  || { echo "managed data-drive gate: isolated HOME exists or is indirect: $RUNTIME_HOME" >&2; exit 73; }
[ ! -e "$ALIAS_HOME" ] && [ ! -L "$ALIAS_HOME" ] \
  || die "temporary alias path already exists or is indirect: $ALIAS_HOME"
...
if uuid.UUID(manifest["id"]) != expected:
    raise SystemExit("drive manifest identity differs from the expected first-launch authority")
if selection["schemaVersion"] != 2 or selection["phase"] != "ready":
    raise SystemExit("selection authority is not a schema-v2 ready record")
if uuid.UUID(selection["driveID"]) != expected:
    raise SystemExit("selection authority differs from the drive manifest")
PY
}
...
manifest = json.loads(pathlib.Path(sys.argv[1]).read_text())
selection = json.loads(pathlib.Path(sys.argv[2]).read_text())
if uuid.UUID(manifest["id"]) != uuid.UUID(sys.argv[3]):
    raise SystemExit("mismatch fixture changed the actual drive identity")
if selection["phase"] != "provisioning":
    raise SystemExit("mismatch fixture unexpectedly advanced selection authority")
if uuid.UUID(selection["driveID"]) != uuid.UUID(sys.argv[4]):
    raise SystemExit("mismatch fixture changed the expected drive identity")
PY

HOME="$RUNTIME_HOME" "$RUNTIME/dory-engine" start --data-drive "$DRIVE" \
  >"$EVIDENCE/first-start.log" 2>&1
[ -S "$SOCKET" ] && [ ! -L "$SOCKET" ] \
  || die "engine socket is missing or indirect after first start"
...
if ! "$DOCKER" image inspect "$IMAGE" >/dev/null 2>&1; then
  [ "$RELEASE_CANDIDATE" -eq 0 ] \
    || die "release candidate image is not preloaded in the isolated engine"
...
image_id_before="$("$DOCKER" image inspect -f '{{.Id}}' "$IMAGE")"
container_id_before="$("$DOCKER" container inspect -f '{{.Id}}' "$NAME")"
network_id_before="$("$DOCKER" network inspect -f '{{.Id}}' "$NETWORK")"
...
[ "$("$DOCKER" image inspect -f '{{.Id}}' "$IMAGE")" = "$image_id_before" ] \
  || die "transient runtime replacement changed the image identity"
[ "$("$DOCKER" container inspect -f '{{.Id}}' "$NAME")" = "$container_id_before" ] \
  || die "transient runtime replacement changed the container identity"
[ "$("$DOCKER" network inspect -f '{{.Id}}' "$NETWORK")" = "$network_id_before" ] \
  || die "transient runtime replacement changed the network identity"
...
if set(data) != {"createdAt", "id", "kind", "product", "schemaVersion"}:
    raise SystemExit("drive manifest has an unexpected shape")
if data["kind"] != "dev.dory.data-drive" or data["schemaVersion"] != 1 or data["product"] != "Dory":
    raise SystemExit("drive manifest has an unexpected contract")
drive_id = uuid.UUID(data["id"])
if datetime.datetime.fromisoformat(data["createdAt"].replace("Z", "+00:00")).tzinfo is None:
    raise SystemExit("drive manifest timestamp is not timezone-aware")
if selection["schemaVersion"] != 2 or selection["phase"] != "ready":
    raise SystemExit("selection authority has an unexpected shape")
if uuid.UUID(selection["driveID"]) != drive_id:
    raise SystemExit("selection authority differs from the drive manifest")
PY
...
[ "$RELEASE_CANDIDATE" -eq 0 ] || release_qualifying=true
{
  ...
  printf 'dory_engine_sha256=%s\n' "$(shasum -a 256 "$RUNTIME/dory-engine" | awk '{print $1}')"
  printf 'dory_hv_sha256=%s\n' "$(shasum -a 256 "$RUNTIME/bin/dory-hv" | awk '{print $1}')"
  printf 'gvproxy_sha256=%s\n' "$(shasum -a 256 "$RUNTIME/bin/gvproxy" | awk '{print $1}')"
  printf 'dataplane_proxy_sha256=%s\n' \
    "$(shasum -a 256 "$RUNTIME/bin/dory-dataplane-proxy" | awk '{print $1}')"
  printf 'kernel_asset_sha256=%s\n' \
    "$(shasum -a 256 "$RUNTIME/share/dory/dory-hv-kernel-arm64.lzfse" | awk '{print $1}')"
  printf 'rootfs_asset_sha256=%s\n' \
    "$(shasum -a 256 "$RUNTIME/share/dory/dory-engine-rootfs.ext4.lzfse" | awk '{print $1}')"
  printf 'agent_asset_sha256=%s\n' \
    "$(shasum -a 256 "$RUNTIME/share/dory/dory-agent-linux-arm64" | awk '{print $1}')"
  printf 'image_id=%s\n' "$image_id_before"
  printf 'drive_manifest_sha256=%s\n' "$(shasum -a 256 "$EVIDENCE/drive.json" | awk '{print $1}')"
  printf 'selection_authority_sha256=%s\n' "$(shasum -a 256 "$SELECTION_RECORD" | awk '{print $1}')"
