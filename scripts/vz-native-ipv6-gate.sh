...
for command in cmp codesign curl lsof nc netstat python3 shasum spctl ssh ssh-add sudo xcrun; do
  command -v "$command" >/dev/null \
    || { echo "VZ native IPv6 gate: missing command: $command" >&2; exit 69; }
done

direct_file() {
  local path="$1" logical physical
  [ -f "$path" ] && [ ! -L "$path" ] \
    || { echo "VZ native IPv6 gate: input must be a direct file: $path" >&2; exit 66; }
  logical="$(python3 -c 'import os,sys; print(os.path.abspath(sys.argv[1]))' "$path")"
  physical="$(cd "$(dirname "$path")" && pwd -P)/$(basename "$path")"
  [ "$logical" = "$physical" ] \
    || { echo "VZ native IPv6 gate: input has an indirect ancestor: $path" >&2; exit 66; }
  printf '%s\n' "$physical"
}
VMM="$(direct_file "$VMM")"
GVPROXY="$(direct_file "$GVPROXY")"
KERNEL="$(direct_file "$KERNEL")"
ROOTFS="$(direct_file "$ROOTFS")"
DOCKER="$(direct_file "$DOCKER")"
if [ -n "$HV" ]; then HV="$(direct_file "$HV")"; fi
if [ -n "$GVPROXY_PROVENANCE" ]; then
  GVPROXY_PROVENANCE="$(direct_file "$GVPROXY_PROVENANCE")"
fi
if [ -n "$PAYLOAD_INVENTORY" ]; then
  PAYLOAD_INVENTORY="$(direct_file "$PAYLOAD_INVENTORY")"
fi
...
case "$FIXTURE_IMAGE" in *[[:space:]]*) echo "VZ native IPv6 gate: fixture image contains whitespace" >&2; exit 64 ;; esac
...
if [ "$SOURCE_ENABLED" = 1 ]; then
  ...
  [ -d "$SOURCE_APP" ] && [ ! -L "$SOURCE_APP" ] && [ "$(basename "$SOURCE_APP")" = Dory.app ] \
    || { echo "VZ native IPv6 gate: candidate app must be a direct Dory.app" >&2; exit 66; }
  SOURCE_APP_LOGICAL="$(python3 -c 'import os,sys; print(os.path.abspath(sys.argv[1]))' "$SOURCE_APP")"
  SOURCE_APP="$(cd "$SOURCE_APP" && pwd -P)"
  [ "$SOURCE_APP" = "$SOURCE_APP_LOGICAL" ] \
    || { echo "VZ native IPv6 gate: candidate app has an indirect ancestor" >&2; exit 66; }
  printf '%s\n' "$SOURCE_SERVER_IMAGE" | grep -Eq '^.+@sha256:[0-9a-f]{64}$' \
    || { echo "VZ native IPv6 gate: source server image must be digest-pinned" >&2; exit 64; }
  case "$SOURCE_SERVER_IMAGE" in *[[:space:]]*) echo "VZ native IPv6 gate: source server image contains whitespace" >&2; exit 64 ;; esac
  for peer in "$SOURCE_LAN_PEER" "$SOURCE_TAILSCALE_PEER"; do
    printf '%s\n' "$peer" | grep -Eq '^[A-Za-z0-9_.+-]+@[A-Za-z0-9_.:-]+$' \
      || { echo "VZ native IPv6 gate: source peer must be one safe USER@HOST authority" >&2; exit 64; }
  done
...
for raw in sys.argv[1:]:
    ...
    if value.is_unspecified or value.is_multicast or value.is_loopback:
        raise SystemExit("source address is not unicast")
PY
  sudo -n true >/dev/null 2>&1 \
    || { echo "VZ native IPv6 gate: physical source certification requires noninteractive sudo" >&2; exit 77; }
fi

case "$WORKROOT" in /*) ;; *) echo "VZ native IPv6 gate: workroot must be absolute" >&2; exit 64 ;; esac
WORKROOT_PARENT="$(dirname "$WORKROOT")"
WORKROOT_NAME="$(basename "$WORKROOT")"
case "$WORKROOT_NAME" in
  dory-vz-sonoma|dory-vz-native-ipv6-evidence) ;;
  *) echo "VZ native IPv6 gate: workroot must use a dedicated VZ gate name" >&2; exit 64 ;;
esac
[ -d "$WORKROOT_PARENT" ] && [ ! -L "$WORKROOT_PARENT" ] \
  || { echo "VZ native IPv6 gate: workroot parent must be a direct directory" >&2; exit 64; }
WORKROOT_PARENT="$(cd "$WORKROOT_PARENT" && pwd -P)"
WORKROOT="$WORKROOT_PARENT/$WORKROOT_NAME"
TEMP_AUTHORITY="${RUNNER_TEMP:-${TMPDIR:-/tmp}}"
[ -d "$TEMP_AUTHORITY" ] || { echo "VZ native IPv6 gate: temporary authority is unavailable" >&2; exit 64; }
TEMP_AUTHORITY="$(cd "$TEMP_AUTHORITY" && pwd -P)"
case "$TEMP_AUTHORITY" in /|"$HOME"|"$ROOT"|"$SOURCE_APP") echo "VZ native IPv6 gate: unsafe temporary authority" >&2; exit 64 ;; esac
case "$WORKROOT" in "$TEMP_AUTHORITY"/*) ;; *) echo "VZ native IPv6 gate: workroot must be inside runner temporary storage" >&2; exit 64 ;; esac
for input in "$VMM" "$HV" "$GVPROXY" "$GVPROXY_PROVENANCE" "$PAYLOAD_INVENTORY" \
  "$KERNEL" "$ROOTFS" "$DOCKER" "$SOURCE_APP"; do
  [ -z "$input" ] && continue
  case "$input/" in "$WORKROOT/"*) echo "VZ native IPv6 gate: workroot cannot contain an input" >&2; exit 64 ;; esac
  case "$WORKROOT/" in "$input/"*) echo "VZ native IPv6 gate: workroot cannot be inside an input" >&2; exit 64 ;; esac
done
[ "$(uname -s)" = Darwin ] && [ "$(uname -m)" = arm64 ] \
  || { echo "VZ native IPv6 gate: physical Apple-silicon macOS is required" >&2; exit 69; }
[ "$(sysctl -n kern.hv_support 2>/dev/null || printf 0)" = 1 ] \
  || { echo "VZ native IPv6 gate: Hypervisor.framework is unavailable" >&2; exit 69; }
[ "$(sysctl -in kern.hv_vmm_present 2>/dev/null || printf 0)" != 1 ] \
  || { echo "VZ native IPv6 gate: nested virtualization does not qualify" >&2; exit 69; }
case "$(sysctl -n hw.model 2>/dev/null || printf unknown)" in
  VirtualMac*) echo "VZ native IPv6 gate: a physical Mac is required" >&2; exit 69 ;;
esac
if [ -e "$WORKROOT" ] || [ -L "$WORKROOT" ]; then
  [ -d "$WORKROOT" ] && [ ! -L "$WORKROOT" ] \
    || { echo "VZ native IPv6 gate: workroot must not be indirect" >&2; exit 64; }
  [ "$(stat -f '%u' "$WORKROOT")" = "$(id -u)" ] \
    || { echo "VZ native IPv6 gate: workroot is not owned by this user" >&2; exit 64; }
else
  mkdir "$WORKROOT"
fi
chmod 700 "$WORKROOT"
...
cleanup_privileged_helper() {
  if [ "$HELPER_REGISTERED" -eq 1 ] && [ -n "$SOURCE_APP" ]; then
    "$SOURCE_APP/Contents/MacOS/Dory" --unregister-network-helper >/dev/null 2>&1 || true
  fi
}
...
[ ! -e "$RUN_ROOT" ] && [ ! -L "$RUN_ROOT" ] \
  || { echo "VZ native IPv6 gate: run authority already exists" >&2; exit 64; }
mkdir "$RUN_ROOT"
chmod 700 "$RUN_ROOT"
mkdir "$EVIDENCE" "$WORK"
if [ "$SOURCE_ENABLED" = 1 ]; then
...
if [ "$SOURCE_ENABLED" = 1 ]; then
  ...
  source_assessment="$(spctl --assess --type execute --verbose=4 "$SOURCE_APP" 2>&1)" \
    || { echo "VZ native IPv6 gate: Gatekeeper rejected the exact app" >&2; exit 1; }
  grep -F 'source=Notarized Developer ID' <<< "$source_assessment" >/dev/null \
    || { echo "VZ native IPv6 gate: app is not accepted as Notarized Developer ID" >&2; exit 1; }
  source_signature="$(codesign -dv --verbose=4 "$SOURCE_APP" 2>&1)"
  grep -F 'TeamIdentifier=864H636QW4' <<< "$source_signature" >/dev/null \
    || { echo "VZ native IPv6 gate: app has the wrong signing team" >&2; exit 1; }
  [ "$VMM" = "$SOURCE_APP/Contents/Helpers/dory-vmm" ] \
    && [ "$HV" = "$SOURCE_APP/Contents/Helpers/dory-hv" ] \
    && [ "$GVPROXY" = "$SOURCE_APP/Contents/Helpers/gvproxy" ] \
    && [ "$DOCKER" = "$SOURCE_APP/Contents/Helpers/docker" ] \
    && [ "$GVPROXY_PROVENANCE" = "$SOURCE_APP/Contents/Resources/gvproxy-provenance.txt" ] \
    && [ "$PAYLOAD_INVENTORY" = "$SOURCE_APP/Contents/Resources/dory-payload-sha256.txt" ] \
    && [ "$KERNEL" = "$SOURCE_APP/Contents/Resources/dory-hv-kernel-arm64.lzfse" ] \
    && [ "$ROOTFS" = "$SOURCE_APP/Contents/Resources/dory-engine-rootfs-arm64.ext4.lzfse" ] \
    || { echo "VZ native IPv6 gate: release inputs are not exact embedded app artifacts" >&2; exit 1; }
  if sudo -n /bin/launchctl print system/dev.dory.network-helper >/dev/null 2>&1; then
    echo "VZ native IPv6 gate: a pre-existing network helper would be replaced" >&2
    exit 1
  fi
  sudo -n test ! -e /var/run/dev.dory/pf-enable-token \
    || { echo "VZ native IPv6 gate: pre-existing PF authority marker" >&2; exit 1; }
  sudo -n test ! -e /var/run/dev.dory/ipv4-forwarding-owner \
    || { echo "VZ native IPv6 gate: pre-existing forwarding authority marker" >&2; exit 1; }
  HELPER_REGISTERED=1
...
def tcp_open(target_port, payload=None):
    ...
    if payload is not None:
        ...
        if connection.recv(1024).decode().strip() != payload:
            raise SystemExit("TCP echo payload mismatch")
...
if operation == "run":
    ...
    udp.send((token + "\n").encode())
    if udp.recv(1024).decode().strip() != token:
...
if body.get("machineID") != "docker":
    raise SystemExit("handoff machine identity mismatch")
if not body.get("dockerdSocketPath"):
    raise SystemExit("handoff omitted the Docker socket path")
...
require(bridge.get("EnableIPv6") is True, "Docker bridge IPv6 is disabled")
require(any(x.get("Subnet") == "fd7d:6f72:7901::/64" for x in bridge["IPAM"]["Config"]),
        "Docker bridge subnet mismatch")
require("inet6 fd7d:6f72:7901::" in text, "container global IPv6 address is absent")
require("2606:4700:4700::" in text, "Cloudflare AAAA resolution is absent")
require("2600:1f18:" in text, "registry AAAA resolution is absent")
PY
...
stop_vmm
if [ "$SOURCE_ENABLED" = 1 ]; then
  "$SOURCE_APP/Contents/MacOS/Dory" --unregister-network-helper \
    > "$EVIDENCE/source-network-helper-unregistration.txt" 2>&1 \
    || { echo "VZ native IPv6 gate: exact app could not unregister its network helper" >&2; exit 1; }
  HELPER_REGISTERED=0
  if sudo -n /bin/launchctl print system/dev.dory.network-helper >/dev/null 2>&1; then
    echo "VZ native IPv6 gate: network helper survived final cleanup" >&2
    exit 1
  fi
  sudo -n test ! -e /var/run/dev.dory/pf-enable-token \
...
if [ "$SOURCE_ENABLED" != 1 ] || [ "$SOURCE_RESULT" != PASS ]; then RELEASE_QUALIFYING=false; fi
{
  ...
  if [ "$SOURCE_ENABLED" = 1 ]; then
    ...
    echo source_network_helper_unregistered=PASS
  fi
