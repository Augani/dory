...
for input in "$DMG" "$SBOM" "$RELEASE_MANIFEST"; do
  [ -f "$input" ] && [ ! -L "$input" ] && [ -s "$input" ] \
    || die "release input is not a non-empty regular file: $input"
done
[ -n "${RUNNER_TEMP:-}" ] || die "RUNNER_TEMP must identify the dedicated release workspace"
case "$RUNNER_TEMP" in /*) ;; *) die "RUNNER_TEMP must be absolute" ;; esac
[ ! -L "$WORKROOT" ] || die "--workroot must not be a symlink"
workspace="$(python3 - "$RUNNER_TEMP" "$WORKROOT" <<'PY'
import os
import sys
...
if record is None or record.get("sha256") != digest:
    raise SystemExit("DMG differs from the immutable release manifest")
PY
...
cleanup() {
  ...
  /usr/bin/pkill -u "$(/usr/bin/id -u)" -x Dory >/dev/null 2>&1 || true
  if [ -S "$STATE/dory.sock" ] && [ -x "$APP/Contents/Helpers/dory" ]; then
...
/usr/bin/hdiutil attach -readonly -nobrowse -plist "$DMG" > "$EVIDENCE/image-attach.plist"
...
if len(mounts) != 1:
    raise SystemExit(f"expected one DMG mount, found {mounts}")
print(mounts[0])
PY
)"
[ -d "$MOUNT/Dory.app" ] && [ ! -L "$MOUNT/Dory.app" ] \
  || die "DMG does not contain a regular Dory.app bundle"
