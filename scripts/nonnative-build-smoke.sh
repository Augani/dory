...
BUILD_IMAGE="${READINESS_NONNATIVE_BUILD_IMAGE:-}"
...
Options:
  ...
  --image IMAGE         Local digest-pinned BuildKit base image
...
[ "$KEEP" = 0 ] || [ "$KEEP" = 1 ] \
  || { echo "nonnative-build-smoke: keep setting must be 0 or 1" >&2; exit 2; }
[ -f "$DOCKER_BIN" ] && [ ! -L "$DOCKER_BIN" ] && [ -x "$DOCKER_BIN" ] \
  || { echo "nonnative-build-smoke: Docker CLI is not an exact regular executable" >&2; exit 2; }
[ -S "$DORY_SOCK" ] || {
  echo "nonnative-build-smoke: missing Dory socket at $DORY_SOCK" >&2
  exit 1
}
[ "$(stat -f %u "$DORY_SOCK")" = "$(id -u)" ] || {
  echo "nonnative-build-smoke: Dory socket is not owned by the release user" >&2
  exit 1
}
for image in "$ALPINE_IMAGE" "$BUILD_IMAGE"; do
  printf '%s\n' "$image" | grep -Eq '^.+@sha256:[0-9a-f]{64}$' || {
    echo "nonnative-build-smoke: images must be digest-pinned: $image" >&2
    exit 2
  }
done

if [ -z "$WORKDIR" ]; then
  WORKDIR="$(mktemp -d "${RUNNER_TEMP:-${TMPDIR:-/tmp}}/dory-nonnative-build.XXXXXXXX")"
else
  case "$WORKDIR" in /*) ;; *) echo "nonnative-build-smoke: workdir must be absolute" >&2; exit 2 ;; esac
  [ ! -e "$WORKDIR" ] && [ ! -L "$WORKDIR" ] || {
    echo "nonnative-build-smoke: explicit workdir must not already exist" >&2
    exit 2
  }
  mkdir -m 700 "$WORKDIR"
fi
[ -d "$WORKDIR" ] && [ ! -L "$WORKDIR" ] || {
  echo "nonnative-build-smoke: workdir changed while preparing the build" >&2
  exit 2
}
...
docker_e image inspect "$ALPINE_IMAGE" >/dev/null 2>&1 \
  || { echo "nonnative-build-smoke: local fixture image is unavailable: $ALPINE_IMAGE" >&2; exit 1; }
docker_e image inspect "$BUILD_IMAGE" >/dev/null 2>&1 \
  || { echo "nonnative-build-smoke: local build image is unavailable: $BUILD_IMAGE" >&2; exit 1; }
...
if [ "$handler" = FEX-x86_64 ]; then
  grep -qx 'interpreter /usr/lib/dory/fex/FEX' "/proc/sys/fs/binfmt_misc/$handler" \
    || { echo "$handler interpreter is invalid" >&2; exit 1; }
  grep -qx 'flags: POCF' "/proc/sys/fs/binfmt_misc/$handler" \
    || { echo "$handler flags are invalid" >&2; exit 1; }
else
  grep -qx 'interpreter /usr/bin/qemu-aarch64-static' "/proc/sys/fs/binfmt_misc/$handler" \
    || { echo "$handler interpreter is invalid" >&2; exit 1; }
  grep -qx 'flags: F' "/proc/sys/fs/binfmt_misc/$handler" \
    || { echo "$handler flags are invalid" >&2; exit 1; }
fi
' sh "$HANDLER" 2>&1)"; then
    printf '%s\n' "$binfmt_output" >&2
...
RUN --network=none mkdir -p /tmp/tar-source/a/b/c /tmp/tar-output \
...
DOCKER_BUILDKIT=1 docker_e build --pull=false --progress=plain --platform "linux/$TARGET_ARCH" \
...
echo "nonnative-build-smoke: PASS linux/$TARGET_ARCH BuildKit npm ci + tar nested/hardlink + build + test + runtime"
