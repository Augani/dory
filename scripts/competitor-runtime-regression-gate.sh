...
Options:
  ...
  --confirm TOKEN       Must be ISOLATED-ENGINE-COMPETITOR-REGRESSION
  --release-candidate   Require exact release inputs and emit release-qualifying evidence
  -h, --help
...
[ "$CONFIRM" = ISOLATED-ENGINE-COMPETITOR-REGRESSION ] \
  || die "requires --confirm ISOLATED-ENGINE-COMPETITOR-REGRESSION"
...
for command in cmp curl id lsof mkfifo ps python3 shasum stat tar; do
...
case "$STATE_DIR" in /*) ;; *) die "Dory state directory must be absolute" ;; esac
[ -d "$STATE_DIR" ] && [ ! -L "$STATE_DIR" ] \
  || die "Dory state directory is unavailable or indirect: $STATE_DIR"
STATE_DIR="$(cd "$STATE_DIR" && pwd -P)"
case "$WORKROOT" in /*) ;; *) die "workroot must be an absolute path" ;; esac
case "$WORKROOT" in /|"$HOME"|"$STATE_DIR"|"$RUNTIME_HOME") die "unsafe workroot: $WORKROOT" ;; esac
...
[ -d "$workroot_parent" ] && [ ! -L "$workroot_parent" ] \
  || die "workroot parent is unavailable or indirect: $workroot_parent"
workroot_name="$(basename "$WORKROOT")"
case "$workroot_name" in ''|.|..) die "unsafe workroot leaf: $workroot_name" ;; esac
WORKROOT="$(cd "$workroot_parent" && pwd -P)/$workroot_name"
[ ! -e "$WORKROOT" ] && [ ! -L "$WORKROOT" ] \
  || die "canonical workroot already exists or is indirect: $WORKROOT"
...
if [ "$RELEASE_CANDIDATE" -eq 1 ]; then
  ...
  [ -n "$RUNTIME" ] || die "release candidate evidence requires --runtime"
  [ -n "$COMPOSE_BIN" ] || die "release candidate evidence requires --compose"
  [ -n "$BUILDX_BIN" ] || die "release candidate evidence requires --buildx"
fi
...
if [ -n "$RUNTIME" ]; then
  case "$RUNTIME" in /*) ;; *) die "standalone runtime must be an absolute path" ;; esac
  [ -f "$RUNTIME" ] && [ ! -L "$RUNTIME" ] && [ -s "$RUNTIME" ] && [ -x "$RUNTIME" ] \
    || die "standalone runtime is unavailable or indirect: $RUNTIME"
  case "$RUNTIME_HOME" in /*) ;; *) die "standalone runtime HOME must be absolute" ;; esac
  [ -d "$RUNTIME_HOME" ] && [ ! -L "$RUNTIME_HOME" ] \
    || die "standalone runtime HOME is unavailable or indirect: $RUNTIME_HOME"
  runtime_home_real="$(cd "$RUNTIME_HOME" && pwd -P)"
  RUNTIME_HOME="$runtime_home_real"
  workroot_real="$WORKROOT"
  ...
  for runtime_file in dory-engine bin/dory-hv bin/gvproxy bin/dory-dataplane-proxy \
    ...
    [ -f "$runtime_dir/$runtime_file" ] && [ ! -L "$runtime_dir/$runtime_file" ] \
      && [ -s "$runtime_dir/$runtime_file" ] \
      || die "standalone runtime member is missing or indirect: $runtime_file"
  done
fi
...
if [ "$RELEASE_CANDIDATE" -eq 1 ]; then
  for qualified_cli in "$docker_bin_resolved" "$compose_bin_resolved" "$buildx_bin_resolved"; do
    case "$qualified_cli" in /*) ;; *) die "release candidate CLI must resolve to an absolute path" ;; esac
    [ -f "$qualified_cli" ] && [ ! -L "$qualified_cli" ] && [ -s "$qualified_cli" ] \
      && [ -x "$qualified_cli" ] \
      || die "release candidate CLI is unavailable or indirect: $qualified_cli"
  done
fi
{
  ...
  if [ -n "$compose_bin_resolved" ]; then
    ...
    echo "compose_bin_sha256=embedded-docker-cli-plugin"
  fi
  if [ -n "$buildx_bin_resolved" ]; then
    echo "buildx_bin_sha256=$(shasum -a 256 "$buildx_bin_resolved" | awk '{print $1}')"
  else
    echo "buildx_bin_sha256=embedded-docker-cli-plugin"
  fi
  if [ -n "$RUNTIME" ]; then
    for runtime_file in \
      dory-engine \
...
require(model["name"] == project, f"unexpected Compose project: {model['name']!r}")
require(set(model["services"]) == {"initializer", "database", "api", "debug"},
        f"unexpected Compose services: {sorted(model['services'])!r}")
api = model["services"]["api"]
require(api["environment"]["MERGED_VALUE"] == "override", "Compose override merge failed")
require(api["environment"]["DORY_COMPOSE_TOKEN"] == f"compose-token-{run_id}",
        "Compose env-file interpolation failed")
require(api["labels"]["dev.dory.compose-contract"] == "merged", "Compose labels did not merge")
require(api.get("cap_add", []) == [], "Compose !reset did not clear cap_add")
require(set(model["volumes"]) == {"state"}, f"unexpected Compose volumes: {model['volumes']!r}")
require(set(model["networks"]) == {"app", "external"},
        f"unexpected Compose networks: {model['networks']!r}")
PY
...
require(len(networks) == 1, f"expected one inspected network, got {len(networks)}")
network = networks[0]
require(network["Name"] == expected_name, f"network name mismatch: {network!r}")
require(network["Driver"] == "bridge" and network["Scope"] == "local",
        f"network driver/scope mismatch: {network!r}")
require(network["Internal"] is True and network["Attachable"] is True,
        f"network flags mismatch: {network!r}")
require(network["Ingress"] is False, f"network ingress mismatch: {network!r}")
require(network["Labels"]["dev.dory.compatibility"] == expected_owner,
        f"network owner mismatch: {network!r}")
require(network["Labels"]["dev.dory.network-contract"] == "original",
        f"network contract label mismatch: {network!r}")
require(network["Options"]["com.docker.network.bridge.enable_icc"] == "true",
        f"network options mismatch: {network!r}")
require(network["IPAM"]["Driver"] == "default", f"network IPAM driver mismatch: {network!r}")
config = network["IPAM"]["Config"]
require(len(config) == 1, f"network IPAM config mismatch: {config!r}")
require(config[0]["Subnet"] == subnet, f"network subnet mismatch: {config!r}")
require(config[0]["IPRange"] == ip_range, f"network IP range mismatch: {config!r}")
require(config[0]["Gateway"] == gateway, f"network gateway mismatch: {config!r}")
require(config[0]["AuxiliaryAddresses"] == {"reserved": reserved},
        f"network auxiliary address mismatch: {config!r}")
PY
...
if after != before:
    raise SystemExit("duplicate create mutated existing network metadata")
PY
...
if endpoint["IPAddress"] != sys.argv[3]:
    raise SystemExit(f"static network address mismatch: {endpoint!r}")
if "metadata-api" not in endpoint["Aliases"]:
    raise SystemExit(f"network alias missing: {endpoint!r}")
PY
set +e
...
require(len(volumes) == 1, f"expected one inspected volume, got {len(volumes)}")
volume = volumes[0]
require(volume["Name"] == expected_name, f"volume name mismatch: {volume!r}")
require(volume["Driver"] == "local", f"volume driver mismatch: {volume!r}")
require(volume["Scope"] == "local", f"volume scope mismatch: {volume!r}")
require(volume["Labels"]["dev.dory.compatibility"] == expected_owner,
        f"volume owner mismatch: {volume!r}")
require(volume["Labels"]["dev.dory.volume-contract"] == "original",
        f"volume contract label mismatch: {volume!r}")
require(volume["Options"] == {"type": "tmpfs", "device": "tmpfs", "o": "size=4m"},
        f"volume options mismatch: {volume!r}")
PY
...
with open(sys.argv[2], encoding="utf-8") as after_handle:
    after = json.load(after_handle)
if after != before:
    raise SystemExit("same-name create mutated existing volume metadata")
PY
...
if pathlib.Path(path).stat().st_size < 65536:
    raise SystemExit("large Dockerfile fixture is below 64 KiB")
PY
...
echo "results_sha256=$(shasum -a 256 "$RESULTS" | awk '{print $1}')" >> "$MANIFEST"
echo "status=PASS" >> "$MANIFEST"
echo "release_qualifying=$release_qualifying" >> "$MANIFEST"
