...
loader = importlib.machinery.SourceFileLoader("dd_tls", "scripts/dory-doctor")
dd = importlib.util.module_from_spec(importlib.util.spec_from_loader("dd_tls", loader))
sys.modules["dd_tls"] = dd
...
original_factory = dd.ssl.create_default_context
try:
    system_ca = pathlib.Path(tempfile.mkdtemp()) / "cert.pem"
    system_ca.write_text("test CA bundle", encoding="utf-8")
    dd.DARWIN_SYSTEM_CA_FILE = system_ca
    dd.ssl.create_default_context = Context

    sys.platform = "darwin"
    assert dd.default_tls_context().ca_files == [str(system_ca)]

    sys.platform = "linux"
    assert dd.default_tls_context().ca_files == []
finally:
    sys.platform = original_platform
    dd.ssl.create_default_context = original_factory
PY
...
assert commands["component"]["json"] is True
assert "kubernetes" in commands["component"]["components"]
assert "selected Dory data drive" in commands["component"]["notes"]
...
assert commands["sandbox"]["status"] == "supported"
assert commands["sandbox"]["hostFileSharingDefault"] == "none"
assert commands["sandbox"]["scopedMounts"] is True
assert commands["sandbox"]["mountModeDefault"] == "ro"
assert commands["sandbox"]["networkPolicies"] == ["none", "outbound", "full"]
assert commands["sandbox"]["unenforcedNetworkPolicies"] == []
assert commands["sandbox"]["networkPolicyDefault"] == "none"
assert "disk" in commands["sandbox"]["resourceCaps"]
...
'
DORYDCTL_BIN="$TMP_HOME/fake-dorydctl" scripts/dory machine exec dev -- /bin/echo ok | python3 -c '
import json, sys
data = json.load(sys.stdin)
assert data["schema"] == "dev.dory.machine.exec"
assert data["machine"] == "dev"
...
assert data["networkPolicyEnforced"] is True
...
assert data["manifest"]["identity"]["elevated"] is False
assert data["manifest"]["limits"]["diskMB"] == 256
assert data["manifest"]["network"]["dns"] == "blocked"
'
...
# Mounts without an explicit suffix are read-only. Outbound policy records only explicit grants,
# and secret values travel over stdin but never enter the durable manifest or CLI argv.
export DORY_TEST_SANDBOX_SECRET='must-not-enter-sandbox-manifest-7f3b'
secret_capture="$TMP_HOME/sandbox-env-capture.json"
sandbox_grants_json="$(DORY_FAKE_ENV_CAPTURE="$secret_capture" DORYDCTL_BIN="$TMP_HOME/fake-dorydctl" DORY_SANDBOX_KERNEL="$TMP_HOME/kernel" DORY_SANDBOX_ROOTFS="$TMP_HOME/rootfs.ext4" \
  scripts/dory sandbox run --json --keep --name agentgrants --mount "$TMP_HOME/project:/work/default-ro" \
    --network outbound --allow-network 127.0.0.1:8443 --secret-env DORY_TEST_SANDBOX_SECRET -- /bin/echo isolated)"
printf '%s' "$sandbox_grants_json" | python3 -c '
import json, sys
data = json.load(sys.stdin)
assert data["mounts"][0]["mode"] == "ro"
assert data["manifest"]["network"]["mode"] == "outbound"
assert data["manifest"]["network"]["egressFilterEnforced"] is True
assert data["manifest"]["network"]["grants"] == [{"addresses":["127.0.0.1"],"host":"127.0.0.1","port":8443}]
assert data["manifest"]["credentials"]["secretEnvironmentNames"] == ["DORY_TEST_SANDBOX_SECRET"]
assert "must-not-enter-sandbox-manifest" not in json.dumps(data["manifest"])
'
python3 - "$secret_capture" <<'PY'
import json, sys
environment = json.load(open(sys.argv[1], encoding="utf-8"))
assert environment["DORY_TEST_SANDBOX_SECRET"] == "must-not-enter-sandbox-manifest-7f3b"
assert environment["DORY_AGENT_RUN_UID"] != "0"
assert environment["DORY_AGENT_MAX_PROCESSES"] == "128"
PY
! grep -R -F 'must-not-enter-sandbox-manifest-7f3b' "$TMP_HOME/.dory/sandboxes"
scripts/dory sandbox inspect agentgrants --json | python3 -c 'import json,sys; assert json.load(sys.stdin)["sandbox"] == "agentgrants"'
...
if DORYDCTL_BIN="$TMP_HOME/fake-dorydctl" DORY_SANDBOX_KERNEL="$TMP_HOME/kernel" DORY_SANDBOX_ROOTFS="$TMP_HOME/rootfs.ext4" \
  ...
  echo "sandbox accepted elevated execution under a filtered network policy" >&2
  exit 1
fi
