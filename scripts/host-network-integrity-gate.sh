...
IMAGE="${DORY_NETWORK_INTEGRITY_IMAGE:-}"
...
[ "$CYCLES" -le 10 ] || die "cycles must not exceed 10"
[ "$WAKE_TIMEOUT" -le 600 ] || die "wake-timeout must not exceed 600 seconds"
[ "$AUTO_WAKE_SECONDS" -le 600 ] || die "auto-wake-seconds must not exceed 600 seconds"
...
[ ! -L "$WORKROOT" ] || die "--workroot must not be a symlink"
printf '%s\n' "$IMAGE" | grep -Eq '^.+@sha256:[0-9a-f]{64}$' \
  || die "network probe image must be digest-pinned"
python3 - "$PROBE_HOST" "$PROBE_URL" "$CUSTOM_DNS" <<'PY'
import ipaddress
import re
import sys
from urllib.parse import urlsplit

host, raw_url, custom_dns = sys.argv[1:]
if re.fullmatch(r"(?=.{1,253}\Z)[A-Za-z0-9](?:[A-Za-z0-9.-]*[A-Za-z0-9])?", host) is None:
    raise SystemExit("network-integrity: probe host is invalid")
...
if url.scheme != "https" or url.hostname != host or url.username is not None \
    ...
    raise SystemExit("network-integrity: probe URL must use the exact probe host over HTTPS")
if custom_dns:
    try:
        ipaddress.ip_address(custom_dns)
    except ValueError:
        raise SystemExit("network-integrity: custom DNS must be one IP address")
PY
...
[ -f "$DOCKER" ] && [ ! -L "$DOCKER" ] && [ -x "$DOCKER" ] \
  || die "exact Docker CLI is unavailable or indirect: $DOCKER"
...
if [ "$NO_SLEEP" = "0" ]; then
  ...
  [ -d "$APP" ] && [ ! -L "$APP" ] \
    && [ -f "$APP/Contents/MacOS/Dory" ] && [ ! -L "$APP/Contents/MacOS/Dory" ] \
    && [ -x "$APP/Contents/MacOS/Dory" ] \
  ...
  [ -f "$MACHINE_CTL" ] && [ ! -L "$MACHINE_CTL" ] && [ -x "$MACHINE_CTL" ] \
    || die "exact candidate dorydctl is unavailable: $MACHINE_CTL"
  [ -f "$MACHINE_KERNEL" ] && [ ! -L "$MACHINE_KERNEL" ] && [ -s "$MACHINE_KERNEL" ] \
    || die "exact candidate machine kernel is unavailable: $MACHINE_KERNEL"
  [ -f "$MACHINE_ROOTFS" ] && [ ! -L "$MACHINE_ROOTFS" ] && [ -s "$MACHINE_ROOTFS" ] \
    || die "exact candidate machine rootfs is unavailable: $MACHINE_ROOTFS"
  ...
  TAILSCALE_BIN="$(python3 - "$TAILSCALE_BIN" <<'PY'
import os
import sys
...
PY
  ...
  [ -f "$TAILSCALE_BIN" ] && [ ! -L "$TAILSCALE_BIN" ] && [ -x "$TAILSCALE_BIN" ] \
...
[ -d "$WORKROOT" ] && [ ! -L "$WORKROOT" ] \
  && [ -d "$WORKDIR" ] && [ ! -L "$WORKDIR" ] \
  || die "workroot changed while preparing evidence"
...
if [ "$NO_SLEEP" = "0" ]; then
  "$TAILSCALE_BIN" status --json > "$WORKDIR/tailscale-preflight-status.json"
  jq -e '
    (.ExitNodeStatus // null) == null and
    ([.Peer[]? | select((.ExitNode // false) == true)] | length) == 0
  ' "$WORKDIR/tailscale-preflight-status.json" >/dev/null \
    || die "dedicated runner already has an active Tailscale exit node; refusing to change user network state"
...
run_route_churn() {
  ...
  while [ "$round" -le "$ROUTE_CHURN_ROUNDS" ]; do
    ...
    # Arm cleanup before requesting the mutation so a signal between a successful set and the
    # next shell statement cannot leave the dedicated runner using the test exit node.
...
if [ "$NO_SLEEP" = "0" ]; then
  ...
  until host_probe >/dev/null 2>&1 && container_probe >/dev/null 2>&1 \
    ...
    [ "$(date +%s)" -lt "$baseline_deadline" ] \
      || die "host/container/Docker is not healthy before route-churn qualification"
    sleep 2
  done
fi
