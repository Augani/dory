...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --ssh-option)
      need_value "$1" "$#"
      case "$2" in
        StrictHostKeyChecking=yes) STRICT_HOST_KEY_CHECKS=$((STRICT_HOST_KEY_CHECKS + 1)) ;;
        StrictHostKeyChecking=*) die "StrictHostKeyChecking must be yes" ;;
        *) die "unsupported --ssh-option: $2" ;;
      esac
      SSH_OPTIONS+=("-o" "$2")
      shift 2
      ;;
...
if value.is_unspecified or value.is_multicast or value.is_loopback:
    raise SystemExit("host address is not unicast")
PY
printf '%s\n' "$SERVER_IMAGE" | grep -Eq '^.+@sha256:[0-9a-f]{64}$' \
  || die "--server-image must contain one exact lowercase sha256 digest"
case "$SERVER_IMAGE" in *[[:space:]]*) die "--server-image contains whitespace" ;; esac
printf '%s\n' "$PEER_SSH" | grep -Eq '^[A-Za-z0-9_.+-]+@[A-Za-z0-9_.:-]+$' \
  || die "--peer-ssh must be one safe USER@HOST authority"
[ "$STRICT_HOST_KEY_CHECKS" -eq 1 ] \
  || die "exactly one --ssh-option StrictHostKeyChecking=yes is required"
...
[ -d "$APP" ] && [ ! -L "$APP" ] && [ "$(basename "$APP")" = Dory.app ] \
  || die "candidate app must be a direct Dory.app directory: $APP"
APP_LOGICAL="$(python3 -c 'import os,sys; print(os.path.abspath(sys.argv[1]))' "$APP")"
APP="$(cd "$APP" && pwd -P)"
[ "$APP" = "$APP_LOGICAL" ] || die "candidate app cannot have an indirect ancestor"
[ -d "$RUNTIME" ] && [ ! -L "$RUNTIME" ] && [ -x "$RUNTIME/dory-engine" ] \
  || die "runtime must be a direct directory with a launcher"
RUNTIME_LOGICAL="$(python3 -c 'import os,sys; print(os.path.abspath(sys.argv[1]))' "$RUNTIME")"
RUNTIME="$(cd "$RUNTIME" && pwd -P)"
[ "$RUNTIME" = "$RUNTIME_LOGICAL" ] || die "runtime cannot have an indirect ancestor"
[ -x "$DOCKER" ] && [ ! -L "$DOCKER" ] || die "Docker CLI must be a direct executable"
DOCKER="$(cd "$(dirname "$DOCKER")" && pwd -P)/$(basename "$DOCKER")"
[ "$DOCKER" = "$APP/Contents/Helpers/docker" ] \
  || die "Docker CLI is not the exact candidate helper"
case "$WORKROOT" in /*) ;; *) die "workroot must be absolute" ;; esac
WORKROOT_PARENT="$(dirname "$WORKROOT")"
WORKROOT_NAME="$(basename "$WORKROOT")"
[ "$WORKROOT_NAME" = "dory-source-lan-$MODE" ] \
  || die "workroot must use the dedicated dory-source-lan-$MODE name"
...
case "$TEMP_AUTHORITY" in /|"$HOME"|"$ROOT"|"$APP"|"$RUNTIME") die "unsafe temporary authority root" ;; esac
case "$WORKROOT" in "$TEMP_AUTHORITY"/*) ;; *) die "workroot must be inside runner temporary storage" ;; esac
case "$WORKROOT/" in "$APP/"*|"$RUNTIME/"*) die "workroot cannot be inside candidate artifacts" ;; esac
...
if sudo -n /bin/launchctl print system/dev.dory.network-helper >/dev/null 2>&1; then
  die "a pre-existing Dory network helper would be replaced"
fi
sudo -n test ! -e /var/run/dev.dory/pf-enable-token \
  || die "a pre-existing Dory PF authority marker exists"
sudo -n test ! -e /var/run/dev.dory/ipv4-forwarding-owner \
  || die "a pre-existing Dory forwarding authority marker exists"
...
cleanup() {
  ...
  if [ "$HELPER_REGISTERED" -eq 1 ]; then
    "$APP/Contents/MacOS/Dory" --unregister-network-helper >/dev/null 2>&1 || true
  fi
}
trap cleanup EXIT
trap 'exit 130' INT
...
ENGINE_STARTED=0
"$APP/Contents/MacOS/Dory" --unregister-network-helper \
  > "$WORKROOT/evidence/network-helper-unregistration.txt" 2>&1 \
  || die "the exact app could not unregister its privileged helper"
HELPER_REGISTERED=0
if sudo -n /bin/launchctl print system/dev.dory.network-helper >/dev/null 2>&1; then
  die "Dory network helper survived final cleanup"
fi
