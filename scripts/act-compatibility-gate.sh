...
printf '%s\n' "$RUNNER_IMAGE" \
  ...
  || die "runner image must be an exact digest reference"
...
docker_e() { "$DOCKER" "$@"; }
docker_e image inspect "$RUNNER_IMAGE" >/dev/null 2>&1 \
  || die "required offline runner image is missing: $RUNNER_IMAGE"
...
[ -f "$DOWNLOAD/act" ] && [ ! -L "$DOWNLOAD/act" ] && [ -x "$DOWNLOAD/act" ] \
  || die "verified act archive did not contain a direct executable"
...
act_binary_sha256=$(shasum -a 256 "$DOWNLOAD/act" | awk '{print $1}')
