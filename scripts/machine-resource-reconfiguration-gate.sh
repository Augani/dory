...
[ -f "$CTL" ] && [ ! -L "$CTL" ] && [ -x "$CTL" ] \
  || die "dorydctl is not an exact regular executable: $CTL"
[ -f "$KERNEL" ] && [ ! -L "$KERNEL" ] && [ -s "$KERNEL" ] \
  || die "machine kernel is not an exact regular file: $KERNEL"
[ -f "$ROOTFS" ] && [ ! -L "$ROOTFS" ] && [ -s "$ROOTFS" ] \
  || die "machine rootfs is not an exact regular file: $ROOTFS"
