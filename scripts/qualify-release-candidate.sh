...
TESTCONTAINERS_RYUK_IMAGE="${DORY_RELEASE_TESTCONTAINERS_RYUK_IMAGE:-testcontainers/ryuk:0.14.0@sha256:7c1a8a9a47c780ed0f983770a662f80deb115d95cce3e2daa3d12115b8cd28f0}"
DEVCONTAINERS_VERSION="${DORY_RELEASE_DEVCONTAINERS_VERSION:-0.87.0}"
ACT_VERSION="${DORY_RELEASE_ACT_VERSION:-0.2.89}"
ACT_RUNNER_IMAGE="${DORY_RELEASE_ACT_RUNNER_IMAGE:-node:20.19.5-bookworm-slim@sha256:9e70124bd00f47dd023e349cd587132ae61892acc0e47ed641416c3e18f401c3}"
...
Options:
  ...
  --testcontainers-ryuk-image REF Exact digest-pinned Ryuk image
  ...
  --act-runner-image REF      Exact digest-pinned Act runner image
...
seconds. Candidate metadata must describe a signed, notarized, public schema-2 release. The script
never publishes and never touches a pre-existing Dory home; it owns one run-attempt-specific HOME
beneath the qualification root.
EOF
}
...
while [ "$#" -gt 0 ]; do
  case "$1" in
    ...
    --testcontainers-ryuk-image) need_value "$1" "$#"; TESTCONTAINERS_RYUK_IMAGE="$2"; shift 2 ;;
    --devcontainers-version) need_value "$1" "$#"; DEVCONTAINERS_VERSION="$2"; shift 2 ;;
    --act-version) need_value "$1" "$#"; ACT_VERSION="$2"; shift 2 ;;
    --act-runner-image) need_value "$1" "$#"; ACT_RUNNER_IMAGE="$2"; shift 2 ;;
...
[ -d "$BUILD_DIR" ] && [ ! -L "$BUILD_DIR" ] \
  || die "build directory is unavailable or indirect: $BUILD_DIR"
...
printf '%s\n' "$TESTCONTAINERS_RYUK_IMAGE" | grep -Eq '^.+@sha256:[0-9a-f]{64}$' \
  || die "--testcontainers-ryuk-image must be digest-pinned"
...
printf '%s\n' "$ACT_RUNNER_IMAGE" | grep -Eq '^.+@sha256:[0-9a-f]{64}$' \
  || die "--act-runner-image must be digest-pinned"
...
git diff --quiet -- scripts .github/scripts \
  || die "tracked qualification harness differs from the checked-out source commit"
git diff --cached --quiet -- scripts .github/scripts \
  || die "staged qualification harness differs from the checked-out source commit"
QUALIFIER_SHA256="$(shasum -a 256 scripts/qualify-release-candidate.sh | awk '{print $1}')"
METADATA_VALIDATOR_SHA256="$(shasum -a 256 scripts/validate-release-metadata.py | awk '{print $1}')"

MANIFEST="$BUILD_DIR/release-manifest.json"
[ -s "$MANIFEST" ] || die "release manifest is missing: $MANIFEST"
validated_source_commit="$(python3 scripts/validate-release-metadata.py \
  "$BUILD_DIR" "$VERSION" "$BUILD")" \
  || die "signed schema-2 release metadata validation failed"
[ "$validated_source_commit" = "$SOURCE_COMMIT" ] \
  || die "signed release metadata belongs to another source commit"
...
case "$QUALIFICATION_ROOT" in /*) ;; *) die "qualification root must be an absolute path" ;; esac
case "$QUALIFICATION_ROOT" in /|"$HOME") die "unsafe qualification root: $QUALIFICATION_ROOT" ;; esac
if [ ! -e "$QUALIFICATION_ROOT" ]; then
  qualification_parent="$(dirname "$QUALIFICATION_ROOT")"
  [ -d "$qualification_parent" ] && [ ! -L "$qualification_parent" ] \
    || die "qualification root parent is unavailable or indirect"
  [ "$(cd "$qualification_parent" && pwd -P)" = "$qualification_parent" ] \
    || die "qualification root parent must be a direct canonical path"
  mkdir "$QUALIFICATION_ROOT"
  chmod 0700 "$QUALIFICATION_ROOT"
fi
[ -d "$QUALIFICATION_ROOT" ] && [ ! -L "$QUALIFICATION_ROOT" ] \
  || die "qualification root is unavailable or indirect"
[ "$(stat -f %u "$QUALIFICATION_ROOT")" = "$(id -u)" ] \
  || die "qualification root is not owned by the release user"
QUALIFICATION_ROOT="$(cd "$QUALIFICATION_ROOT" && pwd -P)"
[ -d "$HOME" ] && [ ! -L "$HOME" ] && [ "$(cd "$HOME" && pwd -P)" = "$HOME" ] \
  || die "release HOME must be a direct canonical directory"
...
mkdir -p "$WORKDIR/evidence" "$WORKDIR/extracted-app" "$WORKDIR/extracted-runtime" \
  "$WORKDIR/docker-config/cli-plugins"
mkdir "$ENGINE_HOME"
mkdir "$ENGINE_HOME/gate-evidence"
...
COMPONENT_CATALOG="$BUILD_DIR/components/arm64/catalog.json"
...
KUBECTL="$(python3 - "$BUILD_DIR" "$VERSION" <<'PY'
import hashlib
import json
import pathlib
import sys
...
version = sys.argv[2]
catalog = json.loads((root / "components/arm64/catalog.json").read_text(encoding="utf-8"))
matches = [item for item in catalog["components"] if item.get("id") == "kubernetes"]
if len(matches) != 1:
    raise SystemExit("component catalog must contain one Kubernetes component")
assets = matches[0].get("assets", [])
if len(assets) != 1 or assets[0].get("path") != "kubectl":
    raise SystemExit("Kubernetes component must contain exactly kubectl")
asset = assets[0]
name = pathlib.PurePosixPath(urllib.parse.urlparse(asset["url"]).path).name
path = root / "components" / "arm64" / name
if not path.is_file() or path.is_symlink():
    raise SystemExit("Kubernetes component artifact is missing or indirect")
data = path.read_bytes()
if len(data) != asset["downloadBytes"] or len(data) != asset["installedBytes"]:
    raise SystemExit("Kubernetes component size mismatch")
digest = hashlib.sha256(data).hexdigest()
if digest != asset["sha256"] or digest != asset["installedSHA256"]:
    raise SystemExit("Kubernetes component digest mismatch")
if asset["compression"] != "none":
    raise SystemExit("Kubernetes component must be uncompressed")
print(path)
PY
)" || die "focused Kubernetes component metadata is invalid"
...
codesign --verify --strict "$KUBECTL" \
  || die "candidate Kubernetes component is not validly signed"
[ "$(lipo -archs "$KUBECTL")" = arm64 ] \
  || die "candidate Kubernetes component is not arm64-only"
[ -x "$COMPOSE" ] || die "candidate Compose plugin is missing"

APP_EXECUTABLE="$APP/Contents/MacOS/Dory"
DORYD="$APP/Contents/Helpers/doryd"
DORY_VMM="$APP/Contents/Helpers/dory-vmm"
DATAPLANE="$APP/Contents/Helpers/dory-dataplane-proxy"
...
for artifact in "$KERNEL" "$ROOTFS" "$GUEST_AGENT"; do
  [ -f "$artifact" ] && [ ! -L "$artifact" ] && [ -s "$artifact" ] \
    || die "candidate launch artifact is missing or indirect: $(basename "$artifact")"
done
CANDIDATE_APP_EXECUTABLE_SHA="$(shasum -a 256 "$APP_EXECUTABLE" | awk '{print $1}')"
CANDIDATE_DORYD_SHA="$(shasum -a 256 "$DORYD" | awk '{print $1}')"
CANDIDATE_DORY_VMM_SHA="$(shasum -a 256 "$DORY_VMM" | awk '{print $1}')"
CANDIDATE_DATAPLANE_SHA="$(shasum -a 256 "$DATAPLANE" | awk '{print $1}')"
CANDIDATE_DOCKER_SHA="$(shasum -a 256 "$DOCKER" | awk '{print $1}')"
CANDIDATE_COMPOSE_SHA="$(shasum -a 256 "$COMPOSE" | awk '{print $1}')"
CANDIDATE_BUILDX_SHA="$(shasum -a 256 "$BUILDX" | awk '{print $1}')"
CANDIDATE_KUBECTL_SHA="$(shasum -a 256 "$KUBECTL" | awk '{print $1}')"
CANDIDATE_KERNEL_SHA="$(shasum -a 256 "$KERNEL" | awk '{print $1}')"
CANDIDATE_ROOTFS_SHA="$(shasum -a 256 "$ROOTFS" | awk '{print $1}')"
...
docker_at() {
  ...
  env -u DOCKER_API_VERSION -u DOCKER_AUTH_CONFIG -u DOCKER_CERT_PATH \
    ...
    -u DOCKER_TLS -u DOCKER_TLS_VERIFY DOCKER_CONFIG="$DOCKER_CONFIG" \
    DOCKER_HOST="unix://$socket" "$DOCKER" "$@"
}
docker_e() { docker_at "$SOCKET" "$@"; }
migration_docker_e() { docker_at "$MIGRATION_SOURCE_SOCKET" "$@"; }

bounded 30 scripts/gvproxy-qemu-switch-gate.py \
  "$GVPROXY" \
  --expected-sha256 "$CANDIDATE_GVPROXY_SHA" \
  --provenance "$GVPROXY_PROVENANCE" \
  --workroot "$WORKDIR/evidence/gvproxy-qemu-switch" \
  --evidence "$WORKDIR/evidence/gvproxy-qemu-switch/manifest.txt" \
  --source-commit "$SOURCE_COMMIT" \
  --confirm EXACT-GVPROXY-QEMU-SWITCH \
  --release-candidate \
...
bounded 900 scripts/managed-data-drive-gate.sh \
  ...
  --source-commit "$SOURCE_COMMIT" \
  --confirm ISOLATED-ENGINE-MANAGED-DATA-DRIVE \
  --release-candidate \
...
bounded 900 scripts/data-disk-growth-gate.sh \
  ...
  --confirm ISOLATED-RUNTIME-DATA-DISK-GROWTH \
...
require_owned_engine_pid() {
  local pid="$1"
  printf '%s\n' "$pid" | grep -Eq '^[0-9]+$' \
    || die "engine PID metadata is invalid"
  owned_engine_pids | grep -qx "$pid" \
    || die "engine PID is outside the exact isolated runtime authority"
}
cleanup() {
  set +e
...
require_owned_engine_pid "$repaired_dataplane_pid"
...
docker_e version > "$WORKDIR/evidence/docker-version.txt"
...
bounded 600 docker_e pull --platform linux/arm64 "$IMAGE" \
...
[ ! -e "$private_registry_workroot" ] && [ ! -L "$private_registry_workroot" ] \
  || die "private-registry evidence root already exists or is indirect"
...
[ ! -e "$WORKDIR/evidence/private-registry-auth" ] \
  || die "durable private-registry evidence destination already exists"
mkdir "$WORKDIR/evidence/private-registry-auth"
...
bounded 180 scripts/bind-file-coherence-gate.sh \
  --socket "$SOCKET" \
  --docker "$DOCKER" \
  --image "$IMAGE" \
  --workroot "$ENGINE_HOME/gate-evidence/bind-file-coherence" \
...
verify_running_guest_agent() {
  ...
  if ! bounded 60 docker_e run --rm --privileged \
...
bounded 1800 scripts/competitor-runtime-regression-gate.sh \
  ...
  --source-commit "$SOURCE_COMMIT" \
  --confirm ISOLATED-ENGINE-COMPETITOR-REGRESSION \
  --release-candidate \
...
bounded 600 docker_e pull --platform linux/arm64 "$TESTCONTAINERS_RYUK_IMAGE" \
  > "$WORKDIR/evidence/testcontainers-ryuk-pull.log" 2>&1 \
  || die "could not preload the digest-pinned Testcontainers Ryuk fixture"
...
bounded 600 docker_e pull --platform linux/arm64 "$ACT_RUNNER_IMAGE" \
  > "$WORKDIR/evidence/act-runner-pull.log" 2>&1 \
  || die "could not preload the digest-pinned Act runner fixture"
...
bounded 600 docker_e pull --platform linux/arm64 "$LOCALSTACK_IMAGE" \
  > "$WORKDIR/evidence/localstack-image-pull.log" 2>&1 \
  || die "could not preload the digest-pinned LocalStack fixture"
...
scripts/endurance-reliability-soak.sh \
  ...
  --process-root "$ENGINE_HOME/.dory/standalone" \
...
long_leftovers="$(bounded 15 docker_e ps -aq \
...
endurance_containers="$(bounded 15 docker_e ps -aq \
  --filter "label=dev.dory.endurance=$endurance_owner")" \
  || die "could not verify endurance container cleanup"
endurance_volumes="$(bounded 15 docker_e volume ls -q \
  --filter "label=dev.dory.endurance=$endurance_owner")" \
  || die "could not verify endurance volume cleanup"
endurance_networks="$(bounded 15 docker_e network ls -q \
  --filter "label=dev.dory.endurance=$endurance_owner")" \
...
final_validated_source_commit="$(python3 scripts/validate-release-metadata.py \
  "$BUILD_DIR" "$VERSION" "$BUILD")" \
  || die "signed release metadata changed during qualification"
[ "$final_validated_source_commit" = "$SOURCE_COMMIT" ] \
  || die "signed release metadata source authority changed during qualification"
[ "$(git rev-parse HEAD)" = "$SOURCE_COMMIT" ] \
  && git diff --quiet -- scripts .github/scripts \
  && git diff --cached --quiet -- scripts .github/scripts \
  || die "qualification harness authority changed during qualification"
[ "$(shasum -a 256 scripts/qualify-release-candidate.sh | awk '{print $1}')" \
    = "$QUALIFIER_SHA256" ] \
  && [ "$(shasum -a 256 scripts/validate-release-metadata.py | awk '{print $1}')" \
    = "$METADATA_VALIDATOR_SHA256" ] \
  || die "qualification harness bytes changed during qualification"
verify_candidate_digest() {
  local path="$1" expected="$2" label="$3"
  [ "$(shasum -a 256 "$path" | awk '{print $1}')" = "$expected" ] \
    || die "$label changed during qualification"
}
...
verify_candidate_digest "$COMPOSE" "$CANDIDATE_COMPOSE_SHA" "Compose helper"
verify_candidate_digest "$BUILDX" "$CANDIDATE_BUILDX_SHA" "Buildx helper"
verify_candidate_digest "$KUBECTL" "$CANDIDATE_KUBECTL_SHA" "kubectl component"
verify_candidate_digest "$KERNEL" "$CANDIDATE_KERNEL_SHA" "engine kernel"
verify_candidate_digest "$ROOTFS" "$CANDIDATE_ROOTFS_SHA" "engine rootfs"
verify_candidate_digest "$GUEST_AGENT" "$CANDIDATE_GUEST_AGENT_SHA" "guest agent"

manifest_sha="$(shasum -a 256 "$MANIFEST" | awk '{print $1}')"
update_sha="$(shasum -a 256 "$UPDATE_ZIP" | awk '{print $1}')"
component_catalog_sha="$(shasum -a 256 "$COMPONENT_CATALOG" | awk '{print $1}')"
component_catalog_digest_sha="$(shasum -a 256 \
  "$BUILD_DIR/components/arm64/catalog.json.sha256" | awk '{print $1}')"
component_catalog_signature_sha="$(shasum -a 256 \
  "$BUILD_DIR/components/arm64/catalog.json.sig" | awk '{print $1}')"
runtime_sha="$(shasum -a 256 "$RUNTIME_TAR" | awk '{print $1}')"
sbom_sha="$(shasum -a 256 "$BUILD_DIR/Dory-$VERSION.cdx.json" | awk '{print $1}')"
host_facts_sha="$(shasum -a 256 "$WORKDIR/evidence/host-facts.txt" | awk '{print $1}')"
CANDIDATE_BINDING="$WORKDIR/evidence/candidate-binding.txt"
cat > "$CANDIDATE_BINDING" <<EOF
schema_version=2
kind=dev.dory.release-qualification-candidate-binding
version=$VERSION
build=$BUILD
source_commit=$SOURCE_COMMIT
release_manifest_sha256=$manifest_sha
app_update_sha256=$update_sha
runtime_archive_sha256=$runtime_sha
sbom_sha256=$sbom_sha
component_catalog_schema=2
component_catalog_sha256=$component_catalog_sha
component_catalog_digest_file_sha256=$component_catalog_digest_sha
component_catalog_signature_sha256=$component_catalog_signature_sha
app_executable_sha256=$CANDIDATE_APP_EXECUTABLE_SHA
doryd_sha256=$CANDIDATE_DORYD_SHA
dory_vmm_sha256=$CANDIDATE_DORY_VMM_SHA
dory_hv_sha256=$CANDIDATE_DORY_HV_SHA
gvproxy_sha256=$CANDIDATE_GVPROXY_SHA
gvproxy_build_sha256=$CANDIDATE_GVPROXY_BUILD_SHA
dataplane_sha256=$CANDIDATE_DATAPLANE_SHA
docker_cli_sha256=$CANDIDATE_DOCKER_SHA
compose_sha256=$CANDIDATE_COMPOSE_SHA
buildx_sha256=$CANDIDATE_BUILDX_SHA
kubectl_sha256=$CANDIDATE_KUBECTL_SHA
kernel_sha256=$CANDIDATE_KERNEL_SHA
rootfs_sha256=$CANDIDATE_ROOTFS_SHA
guest_agent_sha256=$CANDIDATE_GUEST_AGENT_SHA
host_facts_sha256=$host_facts_sha
qualifier_sha256=$QUALIFIER_SHA256
metadata_validator_sha256=$METADATA_VALIDATOR_SHA256
developer_id=PASS
notarization=PASS
stapling=PASS
gatekeeper=PASS
...
status=PASS
EOF
candidate_binding_sha="$(shasum -a 256 "$CANDIDATE_BINDING" | awk '{print $1}')"
...
python3 - \
  ...
  "$manifest_sha" "$update_sha" "$component_catalog_sha" "$runtime_sha" "$evidence_sha" \
  ...
  "$IMAGE" "$REGISTRY_IMAGE" "$SSH_CLIENT_IMAGE" "$TESTCONTAINERS_RYUK_IMAGE" \
  "$ACT_RUNNER_IMAGE" "$candidate_binding_sha" "$component_catalog_signature_sha" \
  "$QUALIFIER_SHA256" "$METADATA_VALIDATOR_SHA256" "$completed_epoch" <<'PY'
import json
import sys

(
    output, release_qualifying, development_unnotarized, version, build, source_commit,
    run_id, run_attempt, manifest_sha, update_sha, component_catalog_sha, runtime_sha, evidence_sha,
    ...
    guest_agent_sha256, fixture_image, registry_image, ssh_client_image,
    testcontainers_ryuk_image, act_runner_image, candidate_binding_sha,
    component_catalog_signature_sha, qualifier_sha, metadata_validator_sha, completed_epoch,
) = sys.argv[1:]
payload = {
    "schemaVersion": 2,
    ...
    "componentCatalogSha256": component_catalog_sha,
    "componentCatalogSchemaVersion": 2,
    "componentCatalogSignatureSha256": component_catalog_signature_sha,
    "runtimeSha256": runtime_sha,
    "candidateBindingSha256": candidate_binding_sha,
    "qualificationHarnessSha256": qualifier_sha,
    "metadataValidatorSha256": metadata_validator_sha,
