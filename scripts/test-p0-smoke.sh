...
if state != "running":
    raise SystemExit(f"unexpected engine state: {state!r}")
PY
...
# Candidate compatibility output is validated as a closed contract rather than merely parsed.
COMPAT_WORKDIR="$TMP_ROOT/compat-work"
FAKE_DOCTOR="$TMP_ROOT/fake-dory-doctor"
mkdir -p "$COMPAT_WORKDIR"
cat > "$FAKE_DOCTOR" <<'PY'
#!/usr/bin/env python3
import json
import os
...
if sys.argv[1:] == ["compat", "--recipe", "--json"]:
    tools = {
        name: {"title": name, "steps": ["configure"], "verify": f"verify {name}"}
        for name in required
    }
    if os.environ.get("DORY_FAKE_COMPAT_INVALID") == "1":
        tools.pop("docker")
    print(json.dumps({"tools": tools}))
elif sys.argv[1:] == ["compat", "--json"]:
    print(json.dumps({"results": [
        {"id": f"compat.{name}", "status": "pass", "detail": "ok"}
        for name in required
    ]}))
else:
    raise SystemExit(64)
PY
chmod +x "$FAKE_DOCTOR"
WORKDIR="$COMPAT_WORKDIR" DORY_DOCTOR_BIN="$FAKE_DOCTOR" compatibility_smoke
if WORKDIR="$COMPAT_WORKDIR" DORY_DOCTOR_BIN="$FAKE_DOCTOR" \
    DORY_FAKE_COMPAT_INVALID=1 compatibility_smoke \
    > "$TMP_ROOT/invalid-compat.out" 2> "$TMP_ROOT/invalid-compat.err"; then
  echo "test-p0-smoke: malformed compatibility recipes unexpectedly passed" >&2
  exit 1
fi
grep -q 'exact supported tool set' "$TMP_ROOT/invalid-compat.err"

gate_text="$(cat scripts/p0-smoke.sh)"
for required in \
  'DORY_APP must be the absolute exact candidate app' \
...
if grep -Eq 'alpine:latest|scripts/(test-dory-doctor|compat-smoke)\.sh|assert ' <<< "$gate_text"; then
  echo "test-p0-smoke: gate retains a mutable fixture, ignored dependency, or Python assert" >&2
  exit 1
fi
