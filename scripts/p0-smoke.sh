...
compatibility_smoke() {
  local recipes="$WORKDIR/compat-recipes.json" results="$WORKDIR/compat-results.json"
  "$DORY_DOCTOR_BIN" compat --recipe --json > "$recipes"
  "$DORY_DOCTOR_BIN" compat --json > "$results" || true
  python3 - "$recipes" "$results" <<'PY'
import json
import sys
...
required = {
    "docker", "compose", "testcontainers", "act", "kubernetes", "vscode", "cursor",
    "supabase", "localstack", "amd64",
}
tools = recipe_payload.get("tools")
if not isinstance(tools, dict) or set(tools) != required:
    raise SystemExit("candidate compatibility recipes do not expose the exact supported tool set")
for name, recipe in tools.items():
    if not isinstance(recipe, dict):
        raise SystemExit(f"{name}: compatibility recipe is not an object")
    if not isinstance(recipe.get("title"), str) or not recipe["title"]:
        raise SystemExit(f"{name}: compatibility recipe has no title")
    if not isinstance(recipe.get("steps"), list) or not recipe["steps"]:
        raise SystemExit(f"{name}: compatibility recipe has no steps")
    if not isinstance(recipe.get("verify"), str) or not recipe["verify"]:
        raise SystemExit(f"{name}: compatibility recipe has no verification command")

results = result_payload.get("results")
if not isinstance(results, list) or not results:
    raise SystemExit("candidate compatibility diagnostics returned no results")
checked = set()
for result in results:
    if not isinstance(result, dict):
        raise SystemExit("candidate compatibility result is not an object")
    identifier = result.get("id")
    if not isinstance(identifier, str) or not identifier.startswith("compat."):
        raise SystemExit("candidate compatibility result has an invalid identifier")
    parts = identifier.split(".")
    if len(parts) < 2 or parts[1] not in required:
        raise SystemExit(f"candidate compatibility result has an unknown identifier: {identifier}")
    checked.add(parts[1])
    status = result.get("status")
    if status not in {"pass", "warn", "fail", "skip"}:
        raise SystemExit(f"{identifier}: invalid compatibility status")
    if status in {"warn", "fail"} and not (result.get("action") or result.get("detail")):
        raise SystemExit(f"{identifier}: non-pass compatibility result is not actionable")
if checked != required:
    raise SystemExit(f"candidate compatibility diagnostics omitted: {sorted(required - checked)}")
PY
}
...
main() {
  ...
  case "${DORY_APP:-}" in
    /*) ;;
    *) echo "p0-smoke: DORY_APP must be the absolute exact candidate app" >&2; exit 1 ;;
  esac
  [ -d "$DORY_APP" ] && [ ! -L "$DORY_APP" ] \
    || { echo "p0-smoke: exact candidate app is unavailable or indirect: $DORY_APP" >&2; exit 1; }
  for relative in Contents/Helpers/docker Contents/Helpers/dory Contents/Helpers/dorydctl \
      Contents/Helpers/dory-doctor; do
    [ -f "$DORY_APP/$relative" ] && [ ! -L "$DORY_APP/$relative" ] \
      && [ -x "$DORY_APP/$relative" ] \
      || { echo "p0-smoke: candidate helper is unavailable or indirect: $relative" >&2; exit 1; }
  done
  printf '%s\n' "$IMAGE" | grep -Eq '^.+@sha256:[0-9a-f]{64}$' \
    || { echo "p0-smoke: DORY_P0_IMAGE must be digest-pinned" >&2; exit 1; }
  DORY_SOCK="${DORY_SOCK:-$HOME/.dory/dory.sock}"
  DOCKER_BIN="$(resolve_docker_bin)"
  DORY_CLI_BIN="$(resolve_dory_cli)"
  DORYDCTL_BIN="$(resolve_dorydctl)"
  DORY_DOCTOR_BIN="${DORY_DOCTOR_BIN:-$DORY_APP/Contents/Helpers/dory-doctor}"
  [ "$DOCKER_BIN" = "$DORY_APP/Contents/Helpers/docker" ] \
    || { echo "p0-smoke: Docker CLI is not the exact candidate helper" >&2; exit 1; }
  [ "$DORY_CLI_BIN" = "$DORY_APP/Contents/Helpers/dory" ] \
    || { echo "p0-smoke: Dory CLI is not the exact candidate helper" >&2; exit 1; }
  [ "$DORYDCTL_BIN" = "$DORY_APP/Contents/Helpers/dorydctl" ] \
    || { echo "p0-smoke: dorydctl is not the exact candidate helper" >&2; exit 1; }
  [ "$DORY_DOCTOR_BIN" = "$DORY_APP/Contents/Helpers/dory-doctor" ] \
    || { echo "p0-smoke: dory-doctor is not the exact candidate helper" >&2; exit 1; }
  ...
  [ "$(stat -f %u "$DORY_SOCK")" = "$(id -u)" ] \
    || { echo "p0-smoke: Dory socket is not owned by the release user" >&2; exit 1; }
  docker_e image inspect "$IMAGE" >/dev/null 2>&1 \
    || { echo "p0-smoke: required offline fixture image is missing: $IMAGE" >&2; exit 1; }
