...
usage() {
  cat <<'EOF'
Usage: scripts/data-disk-growth-gate.sh --runtime DIR --docker PATH --confirm TOKEN [options]

Required:
  --runtime DIR       Exact runtime bundle containing dory-engine
  --docker PATH       Exact Docker CLI to use
  --confirm TOKEN     Must be ISOLATED-RUNTIME-DATA-DISK-GROWTH

Options:
  --image REF         Digest-pinned Linux image used for guest df + persistence
...
[ "$CONFIRM" = ISOLATED-RUNTIME-DATA-DISK-GROWTH ] \
  || { echo "data-disk growth gate: requires --confirm ISOLATED-RUNTIME-DATA-DISK-GROWTH" >&2; exit 2; }
...
case "$RUNTIME" in /*) ;; *) echo "data-disk growth gate: runtime must be absolute" >&2; exit 66 ;; esac
case "$DOCKER" in /*) ;; *) echo "data-disk growth gate: Docker CLI must be absolute" >&2; exit 66 ;; esac
[ -d "$RUNTIME" ] && [ ! -L "$RUNTIME" ] \
  || { echo "data-disk growth gate: runtime is unavailable or indirect" >&2; exit 66; }
for helper in "$RUNTIME/dory-engine" "$RUNTIME/bin/dory-hv" "$DOCKER"; do
  [ -f "$helper" ] && [ ! -L "$helper" ] && [ -x "$helper" ] \
    || { echo "data-disk growth gate: candidate helper is unavailable or indirect: $helper" >&2; exit 66; }
done
RUNTIME="$(cd "$RUNTIME" && pwd -P)"
DOCKER="$(cd "$(dirname "$DOCKER")" && pwd -P)/$(basename "$DOCKER")"
printf '%s\n' "$IMAGE" \
  | grep -Eq '^[A-Za-z0-9][A-Za-z0-9._:/-]*@sha256:[0-9a-f]{64}$' \
  || { echo "data-disk growth gate: --image must be an exact digest reference" >&2; exit 64; }
case "$WORKROOT" in /*) ;; *) echo "data-disk growth gate: workroot must be absolute" >&2; exit 73 ;; esac
case "$WORKROOT" in /|"$HOME"|"$(pwd)") echo "data-disk growth gate: unsafe workroot" >&2; exit 73 ;; esac
[ ! -e "$WORKROOT" ] && [ ! -L "$WORKROOT" ] \
  || { echo "data-disk growth gate: workroot already exists or is indirect" >&2; exit 73; }
for command in dd python3 shasum stat truncate; do
  command -v "$command" >/dev/null \
    || { echo "data-disk growth gate: $command is required" >&2; exit 69; }
done
...
[ -f "$MKE2FS" ] && [ ! -L "$MKE2FS" ] && [ -x "$MKE2FS" ] \
  || { echo "data-disk growth gate: mke2fs is required as a direct executable" >&2; exit 69; }

mkdir "$WORKROOT"
WORKROOT="$(cd "$WORKROOT" && pwd -P)"
RUN_ID="$(date -u +%Y%m%dT%H%M%SZ)-$$"
RUN_ROOT="$WORKROOT/$RUN_ID"
RUNTIME_HOME="${DORY_DATA_DISK_RUNTIME_HOME:-${TMPDIR:-/tmp}/dory-ddg-$UID-$$}"
RUNTIME_HOME="$(python3 - "$RUNTIME_HOME" <<'PY'
import os
import sys
...
LABEL="dev.dory.data-disk-growth=$RUN_ID"
[ "${RUNTIME_HOME#/}" != "$RUNTIME_HOME" ] \
  || { echo "data-disk growth gate: runtime HOME must be absolute" >&2; exit 73; }
case "$RUNTIME_HOME" in /|"$HOME"|"$(pwd)"|"$WORKROOT"|"$WORKROOT"/*) \
  echo "data-disk growth gate: unsafe runtime HOME" >&2; exit 73 ;; esac
[ ! -e "$RUNTIME_HOME" ] || {
...
cleanup() {
  ...
  if [ -S "$SOCKET" ]; then
    docker_e ps -aq --filter "label=$LABEL" 2>/dev/null \
      | while IFS= read -r id; do
          [ -z "$id" ] || docker_e rm -f "$id" >/dev/null 2>&1 || true
        done
    [ "$volume_created" -eq 0 ] || docker_e volume rm "$NAME" >/dev/null 2>&1 || true
  fi
...
engine_started=1
[ -S "$SOCKET" ] && [ "$(stat -f %u "$SOCKET")" = "$(id -u)" ] \
  || { echo "data-disk growth gate: runtime socket is unavailable or owned by another user" >&2; exit 1; }
...
docker_e image inspect "$IMAGE" >/dev/null 2>&1 \
  || docker_e pull "$IMAGE" >"$EVIDENCE/pull.log" 2>&1
docker_e image inspect "$IMAGE" > "$EVIDENCE/image-inspect.json" \
  || { echo "data-disk growth gate: exact probe image is unavailable after pull" >&2; exit 1; }
[ "$(docker_e volume create --label "$LABEL" "$NAME")" = "$NAME" ] \
  || { echo "data-disk growth gate: named-volume creation returned the wrong authority" >&2; exit 1; }
volume_created=1
docker_e volume inspect "$NAME" > "$EVIDENCE/volume-inspect.json"
python3 - "$EVIDENCE/image-inspect.json" "$EVIDENCE/volume-inspect.json" \
  "$IMAGE" "$NAME" "$LABEL" <<'PY'
import json
import pathlib
import sys

image_document = json.loads(pathlib.Path(sys.argv[1]).read_text(encoding="utf-8"))
volume_document = json.loads(pathlib.Path(sys.argv[2]).read_text(encoding="utf-8"))
if not isinstance(image_document, list) or len(image_document) != 1:
    raise SystemExit("probe image inspect is not one exact image")
image = image_document[0]
requested_name, requested_digest = sys.argv[3].rsplit("@", 1)
...
if not isinstance(repo_digests, list) or not any(
    ...
    raise SystemExit("local probe image does not retain its exact registry authority")
if not isinstance(volume_document, list) or len(volume_document) != 1:
    raise SystemExit("growth volume inspect is not one exact volume")
volume = volume_document[0]
label_key, label_value = sys.argv[5].split("=", 1)
labels = volume.get("Labels")
if volume.get("Name") != sys.argv[4] or not isinstance(labels, dict) or labels.get(label_key) != label_value:
    raise SystemExit("growth volume differs from its exact run authority")
PY

GUEST_KIB="$(docker_e run --rm --pull=never --network none --label "$LABEL" \
  --mount "type=volume,src=$NAME,dst=/data" "$IMAGE" \
  sh -c "df -Pk /data | awk 'NR==2 {print \$2}'")"
...
printf '%s\n' "$NAME" > "$EVIDENCE/volume-create.txt"
docker_e run --name "$NAME" --rm --pull=never --network none --label "$LABEL" \
  --mount "type=volume,src=$NAME,dst=/data" "$IMAGE" \
...
PERSISTED="$(docker_e run --name "$NAME" --rm --pull=never --network none --label "$LABEL" \
  --mount "type=volume,src=$NAME,dst=/data" "$IMAGE" cat /data/marker)"
...
grown = json.loads(pathlib.Path(sys.argv[2]).read_text(encoding="utf-8"))
required = {
    "initialized", "logicalBytes", "allocatedBytes", "capacityGiB",
    "minimumCapacityGiB", "maximumCapacityGiB",
}
if not isinstance(before, dict) or not required.issubset(before):
    raise SystemExit("pre-growth capacity response has an unexpected shape")
if not isinstance(grown, dict) or not required.issubset(grown):
    raise SystemExit("grown capacity response has an unexpected shape")
if before["capacityGiB"] != 128 or before["initialized"] is not True:
    raise SystemExit("pre-growth capacity response is not initialized at 128 GiB")
if grown["capacityGiB"] != 256 or grown["logicalBytes"] != 256 * 1024**3:
    raise SystemExit("explicit growth response is not exactly 256 GiB")
if not isinstance(grown["allocatedBytes"], int) or grown["allocatedBytes"] >= grown["logicalBytes"]:
    raise SystemExit("explicit growth response is not sparse")
if grown["minimumCapacityGiB"] != 128 or grown["maximumCapacityGiB"] != 2048:
    raise SystemExit("explicit growth response has unexpected capacity bounds")
PY
...
USER_GUEST_KIB="$(docker_e run --rm --pull=never --network none --label "$LABEL" \
  --mount "type=volume,src=$NAME,dst=/data" "$IMAGE" \
  sh -c "df -Pk /data | awk 'NR==2 {print \$2}'")"
...
USER_PERSISTED="$(docker_e run --name "$NAME" --rm --pull=never --network none --label "$LABEL" \
  --mount "type=volume,src=$NAME,dst=/data" "$IMAGE" cat /data/marker)"
...
[ -z "$(docker_e ps -aq --filter "label=$LABEL")" ] \
  || { echo "data-disk growth gate: owned probe container survived completion" >&2; exit 1; }
docker_e volume rm "$NAME" > "$EVIDENCE/volume-remove.txt" \
  || { echo "data-disk growth gate: owned volume cleanup failed" >&2; exit 1; }
volume_created=0
...
status=PASS
image=$IMAGE
dory_engine_sha256=$(shasum -a 256 "$RUNTIME/dory-engine" | awk '{print $1}')
dory_hv_sha256=$(shasum -a 256 "$RUNTIME/bin/dory-hv" | awk '{print $1}')
