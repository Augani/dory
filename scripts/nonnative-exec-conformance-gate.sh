...
base_image_id="$(docker_e image inspect -f '{{.Id}}' "$BASE_IMAGE")"
native_image_id="$(docker_e image inspect -f '{{.Id}}' "$NATIVE_IMAGE")"
...
for path, architecture in zip(sys.argv[1:], ("amd64", "arm64")):
    ...
    if not isinstance(payload, list) or len(payload) != 1:
        raise SystemExit(f"image inspect has an unexpected shape: {payload!r}")
    if payload[0].get("Os") != "linux" or payload[0].get("Architecture") != architecture:
        raise SystemExit(f"image platform does not match linux/{architecture}: {payload[0]!r}")
PY
...
for key in ("FEX_INTERPRETER_INSTALLED", "FEX_EXECVEFD", "FEX_SECCOMPFD"):
    if key in os.environ:
        raise SystemExit(f"private FEX handoff marker leaked: {key}")
...
if libc.prctl(38, 1, 0, 0, 0) != 0:
    raise SystemExit(f"PR_SET_NO_NEW_PRIVS failed: errno={ctypes.get_errno()}")
if libc.prctl(22, 2, ctypes.byref(program), 0, 0) != 0:
    raise SystemExit(f"PR_SET_SECCOMP failed: errno={ctypes.get_errno()}")
...
if sys.argv != ["/opt/dory-exec/chain-b.py", "from-shell"]:
    raise SystemExit(f"shebang argument vector changed: {sys.argv!r}")
for key in ("FEX_INTERPRETER_INSTALLED", "FEX_EXECVEFD", "FEX_SECCOMPFD"):
    if key in os.environ:
        raise SystemExit(f"private FEX handoff marker leaked: {key}")
try:
    ...
    if error.errno != errno.EACCES:
        raise SystemExit(f"unexpected mkdir failure under guest seccomp: {error}")
else:
    raise SystemExit("mkdir bypassed inherited guest seccomp")
mkdir = subprocess.run(["/bin/mkdir", "/tmp/dory-seccomp-child"], check=False,
                       stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
if mkdir.returncode == 0 or "Permission denied" not in mkdir.stderr:
    raise SystemExit(f"child mkdir bypassed inherited guest seccomp: {mkdir!r}")
version = subprocess.check_output(["/bin/cat", "/etc/debian_version"], text=True).strip()
if not version:
    raise SystemExit("Debian version probe returned no output")
...
[ "$RELEASE_CANDIDATE" -eq 0 ] || release_qualifying=true
{
  ...
  echo "native_image_id=$native_image_id"
  ...
  echo "isolated_builder_cache_prune=PASS"
