...
DORY_DATA_SCHEMA_VERSION="${DORY_DATA_SCHEMA_VERSION:-1}"
DORY_MINIMUM_READABLE_DATA_SCHEMA="${DORY_MINIMUM_READABLE_DATA_SCHEMA:-1}"
DORY_MAXIMUM_READABLE_DATA_SCHEMA="${DORY_MAXIMUM_READABLE_DATA_SCHEMA:-1}"
DORY_COMPONENT_CATALOG_SCHEMA="${DORY_COMPONENT_CATALOG_SCHEMA:-1}"
...
for schema_value in \
  "$DORY_DATA_SCHEMA_VERSION" \
  "$DORY_MINIMUM_READABLE_DATA_SCHEMA" \
  "$DORY_MAXIMUM_READABLE_DATA_SCHEMA" \
  "$DORY_COMPONENT_CATALOG_SCHEMA"; do
  case "$schema_value" in
    ''|*[!0-9]*|0) echo "appcast error: Dory schema values must be positive integers" >&2; exit 1 ;;
  esac
done
[ "$DORY_MINIMUM_READABLE_DATA_SCHEMA" -le "$DORY_DATA_SCHEMA_VERSION" ] \
  && [ "$DORY_DATA_SCHEMA_VERSION" -le "$DORY_MAXIMUM_READABLE_DATA_SCHEMA" ] \
  || { echo "appcast error: candidate readable data-schema range excludes its target schema" >&2; exit 1; }
...
<rss version="2.0" xmlns:sparkle="http://www.andymatuschak.org/xml-namespaces/sparkle" xmlns:dc="http://purl.org/dc/elements/1.1/" xmlns:dory="https://augani.github.io/dory/appcast">
  <channel>
    ...
    <item>
      ...
      <dory:dataSchemaVersion>$(xml_escape "$DORY_DATA_SCHEMA_VERSION")</dory:dataSchemaVersion>
      <dory:minimumReadableDataSchema>$(xml_escape "$DORY_MINIMUM_READABLE_DATA_SCHEMA")</dory:minimumReadableDataSchema>
      <dory:maximumReadableDataSchema>$(xml_escape "$DORY_MAXIMUM_READABLE_DATA_SCHEMA")</dory:maximumReadableDataSchema>
      <dory:componentCatalogSchema>$(xml_escape "$DORY_COMPONENT_CATALOG_SCHEMA")</dory:componentCatalogSchema>
