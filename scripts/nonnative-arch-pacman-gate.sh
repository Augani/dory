...
if not isinstance(payload, list) or len(payload) != 1:
    raise SystemExit("Arch image inspect is not singular")
if payload[0].get("Os") != "linux" or payload[0].get("Architecture") != "amd64":
    raise SystemExit("Arch fixture is not linux/amd64")
PY
