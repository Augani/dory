...
[ -d "$CANDIDATE_DIR" ] && [ ! -L "$CANDIDATE_DIR" ] \
  || die "--candidate-dir must be a direct directory"
...
[ "$CANDIDATE_DIR" = "$CANDIDATE_LOGICAL" ] \
  || die "--candidate-dir cannot have an indirect ancestor"
case "$WORKROOT" in
  /*) ;;
...
case "$WORKROOT_NAME" in
  dory-homebrew-install|dory-homebrew-install.*) ;;
  *) die "--workroot must use the dedicated dory-homebrew-install name" ;;
esac
...
case "$WORKROOT" in
  "$TEMP_AUTHORITY"/*) ;;
  *) die "--workroot must be inside the runner temporary directory" ;;
esac
...
for row in json.loads(pathlib.Path(sys.argv[2]).read_text(encoding="utf-8")):
    ...
    kind = row["kind"]
    if kind == "missing":
        require(not path.exists() and not path.is_symlink(), f"created Docker plugin remains: {path}")
    elif kind == "symlink":
        require(path.is_symlink() and os.readlink(path) == row["target"], f"Docker plugin symlink changed: {path}")
    elif kind == "file":
        require(path.is_file() and not path.is_symlink(), f"Docker plugin type changed: {path}")
        require(hashlib.sha256(path.read_bytes()).hexdigest() == row["sha256"], f"Docker plugin bytes changed: {path}")
        require(stat.S_IMODE(path.stat().st_mode) == row["mode"], f"Docker plugin mode changed: {path}")
    elif kind == "directory":
        require(path.is_dir() and not path.is_symlink(), f"Docker plugin directory changed: {path}")
        require(stat.S_IMODE(path.stat().st_mode) == row["mode"], f"Docker plugin directory mode changed: {path}")
PY
}
...
for helper in docker docker-compose docker-buildx kubectl dory; do
  [ ! -e "$HOME/.dory/bin/$helper" ] && [ ! -L "$HOME/.dory/bin/$helper" ] \
    || die "Homebrew install mutated terminal integration before first app launch: $helper"
done
...
grep -F 'source=Notarized Developer ID' "$EVIDENCE/gatekeeper.log" >/dev/null \
  || die "Homebrew-installed app is not accepted as Notarized Developer ID"
...
for helper in docker docker-compose docker-buildx kubectl dory; do
  [ -L "$HOME/.dory/bin/$helper" ] \
    || die "first app launch did not install $helper terminal integration"
done
...
require(data.get("kind") == "dev.dory.data-drive", "data-drive kind mismatch")
require(data.get("schemaVersion") == 1, "data-drive schema mismatch")
require(data.get("product") == "Dory", "data-drive product mismatch")
drive_id = uuid.UUID(data["id"])
require(selection.get("schemaVersion") == 2, "selection schema mismatch")
require(selection.get("phase") == "ready", "selection is not ready")
require(uuid.UUID(selection["driveID"]) == drive_id, "selection drive identity mismatch")
PY
...
for row in json.loads(pathlib.Path(sys.argv[2]).read_text(encoding="utf-8")):
    path = home / row["name"]
    if row["kind"] == "missing":
        require(not path.exists() and not path.is_symlink(), f"created profile remains: {path}")
        continue
    require(path.exists() or path.is_symlink(), f"profile is missing: {path}")
    if row["kind"] == "symlink":
        require(path.is_symlink() and os.readlink(path) == row["link"], f"profile symlink changed: {path}")
    else:
        require(not path.is_symlink(), f"profile became a symlink: {path}")
    target = path.resolve(strict=True)
    require(hashlib.sha256(target.read_bytes()).hexdigest() == row["sha256"], f"profile bytes changed: {path}")
    require(stat.S_IMODE(target.stat().st_mode) == row["mode"], f"profile mode changed: {path}")
PY
...
for row in json.loads(pathlib.Path(sys.argv[2]).read_text(encoding="utf-8")):
    path = home / row["name"]
    if row["kind"] == "missing":
        require(not path.exists() and not path.is_symlink(), f"created profile remains after zap: {path}")
        continue
    require(path.exists() or path.is_symlink(), f"profile is missing after zap: {path}")
    if row["kind"] == "symlink":
        require(path.is_symlink() and os.readlink(path) == row["link"], f"profile symlink changed after zap: {path}")
    else:
        require(not path.is_symlink(), f"profile became a symlink after zap: {path}")
    target = path.resolve(strict=True)
    require(hashlib.sha256(target.read_bytes()).hexdigest() == row["sha256"], f"profile bytes changed after zap: {path}")
    require(stat.S_IMODE(target.stat().st_mode) == row["mode"], f"profile mode changed after zap: {path}")
PY
