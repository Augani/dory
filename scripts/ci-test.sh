...
if [ -n "${DORY_CI_TEST_LOG:-}" ]; then
  ...
  [ -d "$(dirname "$LOG")" ] && [ ! -L "$(dirname "$LOG")" ] \
    || { echo "ci-test: log parent must be a direct directory" >&2; exit 64; }
else
...
if ! bash scripts/test-security-contracts.sh; then
  echo "ci-test: security boundary contracts failed" >&2
  exit 1
fi

if ! bash scripts/test-destructive-action-contracts.sh; then
  echo "ci-test: destructive action contracts failed" >&2
  exit 1
fi
...
if ! bash scripts/staged-readiness-resource-gate.sh; then
  echo "ci-test: staged readiness/resource contract failed" >&2
  exit 1
fi

if ! bash scripts/corporate-connectivity-gate.sh; then
  echo "ci-test: corporate connectivity contract failed" >&2
  exit 1
fi

if ! bash scripts/transactional-upgrade-gate.sh; then
  echo "ci-test: transactional upgrade/rollback contract failed" >&2
  exit 1
fi
...
if ! bash scripts/test-benchmark-developer-workflows.sh; then
  echo "ci-test: developer-workflow benchmark and release-performance safety tests failed" >&2
  exit 1
fi
...
for attempt in 1 2; do
  # `test.sh` now has explicit suite modes. Keep this retry focused on the hosted app tests; Rust,
  # Swift packages, gvproxy, and UI each have their own workflow steps and must not be repeated here.
  bash scripts/test.sh app -- -skip-testing:DoryUITests 2>&1 | tee "$LOG"
