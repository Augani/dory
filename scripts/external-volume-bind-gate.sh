...
IMAGE="${DORY_EXTERNAL_VOLUME_IMAGE:-}"
FD_BUDGET=8
CONFIRM=""
DISCONNECT_CONFIRM=""
...
[ -n "$SOCKET" ] && [ -S "$SOCKET" ] || die "candidate socket is unavailable: $SOCKET"
[ "$(stat -f %u "$SOCKET")" = "$(id -u)" ] \
  || die "candidate socket is not owned by the release user"
[ -f "$DOCKER" ] && [ ! -L "$DOCKER" ] && [ -x "$DOCKER" ] \
  || die "Docker CLI is not an exact regular executable: $DOCKER"
[ -f "$DORY" ] && [ ! -L "$DORY" ] && [ -x "$DORY" ] \
  || die "Dory CLI is not an exact regular executable: $DORY"
[ -d "$STATE_DIR" ] && [ ! -L "$STATE_DIR" ] \
  || die "candidate state directory is unavailable or indirect: $STATE_DIR"
...
printf '%s\n' "$IMAGE" | grep -Eq '^.+@sha256:[0-9a-f]{64}$' \
  || die "--image must be a digest-pinned fixture"
case "$FD_BUDGET" in ''|*[!0-9]*) die "--fd-growth must be a non-negative integer" ;; esac
[ "$FD_BUDGET" -le 64 ] || die "--fd-growth must not exceed 64"
...
disk_plist="$(mktemp "${TMPDIR:-/tmp}/dory-external-disk.XXXXXXXX")"
if ! diskutil info -plist "$volume_device" > "$disk_plist"; then
  rm -f "$disk_plist"
  die "could not inspect external APFS device"
fi
if ! disk_json="$(plutil -convert json -o - "$disk_plist")"; then
  rm -f "$disk_plist"
  die "external APFS device metadata is invalid"
fi
rm -f "$disk_plist"
...
[ -d "$HOST_DIR" ] && [ ! -L "$HOST_DIR" ] \
  ...
  && [ -d "$WORKDIR" ] && [ ! -L "$WORKDIR" ] \
  || die "gate workspace changed while preparing fixtures"
