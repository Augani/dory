...
while [ "$#" -gt 0 ]; do
  case "$1" in
    --engines) need_value "$1" "$#"; ENGINES="$2"; shift 2 ;;
    --memory-count) need_value "$1" "$#"; MEMORY_COUNT="$2"; shift 2 ;;
    --settle) need_value "$1" "$#"; SETTLE="$2"; shift 2 ;;
...
engine_socket() {
  ...
  case "$engine" in
    ...
    current)
      "$DOCKER_BIN" context inspect "$("$DOCKER_BIN" context show)" \
        --format '{{ (index .Endpoints "docker").Host }}' 2>/dev/null | sed 's#^unix://##'
      ;;
    *)
      echo "$engine"
      ;;
...
compose_e() {
  DOCKER_HOST="unix://$ENGINE_SOCK" "$DOCKER_BIN" compose "$@"
}
...
require_socket() {
  [ -S "$ENGINE_SOCK" ] || return 1
  if [ "$STRICT" = "1" ]; then
    [ "$(stat -f %u "$ENGINE_SOCK" 2>/dev/null || printf invalid)" = "$(id -u)" ] || return 1
  fi
}
...
test_machines_legacy_container() {
  ...
  local machine_image="${READINESS_MACHINE_CONTAINER_IMAGE:-}"
  printf '%s\n' "$machine_image" \
    | grep -Eq '^[A-Za-z0-9][A-Za-z0-9._:/-]*@sha256:[0-9a-f]{64}$' || {
    echo "READINESS_MACHINE_CONTAINER_IMAGE must be digest-pinned for the legacy container probe"
    return 1
  }
  mkdir -p "$dir"
  cat > "$dir/Dockerfile" <<EOF
...
measure_memory() {
  ...
  cleanup_engine
  docker_e image inspect "$image" >/dev/null 2>&1 || return 1
  sleep "$SETTLE"
  base="$(used_mem)"
...
validate_configuration() {
  ...
  case "${DORY_REQUIRE_VPN:-0}" in 0|1) ;; *) echo "DORY_REQUIRE_VPN must be 0 or 1" >&2; return 2 ;; esac

  for item in "memory-count:$MEMORY_COUNT:1:32" "settle:$SETTLE:0:300" \
      "build-context-mb:$BUILD_CONTEXT_MB:1:1024" \
      "clock-sync-tolerance-ms:$CLOCK_SYNC_TOLERANCE_MS:1:60000" \
      "ready-timeout:${READINESS_READY_TIMEOUT:-90}:1:600"; do
    name="${item%%:*}"
    rest="${item#*:}"
    value="${rest%%:*}"
    rest="${rest#*:}"
    minimum="${rest%%:*}"
    maximum="${rest##*:}"
    case "$value" in ''|*[!0-9]*) echo "$name must be an integer" >&2; return 2 ;; esac
  ...
  case "$WORKROOT" in /*) ;; *) echo "READINESS_WORKDIR must be absolute" >&2; return 2 ;; esac
  ...
  if [ "$RUN_NONNATIVE_ARCH" = "1" ]; then
    printf '%s\n' "$NONNATIVE_BUILD_IMAGE" \
      | grep -Eq '^[A-Za-z0-9][A-Za-z0-9._:/-]*@sha256:[0-9a-f]{64}$' || {
      echo "READINESS_NONNATIVE_BUILD_IMAGE must be an exact digest reference" >&2
      return 2
    }
  fi
  if [ "$RUN_FILE_WATCH" = "1" ]; then
    printf '%s\n' "${READINESS_FILE_WATCH_IMAGE:-$ALPINE_IMAGE}" \
      | grep -Eq '^[A-Za-z0-9][A-Za-z0-9._:/-]*@sha256:[0-9a-f]{64}$' || {
        echo "READINESS_FILE_WATCH_IMAGE must be an exact digest reference" >&2
        return 2
      }
  fi

  if [ "$STRICT" = "1" ] && [ -z "${READINESS_DOCKER_BIN:-}" ]; then
    echo "strict readiness requires READINESS_DOCKER_BIN for the exact candidate CLI" >&2
    return 2
  fi
  if [ -z "$DOCKER_BIN" ]; then
    DOCKER_BIN="$(command -v docker 2>/dev/null || true)"
  fi
  case "$DOCKER_BIN" in /*) ;; *) echo "readiness Docker CLI must be an absolute path" >&2; return 2 ;; esac
  if [ "$STRICT" = "1" ]; then
    [ -f "$DOCKER_BIN" ] && [ ! -L "$DOCKER_BIN" ] && [ -x "$DOCKER_BIN" ] || {
      echo "strict readiness Docker CLI is unavailable or indirect: $DOCKER_BIN" >&2
      return 2
    }
  else
    resolved="$(python3 - "$DOCKER_BIN" <<'PY'
import os
import sys
...
PY
    ...
    [ -f "$resolved" ] && [ -x "$resolved" ] || {
      echo "readiness Docker CLI is unavailable: $DOCKER_BIN" >&2
      return 2
    }
    DOCKER_BIN="$resolved"
  fi
  ...
  if [ "$STOP_ORBSTACK" = "1" ] \
      && [ "${READINESS_STOP_ORBSTACK_CONFIRMED:-}" != STOP-ORBSTACK-FOR-READINESS ]; then
    echo "STOP_ORBSTACK requires READINESS_STOP_ORBSTACK_CONFIRMED=STOP-ORBSTACK-FOR-READINESS" >&2
    return 2
  fi
  ...
  for engine in $ENGINES; do
    ...
    [ -n "$engine" ] || { echo "engine list contains an empty entry" >&2; return 2; }
    if [ "$STRICT" = "1" ]; then
      case "$engine" in dory|doryd|orbstack|docker-desktop|desktop|current) ;;
        *) echo "strict readiness rejects unknown engine: $engine" >&2; return 2 ;;
      esac
    fi
    IFS=','
  done
...
[ -d "$WORKROOT" ] && [ ! -L "$WORKROOT" ] \
  && [ -d "$WORKDIR" ] && [ ! -L "$WORKDIR" ] || {
    echo "readiness evidence root changed while it was being prepared" >&2
    exit 2
  }
