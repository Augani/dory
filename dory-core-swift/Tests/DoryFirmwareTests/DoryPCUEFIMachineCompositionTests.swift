...
@Suite struct DoryPCUEFIMachineCompositionTests {
  @Test func composesVerifiedFirmwareVariablesAndInstallerStorageIntoOneMachine() throws {
    ...
    #expect(composed.usbKeyboardDevice.profile == .keyboard)
    #expect(try composed.xhciController.portState(1).connected)
    #expect(try composed.xhciController.portState(1).powered)
    #expect(try composed.xhciController.portState(1).speed == .high)
