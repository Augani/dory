...
@Suite struct DoryX86LZCNTConsistencyTests {
  ...
  // The decoder must still produce .bitScan for 0F BC (BSF) even with F3.
  // F3 0F BC is TZCNT (BMI1), not LZCNT; it remains decoded as BSF for now.
  @Test func decoderProducesBitScanForF3PrefixedBSF() throws {
    let decoder = DoryX86Decoder()
    let instruction = try decoder.decode([0xF3, 0x0F, 0xBC, 0xC0], at: 0, mode: .long64)
    guard case .bitScan(let reverse, _, _) = instruction.operation else {
      Issue.record("expected .bitScan for F3 0F BC, got \(instruction.operation)")
      return
    }
