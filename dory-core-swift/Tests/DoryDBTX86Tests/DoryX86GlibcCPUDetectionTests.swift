...
@Suite struct DoryX86GlibcCPUDetectionTests {
  @Test func existingV1VendorSkipsLeafOneAndProducesZeroISALevel() throws {
    for tier in tiers {
      ...
      #expect(dispatch.queries == [.init(0, 0), .init(7, 0), .init(7, 1), .init(0xD, 1)])
      #expect(dispatch.visited.contains(0x1A89E))
      #expect(dispatch.visited.contains(0x19642))
      #expect(try memory.readScalar(at: 0x37B30, byteCount: 4) == 0)
      #expect(try memory.readScalar(at: 0x37B2C, byteCount: 4) == 0)
      ...
      #expect(update.queries.isEmpty)
      #expect(try memory.readScalar(at: 0x37B40, byteCount: 4) == 0)
      #expect(try memory.readScalar(at: 0x37C68, byteCount: 4) == 0)
      if tier != nil { #expect(dispatch.nativeInstructions + update.nativeInstructions > 0) }
      ...
      #expect(leaf.edx & baselineMask == baselineMask)
    }
  }

  @Test func theSameAdvertisedBitsPropagateAndPassTheActualBaselineCheck() throws {
    for tier in tiers {
      let memory = try fixture()
      let leaf = DoryX86CPUProfile.compatibleV1.cpuid(leaf: 1)
      try memory.writeScalar(at: 0x37B24, value: UInt64(leaf.eax), byteCount: 4)
      try memory.writeScalar(at: 0x37B28, value: UInt64(leaf.ebx), byteCount: 4)
      ...
      let update = try replay(state: &state, memory: memory, tier: tier, until: 0x190CA)
      #expect(update.visited.contains(0x190BB)) // TEST CH,0x81 checks CX8 and CMOV.
      #expect(update.visited.contains(0x192E0)) // Raw FPU, then MMX/FXSR/SSE/SSE2.
      #expect(try memory.readScalar(at: 0x37B40, byteCount: 4)
        == UInt64(leaf.edx & 0x1788_8110))
      #expect(try memory.readScalar(at: 0x37C68, byteCount: 4) == 1)
      if tier != nil { #expect(update.nativeInstructions > 0) }
    }
  }

  @Test func intelVendorTupleSelectsTheFamilyAwareBranchWithoutChangingFeatureBits() throws {
    let memory = try fixture()
    ...
    #expect(!result.visited.contains(0x19642))
  ...
  private func replay(state: inout DoryX86ArchitecturalState, memory: DoryX86ByteArrayMemory,
    ...
    for _ in 0..<400 {
      ...
      let rip = state.rip
      let chunk = try #require(chunks.first { rip >= $0.0 && rip < $0.0 + UInt64($0.1.count) })
    ...
    Issue.record("glibc fragment exceeded the bounded replay instruction budget")
    return result
  }
