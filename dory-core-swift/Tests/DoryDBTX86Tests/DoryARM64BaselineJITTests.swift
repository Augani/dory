...
@Suite struct DoryARM64BaselineJITTests {
  ...
  @Test func singleReadLegacyMemorySourceChainsIntoTier1ThroughCanonicalEntryTuple() throws {
    #if arch(arm64)
  ...
  @Test func legacyStackWriterRemainsSeparatedFromTier1Target() throws {
    #if arch(arm64)
      let base: UInt64 = 0x1A00
      // call target; mov rax,0x12345678
      let bytes: [UInt8] = [
        0xE8, 0, 0, 0, 0,
      ...
      let rejectionsBefore = executor.diagnostics.chainTargetCompilerABIRejections
      let cold = try run()
      #expect(cold.0.guestInstructionCount == 2)
      #expect(cold.0.residentBlockCount == 2)
      #expect(cold.1.registers.rax == 0x1234_5678)
      #expect(cold.1.registers.rsp == 0x78)
      #expect(cold.1.rip == base + UInt64(bytes.count))
      #expect(try memory.readScalar(at: 0x78, byteCount: 8) == base + 5)
      #expect(cold.2 == 2)
      #expect(executor.diagnostics.directChainPatches == 0)
      #expect(executor.diagnostics.chainTargetAttempts == attemptsBefore + 1)
      #expect(executor.diagnostics.chainTargetCompilerABIRejections == rejectionsBefore + 1)
      ...
      #expect(executor.diagnostics.chainTargetCompilerABIRejections == rejectionsBefore + 2)
    #endif
  }
  ...
  @Test func nativePageTableWriteInvalidatesAnEarlierInlineReadBeforeBlockContinues() throws {
      ...
      var state = try DoryX86ArchitecturalState(
        registers: .init(rax: 0x4000, rdx: 0x0040_0000, rbx: 0x9000 | 0x7),
        rip: 0x6000,
        control: .init(
          cr0: 0x8001_0011,
          cr3: 0x1000,
      ...
      state.rip = 0x7000
      translated.updateContext(.init(state: state, mode: .long64))
      let bytes: [UInt8] = [0x48, 0x89, 0x18, 0x48, 0x8B, 0x0A]
      ...
      #expect(remapAndRead.guestInstructionCount == 2)
      #expect(state.registers.rcx == 0x2222)
      #expect(paging.diagnostics.globalInvalidations == 1)
    #endif
  }
  ...
  @Test func registerPushAndPopMatchInterpreterAcrossTiers() throws {
      ...
      for value: UInt64 in [0, 0x180, 0x0000_8000_0000_0000, .max] {
        cases.append(StackCase(bytes: [0x5C], registers: .init(rsp: 0x100), stackValue: value))
      }
      let flags: DoryX86RFLAGS = [
      ...
      for optimization in [DoryARM64JITOptimization.baseline, .optimizing] {
        for testCase in cases {
          ...
          let decoded = try DoryX86Decoder().decode(testCase.bytes, at: 0x20, mode: .long64)
  ...
  @Test func registerStackMemoryFailuresLeaveArchitecturalStateRestartable() throws {
      ...
      let cases: [([UInt8], DoryX86GeneralRegisters)] = [
        ...
        ([0x5C], .init(rsp: 0x100)),
        ([0x5C], .init(rsp: 0xFC)),
        ([0x5C], .init(rsp: 0x0000_8000_0000_0000)),
      ]
  ...
  @Test func measuredRegisterStackSitesCompileNativelyWhileComplexFormsStayBounded() throws {
    let measured: [([UInt8], UInt64)] = [
      ...
      ([0x5C], 0xFFFF_FFFF_88D6_B18B),  // measured pop rsp
  ...
  @Test func alignedLockedCompareExchangeUsesTranslatedHostAtomicPath() throws {
    #if arch(arm64)
      let physical = try DoryX86MmapMemory(validatingByteCount: Int(getpagesize()))
      let paging = DoryX86PagingUnit()
      var state = try DoryX86ArchitecturalState(
      ...
      let executor = try DoryARM64BaselineExecutor(maximumCodeBytes: 4_096)
      let bytes: [UInt8] = [0xF0, 0x48, 0x0F, 0xB1, 0x0A]
      ...
      #expect(executor.diagnostics.translationCacheMisses == 1)
      #expect(executor.diagnostics.translationCacheFills == 1)
      #expect(executor.diagnostics.translationCacheFallbacks == 0)
    #endif
  }

  @Test func unalignedLockedCompareExchangeUsesSerializedFallback() throws {
    #if arch(arm64)
      let physical = try DoryX86MmapMemory(validatingByteCount: Int(getpagesize()))
      let paging = DoryX86PagingUnit()
      var state = try DoryX86ArchitecturalState(
      ...
      #expect(try physical.readScalar(at: 0x81, byteCount: 8) == 0x8877_6655_4433_2211)
      #expect(state.rflags.contains(.zero))
      #expect(executor.diagnostics.translationCacheMisses == 1)
      #expect(executor.diagnostics.translationCacheFills == 1)
      #expect(executor.diagnostics.translationCacheFallbacks == 0)
  ...
  @Test func lockedExchangeAddUsesTranslatedHostAtomicPath() throws {
    #if arch(arm64)
      let bytes: [UInt8] = [0xF0, 0x48, 0x0F, 0xC1, 0x0A]
      let instructionAddress: UInt64 = 0x3700
      for optimization in [DoryARM64JITOptimization.baseline, .optimizing] {
        let physical = try DoryX86MmapMemory(validatingByteCount: Int(getpagesize()))
        let interpretedMemory = try DoryX86ByteArrayMemory(byteCount: Int(getpagesize()))
        let paging = DoryX86PagingUnit()
        let initial = try DoryX86ArchitecturalState(
          registers: .init(rcx: 2, rdx: 0x80),
        ...
        try interpretedMemory.writeScalar(at: 0x80, value: UInt64.max - 1, byteCount: 8)
        try physical.writeScalar(at: 0x80, value: UInt64.max - 1, byteCount: 8)
        let translated = DoryX86TranslatedMemory(
          physicalMemory: physical,
          pagingUnit: paging,
          context: .init(state: state, mode: .long64)
        ...
        let first = try #require(executor.execute(
          bytes: bytes,
          at: instructionAddress,
          mode: .long64,
          addressSpaceID: UInt64(optimization == .baseline ? 20 : 21),
          maximumInstructions: 1,
          state: &state,
          memory: translated
        ))
        ...
        #expect(try physical.readScalar(at: 0x80, byteCount: 8) == 0)
        #expect(state.registers.rcx == UInt64.max - 1)
        #expect(state.rflags.contains(.carry))
        #expect(state.rflags.contains(.zero))
        #expect(paging.diagnostics.translationRequests == 1)

        expected.rip = instructionAddress
        expected.registers.rcx = 1
        state.rip = instructionAddress
        ...
        let second = try #require(executor.execute(
          bytes: bytes,
          at: instructionAddress,
          mode: .long64,
          addressSpaceID: UInt64(optimization == .baseline ? 20 : 21),
        ...
        #expect(second.block.tier.rawValue == optimization.rawValue)
        #expect(state == expected)
        #expect(try physical.readScalar(at: 0x80, byteCount: 8) == 1)
        #expect(state.registers.rcx == 0)
        #expect(paging.diagnostics.translationRequests == 1)
        #expect(executor.diagnostics.translationCacheHits == 1)
        #expect(executor.diagnostics.translationCacheMisses == 1)
  ...
  @Test func unalignedLockedExchangeAddUsesSerializedFallback() throws {
    #if arch(arm64)
      let bytes: [UInt8] = [0xF0, 0x0F, 0xC1, 0x0A]
      let instructionAddress: UInt64 = 0x3800
      for optimization in [DoryARM64JITOptimization.baseline, .optimizing] {
        let physical = try DoryX86MmapMemory(validatingByteCount: Int(getpagesize()))
        let interpretedMemory = try DoryX86ByteArrayMemory(byteCount: Int(getpagesize()))
        let paging = DoryX86PagingUnit()
        let initial = try DoryX86ArchitecturalState(
          registers: .init(rcx: 2, rdx: 0x81),
          rip: instructionAddress,
          rflags: [.reservedOne, .direction]
        )
        var expected = initial
        var state = initial
        try interpretedMemory.write(at: instructionAddress, bytes: bytes)
        try interpretedMemory.writeScalar(at: 0x81, value: 0xFFFF_FFFE, byteCount: 4)
        try physical.write(at: instructionAddress, bytes: bytes)
        try physical.writeScalar(at: 0x81, value: 0xFFFF_FFFE, byteCount: 4)
        let translated = DoryX86TranslatedMemory(
          physicalMemory: physical,
          pagingUnit: paging,
          context: .init(state: state, mode: .long64)
        ...
        let execution = try #require(executor.execute(
          ...
          at: instructionAddress,
          mode: .long64,
          addressSpaceID: UInt64(optimization == .baseline ? 22 : 23),
          maximumInstructions: 1,
          state: &state,
          memory: translated
        ))
        ...
        #expect(execution.exitCode == .interpreter)
        #expect(state == initial)
        #expect(try physical.readScalar(at: 0x81, byteCount: 4) == 0xFFFF_FFFE)
        #expect(paging.diagnostics.translationRequests == 1)

        _ = DoryX86Interpreter().step(state: &state, memory: physical, mode: .long64)
        #expect(state == expected)
        #expect(try physical.readScalar(at: 0x81, byteCount: 4) == 0)
        #expect(state.registers.rcx == 0xFFFF_FFFE)
        #expect(state.rflags.contains(.carry))
        #expect(state.rflags.contains(.zero))
      }
    #endif
  ...
  @Test func alignedLockedBinaryRMWMatchesInterpreterAcrossOperationsAndWidths() throws {
    #if arch(arm64)
      ...
      let cases: [OperationCase] = [
        .init(opcode: 0x01, destination: 0xFFFF_FFFE_FFFF_FFFE, source: 2),
        .init(opcode: 0x09, destination: 0x8000_0000_8000_0000, source: 0x10),
        .init(opcode: 0x21, destination: 0xFFFF_0000_FFFF_0000, source: 0x8000_0010),
        .init(opcode: 0x29, destination: 0, source: 1),
        .init(opcode: 0x31, destination: 0x8000_0000_8000_0000, source: 0x8000_0000),
      ]
      let instructionAddress: UInt64 = 0x3900
      for is64Bit in [false, true] {
        for optimization in [DoryARM64JITOptimization.baseline, .optimizing] {
          for (index, testCase) in cases.enumerated() {
            let bytes = [UInt8(0xF0)] + (is64Bit ? [0x48] : [])
              + [testCase.opcode, 0x0A]
            let byteCount = is64Bit ? 8 : 4
            let interpretedMemory = try DoryX86ByteArrayMemory(byteCount: Int(getpagesize()))
            let physical = try DoryX86MmapMemory(validatingByteCount: Int(getpagesize()))
            let paging = DoryX86PagingUnit()
            let initial = try DoryX86ArchitecturalState(
              registers: .init(rcx: testCase.source, rdx: 0x80),
              rip: instructionAddress,
              rflags: [.reservedOne, .carry, .direction, .overflow]
            )
            var expected = initial
            ...
            let execution = try #require(DoryARM64BaselineExecutor(
              ...
              at: instructionAddress,
              mode: .long64,
              addressSpaceID: UInt64(100 + index + (is64Bit ? 10 : 0)),
              maximumInstructions: 1,
              state: &state,
              memory: translated
            ))
  ...
  @Test func lockedImmediateRMWStaysNativeWhileUnalignedRMWDeclinesPrecisely() throws {
    #if arch(arm64)
      let instructionAddress: UInt64 = 0x3A00
      for optimization in [DoryARM64JITOptimization.baseline, .optimizing] {
        let immediateBytes: [UInt8] = [0xF0, 0x48, 0x83, 0x02, 0xFF]
        let interpretedMemory = try DoryX86ByteArrayMemory(byteCount: Int(getpagesize()))
        let physical = try DoryX86MmapMemory(validatingByteCount: Int(getpagesize()))
        let paging = DoryX86PagingUnit()
        let initial = try DoryX86ArchitecturalState(
        ...
        var expected = initial
        var state = initial
        try interpretedMemory.write(at: instructionAddress, bytes: immediateBytes)
        try interpretedMemory.writeScalar(at: 0x80, value: 0, byteCount: 8)
        try physical.writeScalar(at: 0x80, value: 0, byteCount: 8)
        _ = DoryX86Interpreter().step(
          state: &expected,
          memory: interpretedMemory,
          mode: .long64
        ...
        let immediateExecution = try #require(executor.execute(
          bytes: immediateBytes,
          at: instructionAddress,
          mode: .long64,
          addressSpaceID: UInt64(optimization == .baseline ? 130 : 131),
          maximumInstructions: 1,
          state: &state,
          memory: translated
        ))
        #expect(immediateExecution.block.tier.rawValue == optimization.rawValue)
        #expect(state == expected)
        #expect(try physical.readScalar(at: 0x80, byteCount: 8) == UInt64.max)
        #expect(paging.diagnostics.translationRequests == 1)

        let unalignedBytes: [UInt8] = [0xF0, 0x48, 0x31, 0x0A]
        let unalignedInitial = try DoryX86ArchitecturalState(
          registers: .init(rcx: 0xFF00_FF00_FF00_FF00, rdx: 0x81),
          rip: instructionAddress,
          rflags: [.reservedOne, .carry, .direction, .overflow]
        )
        var unalignedState = unalignedInitial
        try physical.writeScalar(
          at: 0x81,
          value: 0x00FF_00FF_00FF_00FF,
          byteCount: 8
        )
        translated.updateContext(.init(state: unalignedState, mode: .long64))
        let unalignedExecution = try #require(executor.execute(
          bytes: unalignedBytes,
          at: instructionAddress,
          mode: .long64,
          addressSpaceID: UInt64(optimization == .baseline ? 132 : 133),
          maximumInstructions: 1,
          state: &unalignedState,
          memory: translated
        ))
        #expect(unalignedExecution.block.tier.rawValue == optimization.rawValue)
        #expect(unalignedExecution.exitCode == .interpreter)
        #expect(unalignedState == unalignedInitial)
        #expect(
