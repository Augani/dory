...
@Suite struct DoryARM64Tier1JITTests {
  ...
  @Test func measuredRAXByRDIMemoryBitTestFeedsCarryBranchAndMatchesInterpreter() throws {
    let codeAddress: UInt64 = 0xFFFF_FFFF_8168_1086
    let bytes: [UInt8] = [
      0x48, 0x0F, 0xA3, 0x07,  // btq %rax,(%rdi)
      0x72, 0x4C,  // jb +76
    ]
    let block = try DoryX86IRTranslator().translate(bytes, at: codeAddress, mode: .long64)
    #expect(block.guestByteCount == 6)
    ...
    #expect(
      block.terminator
        == .conditional(
          condition: "x86.condition.2",
          taken: codeAddress + 82,
          notTaken: codeAddress + 6
        ))
    let compiled = try #require(DoryARM64Tier1Emitter().compile(block))
    #expect(compiled.tier == .tier1)
    #expect(compiled.requiresMemoryCallbacks)
    ...
    #expect(DoryARM64Tier1Emitter().compile(sameOperationElsewhere) == nil)
      ...
      for signedIndex: Int64 in [0, 64, -1] {
        ...
        for selected in [false, true] {
          ...
          let initial = try DoryX86ArchitecturalState(
            registers: .init(
              rax: UInt64(bitPattern: signedIndex),
              rdi: dataAddress
            ),
            rip: codeAddress,
            rflags: [.reservedOne, .carry, .direction]
          ...
          for _ in 0..<2 {
            guard case .retired = DoryX86Interpreter().step(
              ...
              Issue.record("interpreter did not retire measured RAX/RDI memory BT fixture")
              return
            }
          }
          var tier1 = initial
          ...
          #expect(tier1Memory.snapshot() == interpretedMemory.snapshot())
          #expect(tier1.rflags.contains(.carry) == selected)
          #expect(tier1.rip == (selected ? codeAddress + 82 : codeAddress + 6))
        }
      }
      ...
      let failedInitial = try DoryX86ArchitecturalState(
        registers: .init(rax: 0, rdi: codeAddress + 0x1000),
        rip: codeAddress,
        rflags: [.reservedOne, .direction]
      )
  ...
  @Test func measuredRDIByRSIMemoryBitTestFeedsNearNotCarryBranch() throws {
    let codeAddress: UInt64 = 0xFFFF_FFFF_81E1_C65C
    let bytes: [UInt8] = [
      0x48, 0x0F, 0xA3, 0xBE, 0x28, 0x02, 0x00, 0x00,  // btq %rdi,0x228(%rsi)
      0x0F, 0x83, 0xAB, 0x00, 0x00, 0x00,  // jae +171
    ]
    let block = try DoryX86IRTranslator().translate(bytes, at: codeAddress, mode: .long64)
    #expect(block.guestByteCount == 14)
    #expect(block.guestInstructionCount == 2)
    #expect(block.statements.count == 1)
    #expect(
      block.terminator
        == .conditional(
          ...
          taken: codeAddress + 185,
          notTaken: codeAddress + 14
        ))
    let compiled = try #require(DoryARM64Tier1Emitter().compile(block))
    #expect(compiled.tier == .tier1)
    #expect(compiled.requiresMemoryCallbacks)
    ...
    #expect(DoryARM64Tier1Emitter().compile(sameOperationElsewhere) == nil)
      ...
      for signedIndex: Int64 in [0, 63, 64, -1, -65] {
        ...
        for selected in [false, true] {
          ...
          let initial = try DoryX86ArchitecturalState(
            registers: .init(
              rsi: dataAddress - 0x228,
              rdi: UInt64(bitPattern: signedIndex)
            ),
            rip: codeAddress,
            rflags: [.reservedOne, .carry, .direction]
          )
          ...
          for _ in 0..<2 {
            guard case .retired = DoryX86Interpreter().step(
              ...
              Issue.record("interpreter did not retire measured RDI/RSI memory BT fixture")
              return
            }
          }
          var tier1 = initial
          ...
          #expect(tier1Memory.snapshot() == interpretedMemory.snapshot())
          #expect(tier1.rflags.contains(.carry) == selected)
          #expect(tier1.rip == (selected ? codeAddress + 14 : codeAddress + 185))
        }
      }
      ...
      let failedInitial = try DoryX86ArchitecturalState(
        registers: .init(rsi: codeAddress + 0x1000, rdi: 0),
        rip: codeAddress,
        rflags: [.reservedOne, .direction]
      )
      var failed = failedInitial
  ...
  @Test func measuredRBXByRAXMemoryBitTestFeedsBackwardNotCarryBranch() throws {
    let codeAddress: UInt64 = 0xFFFF_FFFF_81E1_C104
    let bytes: [UInt8] = [
      0x48, 0x0F, 0xA3, 0x98, 0x28, 0x02, 0x00, 0x00,  // btq %rbx,0x228(%rax)
      0x73, 0xDB,  // jae -37
    ]
    let block = try DoryX86IRTranslator().translate(bytes, at: codeAddress, mode: .long64)
    #expect(block.guestByteCount == 10)
    ...
    #expect(DoryARM64Tier1Emitter().compile(adjacent) == nil)
    var changedBranchBytes = bytes
    changedBranchBytes[8] = 0x72
    let changedBranch = try DoryX86IRTranslator().translate(
      changedBranchBytes,
      at: codeAddress,
      mode: .long64
    )
    #expect(DoryARM64Tier1Emitter().compile(changedBranch) == nil)
      ...
      for signedIndex: Int64 in [0, 64, -1] {
        ...
        for selected in [false, true] {
          ...
          let initial = try DoryX86ArchitecturalState(
            registers: .init(
              rax: dataAddress - 0x228,
              rbx: UInt64(bitPattern: signedIndex),
          ...
          for _ in 0..<2 {
            guard case .retired = DoryX86Interpreter().step(
              ...
              Issue.record("interpreter did not retire measured RBX/RAX memory BT fixture")
              return
            }
          }
          var tier1 = initial
          ...
          #expect(tier1Memory.snapshot() == interpretedMemory.snapshot())
          #expect(tier1.rflags.contains(.carry) == selected)
          #expect(tier1.rip == (selected ? codeAddress + 10 : codeAddress - 27))
        }
      }
      ...
      let failedInitial = try DoryX86ArchitecturalState(
        registers: .init(rax: codeAddress + 0x1000, rbx: 0),
        rip: codeAddress,
        rflags: [.reservedOne, .direction]
      )
      var failed = failedInitial
  ...
  @Test func measuredRIPRelativeMemoryBitTestFeedsNotCarryBranch() throws {
    let codeAddress: UInt64 = 0xFFFF_FFFF_813A_96AB
    let dataAddress: UInt64 = 0xFFFF_FFFF_82BF_3DA0
    let bytes: [UInt8] = [
      0x48, 0x0F, 0xA3, 0x05, 0xED, 0xA6, 0x84, 0x01,  // btq %rax,0x184a6ed(%rip)
      0x73, 0x1E,  // jae +30
    ]
    let block = try DoryX86IRTranslator().translate(bytes, at: codeAddress, mode: .long64)
    #expect(block.guestByteCount == 10)
    ...
    #expect(DoryARM64Tier1Emitter().compile(adjacent) == nil)
    var changedConditionBytes = bytes
    changedConditionBytes[8] = 0x72  // jb +30
    let changedCondition = try DoryX86IRTranslator().translate(
      changedConditionBytes,
      at: codeAddress,
      mode: .long64
    )
    #expect(DoryARM64Tier1Emitter().compile(changedCondition) == nil)
    var changedDisplacementBytes = bytes
    changedDisplacementBytes[4] ^= 0x01
    ...
    #if arch(arm64)
      let memoryBase = codeAddress - 0x100
      let memoryByteCount = Int(dataAddress - memoryBase) + 0x100
      for signedIndex: Int64 in [0, 64, -1] {
        let elementOffset = signedIndex >> 6
        let bitOffset = UInt64(bitPattern: signedIndex) & 63
        let selectedAddress = UInt64(
        ...
        for selected in [false, true] {
          ...
          let qword = selected ? pattern | bit : pattern & ~bit
          let qwordBytes = (0..<8).map { UInt8(truncatingIfNeeded: qword >> ($0 * 8)) }
          for memory in [interpretedMemory, tier1Memory] {
            try memory.write(at: codeAddress, bytes: bytes)
            try memory.write(at: selectedAddress, bytes: qwordBytes)
          ...
          let initial = try DoryX86ArchitecturalState(
            registers: .init(rax: UInt64(bitPattern: signedIndex)),
            rip: codeAddress,
            rflags: [.reservedOne, .carry, .direction]
          )
          var interpreted = initial
          for _ in 0..<2 {
            guard case .retired = DoryX86Interpreter().step(
              ...
              Issue.record("interpreter did not retire RIP-relative memory BT fixture")
              return
            }
          }
          var tier1 = initial
          ...
          #expect(tier1Memory.snapshot() == interpretedMemory.snapshot())
          #expect(tier1.rflags.contains(.carry) == selected)
          #expect(tier1.rip == (selected ? codeAddress + 10 : codeAddress + 40))
        }
      }
  ...
  @Test func measuredPatchedMemoryBitResetIsExactAndMatchesInterpreter() throws {
    let codeAddress: UInt64 = 0xFFFF_FFFF_812D_C46A
    let bytes: [UInt8] = [
      0x3E,  // DS prefix replaces LOCK through the single-vCPU alternatives patch
      0x48, 0x0F, 0xB3, 0x90, 0xC0, 0x05, 0x00, 0x00,  // btrq %rdx,0x5c0(%rax)
    ]
    let block = try DoryX86IRTranslator().translate(bytes, at: codeAddress, mode: .long64)
    ...
    #expect(DoryARM64Tier1Emitter().compile(mutated) == nil)
      ...
      for signedIndex: Int64 in [0, 64, -1] {
        ...
        for selected in [false, true] {
          ...
          guard case .retired = DoryX86Interpreter().step(
            ...
            Issue.record("interpreter did not retire patched memory BTR fixture")
            return
          }
          var tier1 = initial
          let execution = try #require(
      ...
      try rejectedWriteMemory.backing.write(at: codeAddress, bytes: bytes)
      let rejectedAddress = dataAddress
      try rejectedWriteMemory.backing.write(
        at: rejectedAddress,
        bytes: [0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF, 0xFF]
      )
      let rejectedInitial = try DoryX86ArchitecturalState(
        registers: .init(rax: dataAddress - 0x5C0, rdx: 7),
        rip: codeAddress,
        rflags: [.reservedOne, .direction]
      )
  ...
  @Test func measuredR14MemoryBitResetIsExactAndMatchesInterpreter() throws {
    let codeAddress: UInt64 = 0xFFFF_FFFF_8168_1078
    let bytes: [UInt8] = [0x4C, 0x0F, 0xB3, 0x30]  // btrq %r14,(%rax)
    let block = try DoryX86IRTranslator().translate(bytes, at: codeAddress, mode: .long64)
    let compiled = try #require(DoryARM64Tier1Emitter().compile(block))
    #expect(compiled.tier == .tier1)
    #expect(compiled.guestByteCount == 4)
    #expect(compiled.guestInstructionCount == 1)
    #expect(compiled.requiresMemoryCallbacks)
    #expect(compiled.requiresRestartableMemoryReads)
    #expect(compiled.mayExitToInterpreter)
      ...
      for signedIndex: Int64 in [0, 64, -1] {
        ...
        for selected in [false, true] {
          ...
          let initial = try DoryX86ArchitecturalState(
            registers: .init(
              rax: dataAddress,
              r14: UInt64(bitPattern: signedIndex)
            ),
            rip: codeAddress,
            rflags: selected
              ? [.reservedOne, .direction]
          ...
          guard case .retired = DoryX86Interpreter().step(
            ...
            Issue.record("interpreter did not retire measured R14 memory BTR fixture")
            return
          }
          var tier1 = initial
          let execution = try #require(
  ...
  @Test func measuredAlternativesPatchedMemoryBitSetMatchesInterpreterAndRollsBack() throws {
    let codeAddress: UInt64 = 0xFFFF_FFFF_8133_CB0F
    let bytes: [UInt8] = [
      0x48, 0x63, 0xC1,  // movslq %ecx,%rax
      0x3E,  // DS prefix replaces LOCK through the single-vCPU alternatives patch
      0x48, 0x0F, 0xAB, 0x06,  // btsq %rax,(%rsi)
    ]
    let block = try DoryX86IRTranslator().translate(bytes, at: codeAddress, mode: .long64)
    #expect(block.guestByteCount == 8)
    #expect(block.guestInstructionCount == 2)
    ...
    #expect(DoryARM64Tier1Emitter().compile(adjacent) == nil)
    var changedBaseBytes = bytes
    changedBaseBytes[7] = 0x07  // btsq %rax,(%rdi)
    let changedBase = try DoryX86IRTranslator().translate(
      changedBaseBytes,
      at: codeAddress,
      mode: .long64
    )
    #expect(DoryARM64Tier1Emitter().compile(changedBase) == nil)

    #if arch(arm64)
      let memoryBase = codeAddress - 0x200
      let dataAddress = codeAddress + 0x1800
      for signedIndex: Int32 in [0, 64, -1] {
        let elementOffset = Int64(signedIndex) >> 6
        let bitOffset = UInt64(UInt32(bitPattern: signedIndex)) & 63
        let selectedAddress = UInt64(
          bitPattern: Int64(bitPattern: dataAddress) &+ elementOffset &* 8
        ...
        for selected in [false, true] {
          ...
          for _ in 0..<2 {
            guard case .retired = DoryX86Interpreter().step(
              ...
              Issue.record("interpreter did not retire patched memory BTS fixture")
              return
            }
          }
          var tier1 = initial
  ...
  @Test func measuredStandaloneAtomicMemoryBitSetMatchesInterpreter() throws {
    let codeAddress: UInt64 = 0xFFFF_FFFF_8133_CB12
    let bytes: [UInt8] = [0xF0, 0x48, 0x0F, 0xAB, 0x06]  // lock btsq %rax,(%rsi)
    let block = try DoryX86IRTranslator().translate(bytes, at: codeAddress, mode: .long64)
      ...
      for signedIndex: Int64 in [0, 64, -1] {
        ...
        for selected in [false, true] {
          ...
          guard case .retired = DoryX86Interpreter().step(
            ...
            Issue.record("interpreter did not retire standalone atomic memory BTS fixture")
            return
          }
          var tier1 = initial
          let execution = try #require(
      ...
      #expect(DoryARM64Tier1Emitter().compile(wrongBase) == nil)
        ...
        for zeroIsSet in [false, true] {
          ...
          guard case .retired = DoryX86Interpreter().step(
            ...
            Issue.record("interpreter did not retire additional measured memory SETNE")
            return
          }
  ...
  @Test func measuredFreeFrozenPageByteShiftIsExactAndMatchesInterpreter() throws {
    let codeAddress: UInt64 = 0xFFFF_FFFF_815C_C28B
    let bytes: [UInt8] = [0xD0, 0x6E, 0x19]  // shrb 0x19(%rsi)
    let block = try DoryX86IRTranslator().translate(bytes, at: codeAddress, mode: .long64)
    let compiled = try #require(DoryARM64Tier1Emitter().compile(block))
    #expect(compiled.tier == .tier1)
    #expect(compiled.guestByteCount == 3)
    ...
    #expect(DoryARM64Tier1Emitter().compile(adjacent) == nil)
    var changedDisplacementBytes = bytes
    changedDisplacementBytes[2] = 0x1A
    let changedDisplacement = try DoryX86IRTranslator().translate(
      changedDisplacementBytes,
      at: codeAddress,
      mode: .long64
    ...
    #expect(DoryARM64Tier1Emitter().compile(changedDisplacement) == nil)
      ...
      for (index, value) in [UInt8(0x00), 0x01, 0x80, 0xFF].enumerated() {
        let interpretedMemory = try DoryX86ByteArrayMemory(
          baseAddress: codeAddress,
          byteCount: 0x1000
        )
        ...
        for memory in [interpretedMemory, tier1Memory] {
          try memory.write(at: codeAddress, bytes: bytes)
          try memory.write(at: dataAddress, bytes: [value])
        }
        let initial = try DoryX86ArchitecturalState(
          registers: .init(
            rax: 0x0123_4567_89AB_CDEF,
            rsi: dataAddress - 0x19,
            r15: 0xFEDC_BA98_7654_3210
          ),
          rip: codeAddress,
          rflags: [
        ...
        guard case .retired = DoryX86Interpreter().step(
          ...
          Issue.record("interpreter did not retire measured byte SHR")
          return
        }
        ...
        #expect(tier1 == interpreted)
        #expect(tier1Memory.snapshot() == interpretedMemory.snapshot())
        #expect(try tier1Memory.read(at: dataAddress, byteCount: 1) == [value >> 1])
      }
      ...
      try rejectedWriteMemory.backing.write(at: codeAddress, bytes: bytes)
      try rejectedWriteMemory.backing.write(at: dataAddress, bytes: [0xFF])
      let rejectedInitial = try DoryX86ArchitecturalState(
        registers: .init(
          rax: 0x0123_4567_89AB_CDEF,
          rsi: dataAddress - 0x19,
