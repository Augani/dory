...
public final class DoryARM64BaselineExecutor: @unchecked Sendable {
  ...
  /// A source snapshot confirmed immediately before its generated candidate becomes visible.
  /// `nil` generation remains valid: providers that cannot supply a generation token rely on
  /// the mandatory exact-byte confirmation instead.
  private struct ResidentSourceValidation {
    let memoryCodeGeneration: UInt64?
  }
  ...
  private func compileResident(
    ...
    let guestByteCount = Int(compiled.guestByteCount)
    let guestBytes = Array(bytes.prefix(guestByteCount))
    guard guestBytes.count == guestByteCount else {
      return .init(resident: nil, emitterDeclineByteCount: nil, declineReason: nil)
    }
    ...
    try protectValidatedGuestCode(
      ...
      memoryCodeGeneration: sourceValidation.memoryCodeGeneration,
      memory: memory
    )
    let resident = ResidentBlock(
      key: key,
  ...
  /// Translation starts from a speculative fetch. Once decoding has established the exact guest
  /// byte count, confirm that those exact bytes still match and that any available generation
  /// remained stable across that read. The caller performs this only after cache capacity
  /// handling and immediately before publication, because neither operation is synchronized
  /// with guest/device writers.
  private func validateResidentCompilationSource(
    _ originalBytes: [UInt8],
    byteProvider: (_ maximumCount: Int) throws -> [UInt8],
    codeGenerationProvider: ((_ byteCount: Int) throws -> UInt64?)?
  ) throws -> ResidentSourceValidation? {
    let byteCount = originalBytes.count
    let generationBefore = readCodeGeneration(
      using: codeGenerationProvider,
      byteCount: byteCount
    )
    let confirmedBytes = try speculativeInstructionBytes(
      using: byteProvider,
      maximumCount: byteCount
    )
    guard confirmedBytes.count == byteCount,
      confirmedBytes.elementsEqual(originalBytes)
    else { return nil }
    let generationAfter = readCodeGeneration(
      using: codeGenerationProvider,
      byteCount: byteCount
    )
    guard generationAfter == generationBefore else { return nil }
    return .init(memoryCodeGeneration: generationAfter)
  }
