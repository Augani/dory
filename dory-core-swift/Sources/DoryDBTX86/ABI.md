...
convention internally. Direct chains target the same complete block entries after
the source publishes architectural state and restores the target's entry tuple.
...
tail-branches to the target's complete entry; pinned values never cross compiler
...
Legacy memory blocks keep the architectural context and callback bank in
`x19`...`x24`. Their chain epilogue restores the canonical `x0`...`x5` tuple,
which permits a one-way legacy-memory-to-tier-1 edge. Callback-free legacy blocks
do not establish or preserve that tuple and remain compiler-separated. The
reverse tier-1-to-legacy direction also remains separated because legacy code
cannot consume tier-1's private lazy-flags descriptor.
