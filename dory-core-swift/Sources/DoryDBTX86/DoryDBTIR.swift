...
public enum DoryIRStatement: Codable, Sendable, Hashable {
  ...
  case setCarryFlag(enabled: Bool)
  ...
  case signedAccumulatorDivide(source: DoryIROperand)
...
public struct DoryX86IRTranslator: Sendable {
  ...
  private func lower(
    ...
    case .bitTest(let operation, let base, let index) where mode == .long64:
        ...
        memory.width == .doubleword || memory.width == .quadword
      {
        ...
        if case .register(_, let indexWidth) = index, indexWidth == memory.width {
          return ([.bitTestMemoryRegister(
            operation: operation,
            base: operand(base, instructionRelativeBase: instruction.nextInstructionAddress),
            index: operand(index)
          )], nil)
    ...
    case .setCarry(let enabled):
      return ([.setCarryFlag(enabled: enabled)], nil)
    case .complementCarry:
      return ([.complementCarryFlag], nil)
    ...
    case .accumulatorArithmetic(.signedDivide, let source) where mode == .long64:
      guard case .register(_, let width) = source, width == .doubleword || width == .quadword
      else { return fallback(instruction, reason: .interpreter) }
      return (
        [.signedAccumulatorDivide(
          source: operand(source, instructionRelativeBase: instruction.nextInstructionAddress)
        )],
        .next(instruction.nextInstructionAddress)
      )
    ...
    case .exchangeAdd(let destination, let source) where mode == .long64:
      let destinationOperand = operand(
        destination,
        instructionRelativeBase: instruction.nextInstructionAddress
      )
      ...
      guard case .memory(_, let memoryWidth) = destinationOperand,
        memoryWidth == .i32 || memoryWidth == .i64,
        case .register(let sourceRegister) = sourceOperand,
        sourceRegister.bank == "x86.gpr", sourceRegister.index < 16,
        sourceRegister.width == memoryWidth
      else { return fallback(instruction, reason: .interpreter) }
      return (
        [.exchangeAddMemory(destination: destinationOperand, source: sourceRegister)],
        .next(instruction.nextInstructionAddress)
      )
    ...
    case .readControlRegister(let index, let destination) where mode == .long64 && index == 3:
      return (
        [
          .readControlRegister(
            index: index,
            destination: irRegister(destination, width: .i64)
          )
        ],
        nil
      )
  ...
  private func isBaselineJITSupported(_ statement: DoryIRStatement) -> Bool {
    ...
    case .copy(let destination, let source):
      switch destination {
      case .register(let target) where isJITNarrowRegister(target):
        switch source {
        case .register(let register):
          return isJITNarrowRegister(register) && register.width == target.width
        case .immediate(_, let width):
    ...
    case .unary(let operation, let operand):
      ...
      case .register(let register):
        return isJITGeneralRegister(register)
          || (operation == .bitwiseNot && isJITNarrowRegister(register))
      case .memory(let address, let width):
    ...
    case .bitTestMemoryRegister(_, let base, let index):
      guard case .memory(let address, let width) = base,
        (width == .i32 || width == .i64), isJITMemoryAddress(address),
        case .register(let register) = index
      else { return false }
      return register.width == width && isJITGeneralRegister(register)
  ...
  private func isJITNarrowRegister(_ register: DoryIRRegister) -> Bool {
    register.bank == "x86.gpr" && register.index < 16
      && (register.width == .i8 || register.width == .i16)
  }
