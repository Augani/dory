...
struct DoryARM64Tier1ALUEmitter: Sendable {
  ...
  func emitMeasuredPatchedByteXOR(
    address: DoryIRMemoryAddress,
    into words: inout [UInt32]
  ) -> Bool {
    var fragment: [UInt32] = []
    ...
    fragment.append(
      Self.encodeLogical(
        .xor, is64Bit: true, left: 17, right: 16, destination: 17))
    fragment.append(Self.encodeStore64(register: 17, word: .lazyFlagsSource2))
  ...
  /// Emits the qword accumulator MUL form, publishing the complete product through RDX:RAX.
  ...
  private static func emitUnsignedAccumulatorMultiplyCore(
    ...
    fragment.append(
      Self.encodeLogical(.and, is64Bit: true, left: 25, right: 16, destination: 25))
    fragment.append(
      Self.encodeLogical(.or, is64Bit: true, left: 25, right: 26, destination: 25))
    fragment.append(
      Self.encodeLogical(
  ...
  private static func encodeUnsignedMultiplyHigh64(
    left: UInt32,
    right: UInt32,
    destination: UInt32
  ) -> UInt32 {
