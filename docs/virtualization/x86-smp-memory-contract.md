...
As of implementation checkpoint `376846fd9` on 2026-09-20:
...
| Mixed interpreter/JIT locked operations | Aligned 1/2/4/8-byte interpreter families use host compare/exchange transactions shared with native JIT helpers. Unaligned and split-backing locked fallbacks plus interpreter CMPXCHG16B take one exclusive multi-range lease after complete preflight, excluding overlapping ordinary/native access. | Extend contention and precise-fault campaigns across translated/PC routing, protected code, every tier pair, and 1/2/4 vCPUs. |
| Ordinary scalar RAM | Every direct Arm load and store uses the backing-address authority and is followed by conservative `DMB ISH`; checked byte-array/mmap/translated/PC RAM paths enter ordinary leases, and aligned 1/2/4/8-byte Swift loads/stores use host atomics. | Prove the complete load/load, load/store, store/store, and store/load TSO mapping for every emitter, callback, bulk/string, DMA, shared-mapping, and replay path. Remove conservatism only with litmus and inspection authority. |
| TLB invalidation | A machine-scoped coordinator publishes targeted or global invalidations to every paging unit and baseline/optimizing native TLB, records a required generation per vCPU, and couples the run session to per-vCPU pending generations. The single-vCPU owner acknowledges invalidations on its worker, including maintenance while parked on a result. Page-table writes coalesce a global flush, and generation wrap performs a global reset. | Activate the owner protocol for every simultaneous worker, then prove it under concurrent page-table rewrite, interpreted/native/halted/quiescing/fault/stop states, and every tier pair. Single-owner evidence is not free-running SMP proof. |
...
| Device/shared state | Every vCPU physical-memory bus and the port-I/O bus share one machine-scoped guest-entry domain. MMIO, ECAM, PCI BAR, and port callbacks are serialized across vCPUs; ordinary RAM and DMA synchronization remain outside it. Built-in models retain local locks and sealed route tables. The inventory and lock/callback rules are in `x86-device-shared-state-audit.md`. | Complete asynchronous clock/input/reset/completion/interrupt callback review, audit every public MMIO/PCI extension and future shared mapping, and pass the race/lifecycle matrix under sustained SMP. |
| Scheduling | `DoryPCVCPURuntime` owns one persistent host thread per vCPU for the machine lifetime; native blocks poll per-vCPU pending-work bytes, and all parallel error paths rendezvous every submitted worker before releasing machine ownership. The one-vCPU long-running worker now owns processor events, interrupts, and invalidations. | Requests above one vCPU still use coordinator-submitted bounded slices. Extend the owner loop and exact session protocol to all simultaneous vCPUs before sustained shared-memory SMP is admitted. |
| Qualification | Instruction inspection covers the direct barriers; focused mixed-tier atomic, remote-invalidation, DMA-range, and code-cache hazard tests pass. At the current checkpoint the complete PC target passes 401 tests in 53 suites and the device-heavy TSan matrix passes 99 tests in 9 suites. Two signed PVH workload/poweroff observations are retained for older exact commit `298d6668e`, not relabeled for current head. | Run the complete tier-pair TSO/atomic/TLB/DMA/SMC matrix on free-running workers at 1, 2, and 4 vCPUs and retain exact-candidate boot/lifecycle receipts. |
