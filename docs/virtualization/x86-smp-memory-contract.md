...
As of implementation checkpoint `e7c6f1eec` on 2026-09-20:
...
| TLB invalidation | A machine-scoped coordinator publishes targeted or global invalidations to every paging unit and baseline/optimizing native TLB, records a required generation per vCPU, and couples the run session to per-vCPU pending generations. Every owner can acknowledge invalidations on its worker, including maintenance while parked on a result. The internal interpreter pair acknowledges an externally published page-table flush on both owners before async poweroff joins. Page-table writes coalesce a global flush, and generation wrap performs a global reset. | Prove guest-driven page-table rewrite plus local/remote targeted and global invalidation during active interpreted/native TLB hits and halted/quiescing/fault/stop states for every tier pair. The initial external-write pair test is not full free-running TLB proof. |
