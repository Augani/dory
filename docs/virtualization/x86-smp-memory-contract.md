...
| TLB invalidation | A machine-scoped coordinator publishes targeted or global invalidations to every paging unit and baseline/optimizing native TLB, records a required generation per vCPU, and requires acknowledgement before the next coordinated dispatch can retire guest work. Page-table writes coalesce a global flush, and generation wrap performs a global reset. | Deliver and acknowledge invalidations while vCPU dispatch loops remain free-running, then prove the protocol under concurrent page-table rewrite and every tier pair. Coordinator-return acknowledgement is not free-running SMP proof. |
...
| Qualification | Instruction inspection covers the direct barriers; focused mixed-tier atomic, remote-invalidation, DMA-range, and code-cache hazard tests pass. The optimized x86/PC/firmware/runner graph passes 2,058 tests, and the clean implementation candidate has two signed single-vCPU PVH workload/poweroff observations. | Run the complete tier-pair TSO/atomic/TLB/DMA/SMC matrix on free-running workers at 1, 2, and 4 vCPUs and retain exact-candidate receipts. |
