# Dory container-engine performance qualification

Container-engine performance is a release-evidence contract, not a collection of local
screenshots. Dory 0.4 does
...
`Dory-<version>-container-engine-performance-evidence.zip` to the matching GitHub Release. If that
asset is absent, all comparative container-engine performance claims remain unqualified. This
contract does not authorize full Linux VM support or whole-VM performance claims.
...
- `scripts/qualify-container-engine-performance.sh` — exact-candidate container-engine
  orchestration, verification, redaction, digest manifest, and the release-attachable evidence
  ZIP.
...
`Dory-<version>-container-engine-performance-evidence.zip` must contain:
