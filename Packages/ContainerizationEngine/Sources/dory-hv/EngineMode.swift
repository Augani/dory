    ...
    private static func guestGPUReadinessCommand(gpuMode: GPUAccelerationMode) -> String {
        ...
        case .venus:
            return [
                ...
                "env -u LD_LIBRARY_PATH -u VK_ADD_DRIVER_FILES -u MESA_VK_DEVICE_SELECT -u MESA_LOADER_DRIVER_OVERRIDE VK_DRIVER_FILES=\"$DORY_VENUS_ICD\" timeout 10 \"$DORY_VENUS_PROBE\" >/var/log/dory-gpu-venus-probe.log 2>&1 || { echo DORY-GPU-VENUS-PROBE-FAILED; cat /var/log/dory-gpu-venus-probe.log 2>/dev/null; cp /var/log/dory-gpu-venus-probe.log /mnt/dory-logs/gpu-venus-probe.log 2>/dev/null || true; sync; poweroff -f; exit 1; }",
                "env -u LD_LIBRARY_PATH -u VK_ADD_DRIVER_FILES -u MESA_VK_DEVICE_SELECT -u MESA_LOADER_DRIVER_OVERRIDE VK_DRIVER_FILES=\"$DORY_VENUS_ICD\" vulkaninfo --summary >/var/log/dory-gpu-vulkaninfo.log 2>&1 || { echo DORY-GPU-VULKANINFO-FAILED; cat /var/log/dory-gpu-vulkaninfo.log 2>/dev/null; cp /var/log/dory-gpu-vulkaninfo.log /mnt/dory-logs/gpu-vulkaninfo.log 2>/dev/null || true; sync; poweroff -f; exit 1; }",
