...
final class HostFS: @unchecked Sendable {
    ...
    private struct Node: Sendable {
        ...
        /// Admission for this non-root identity descriptor. Root is structural and has no token.
        var resourceToken: FuseResourceToken? = nil
    ...
    let resourceQuota: FuseResourceQuota
    ...
    /// Entry names hidden only as direct children of the export root. Safe whole-home shares use
    /// this set so home credentials remain private without shadowing normal project directories.
    private let rootHiddenNameKeys: Set<String>
    ...
    public convenience init(
        ...
        resourceLimits: FuseResourceLimits = .production
    ) throws {
    ...
    public convenience init(
        ...
        guard rootDirectoryFileDescriptor >= 0,
              eventRootPath.hasPrefix("/"),
              !eventRootPath.utf8.contains(0) else {
            throw HostFSError.invalidRoot(eventRootPath)
        }
        let fd = fcntl(rootDirectoryFileDescriptor, F_DUPFD_CLOEXEC, 0)
        guard fd >= 0 else {
            throw HostFSError.systemCall("duplicate authorized root", errno)
        }
        let standardizedRoot = URL(fileURLWithPath: eventRootPath).standardizedFileURL.path
        try self.init(
            ownedRootFD: fd,
    ...
    /// Constructs the production share from pinned descriptor authority only. The diagnostic
    /// spelling used by the worker-local event machinery is derived from that descriptor with
    /// `F_GETPATH`; no caller can pair an authorized descriptor with an unrelated host path.
    public convenience init(
        rootDirectoryFileDescriptor: Int32,
        guestUID: UInt32 = getuid(),
        guestGID: UInt32 = getgid(),
        ...
        guard rootDirectoryFileDescriptor >= 0 else {
            throw HostFSError.invalidRoot("authorized root descriptor")
        }
        var path = [CChar](repeating: 0, count: Int(PATH_MAX))
        guard fcntl(rootDirectoryFileDescriptor, F_GETPATH, &path) == 0 else {
            throw HostFSError.systemCall("inspect authorized root", errno)
        }
        let pathBytes = path.prefix { $0 != 0 }.map { UInt8(bitPattern: $0) }
        let eventRootPath = String(decoding: pathBytes, as: UTF8.self)
        guard eventRootPath.hasPrefix("/"), eventRootPath != "/" else {
            throw HostFSError.invalidRoot("authorized root descriptor")
        }
        try self.init(
            rootDirectoryFileDescriptor: rootDirectoryFileDescriptor,
    ...
    /// Designated initializer taking ownership of `ownedRootFD`, including every failure path.
    private init(
        ownedRootFD: Int32,
        rootPath: String,
        hostEventRootPaths: [String],
        invalidRootDescription: String,
        guestUID: UInt32,
        guestGID: UInt32,
        readOnly: Bool,
        hiddenNames: Set<String>,
        rootHiddenNames: Set<String>,
        resourceLimits: FuseResourceLimits
    ) throws {
        self.rootPath = rootPath
        self.hostEventRootPaths = hostEventRootPaths
        self.rootFD = ownedRootFD
        self.resourceQuota = FuseResourceQuota(limits: resourceLimits)
        self.guestUID = guestUID
        self.guestGID = guestGID
        self.readOnly = readOnly
        self.hiddenNameKeys = Set(hiddenNames.map(Self.hiddenNameKey))
        self.rootHiddenNameKeys = Set(rootHiddenNames.map(Self.hiddenNameKey))

        var st = stat()
        guard fstat(ownedRootFD, &st) == 0,
              st.st_mode & S_IFMT == S_IFDIR else {
            Darwin.close(ownedRootFD)
            throw HostFSError.invalidRoot(invalidRootDescription)
        }
        let identityFD = fcntl(ownedRootFD, F_DUPFD_CLOEXEC, 0)
    ...
    private func eventRelativePath(forHostPath hostPath: String) -> String? {
        ...
        guard !components.contains(where: isHiddenAtAnyDepth),
              components.first.map({ !rootHiddenNameKeys.contains(Self.hiddenNameKey($0)) }) ?? true else {
            return nil
        }
        return relativePath
    }
    ...
    public func lookupIfExists(parent: UInt64, name: String) throws -> HostFSEntry? {
        try lookupIfExists(parent: parent, name: name, nodeReservation: nil)
    }

    private func lookupIfExists(
        parent: UInt64,
        name: String,
        nodeReservation suppliedReservation: FuseResourceToken?
    ) throws -> HostFSEntry? {
        ...
        let nodeReservation: FuseResourceToken
        if let suppliedReservation {
            nodeReservation = suppliedReservation
        } else {
            nodeReservation = try resourceQuota.acquire(.liveNonRootNodes)
        }
    ...
    public func createFileAndOpen(
        ...
        let relative = join(parentNode.relativePath, name)
        // Reserve identity capacity before O_CREAT can mutate the exported namespace. Coalescing
        // with an already-pinned inode returns the token automatically after registration.
        let nodeReservation = try resourceQuota.acquire(.liveNonRootNodes)
    ...
    public func mkdir(
        ...
        guard let entry = try lookupIfExists(
            parent: parent,
            name: name,
            nodeReservation: nodeReservation
        ) else {
            throw HostFSError.notFound(name)
        }
    ...
    public func link(nodeID: UInt64, newParent: UInt64, name: String) throws -> HostFSEntry {
        guard !readOnly else { throw HostFSError.readOnly }
        try validateComponent(name)
        try requireVisible(name, parent: newParent)
    ...
    public func rename(parent: UInt64, name: String, newParent: UInt64, newName: String) throws -> HostFSEntry {
        ...
        try requireVisible(name, parent: parent)
        try requireVisible(newName, parent: newParent)
        let parentNode = try attachedNode(for: parent)
    ...
    /// Opens one identity-checked directory stream. The caller owns stable FUSE cookie slots; this
    /// object exposes names incrementally and never snapshots, sorts, or registers the directory.
    func openDirectoryCursor(nodeID: UInt64) throws -> HostFSDirectoryCursor {
        let node = try attachedNode(for: nodeID)
        ...
        return HostFSDirectoryCursor(
            owner: ObjectIdentifier(self),
            nodeID: nodeID,
            directory: directory,
            pathForErrors: node.relativePath
        )
    }

    func rewindDirectoryCursor(_ cursor: HostFSDirectoryCursor) throws {
        guard cursor.owner == ObjectIdentifier(self) else {
            throw HostFSError.invalidRoot("directory cursor belongs to another HostFS authority")
        }
        cursor.rewind()
    }

    func nextDirectoryName(from cursor: HostFSDirectoryCursor) throws -> String? {
        guard cursor.owner == ObjectIdentifier(self) else {
            throw HostFSError.invalidRoot("directory cursor belongs to another HostFS authority")
        }
        while let name = try cursor.nextRawName() {
            if name != ".", name != "..", !isHiddenName(name, parent: cursor.nodeID) {
                return name
            }
        }
        return nil
    ...
    private func requireVisible(_ name: String, parent: UInt64) throws {
        guard !isHiddenName(name, parent: parent) else {
            throw HostFSError.notFound(name)
        }
    }

    private func isHiddenName(_ name: String, parent: UInt64) -> Bool {
        isHiddenAtAnyDepth(name)
            || (parent == Self.rootNodeID && rootHiddenNameKeys.contains(Self.hiddenNameKey(name)))
    }

    private func isHiddenAtAnyDepth(_ name: String) -> Bool {
...
/// A live descriptor-relative directory stream. Its lock makes rewind/read indivisible per call;
/// `FuseServer` additionally serializes complete page construction for one guest directory handle.
/// `closedir` owns and closes the descriptor accepted by `fdopendir`.
final class HostFSDirectoryCursor: @unchecked Sendable {
    fileprivate let owner: ObjectIdentifier
    fileprivate let nodeID: UInt64

    private let directory: UnsafeMutablePointer<DIR>
    private let pathForErrors: String
    private let lock = NSLock()
    ...
    fileprivate init(
        ...
        self.pathForErrors = pathForErrors
    }

    fileprivate func rewind() {
        lock.withLock { rewinddir(directory) }
    }

    fileprivate func nextRawName() throws -> String? {
        try lock.withLock {
            errno = 0
            guard let entry = readdir(directory) else {
                let savedErrno = errno
                guard savedErrno == 0 else {
                    throw HostFSError.systemCall("readdir \(pathForErrors)", savedErrno)
                }
                return nil
            }
