...
public extension FuseOpcode {
    var workerOpcodeClass: DoryFSWorkerOpcodeClass {
        switch self {
        case .initOp, .destroy, .notifyReply:
            return .control
        case .lookup, .forget, .getattr, .readlink, .statfs, .getxattr,
             .listxattr, .opendir, .readdir, .releasedir, .getlk, .batchForget:
            return .metadata
        case .open, .read, .release, .flush, .fsync, .fsyncdir, .lseek, .syncfs:
            return .data
        case .interrupt:
            return .interrupt
        default:
...
public enum FuseProtocol {
    ...
    public static func negotiateInit(
        ...
        if writebackCache {
            ...
            // trip on the create storm. Linux ignores FOPEN_NOFLUSH while it is enabled. Production
            // leaves this disabled until dirty-page conflict handling can preserve host edits.
