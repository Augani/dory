...
struct HostFSTests {
    ...
    @Test func authorizedRootDescriptorIsTheOnlyFilesystemAuthority() throws {
        let root = try TestHostFSRoot()
        try root.write("authorized", to: "authorized.txt")
        let authorizedFD = open(root.url.path, O_RDONLY | O_DIRECTORY | O_CLOEXEC)
        #expect(authorizedFD >= 0)
        guard authorizedFD >= 0 else { return }
        defer { Darwin.close(authorizedFD) }

        let moved = root.url.deletingLastPathComponent().appendingPathComponent(
            "\(root.url.lastPathComponent)-moved",
            isDirectory: true
        )
        defer { try? FileManager.default.removeItem(at: moved) }
        try FileManager.default.moveItem(at: root.url, to: moved)
        try FileManager.default.createDirectory(at: root.url, withIntermediateDirectories: false)
        try Data("replacement".utf8).write(to: root.url.appendingPathComponent("replacement.txt"))

        var fs: HostFS? = try HostFS(
            rootDirectoryFileDescriptor: authorizedFD,
            eventRootPath: root.url.path
        )
        let entry = try #require(try fs?.lookup(
            parent: HostFS.rootNodeID,
            name: "authorized.txt"
        ))
        let handle = try #require(try fs?.openRead(nodeID: entry.nodeID))
        #expect(String(decoding: try #require(try fs?.read(handle: handle, offset: 0, count: 32)), as: UTF8.self) == "authorized")
        fs?.close(handle: handle)
        #expect(throws: HostFSError.notFound("replacement.txt")) {
            _ = try fs?.lookup(parent: HostFS.rootNodeID, name: "replacement.txt")
        }

        // HostFS owns only its CLOEXEC duplicate; the bootstrap authority keeps its original lease.
        fs = nil
        #expect(fcntl(authorizedFD, F_GETFD) >= 0)
    }

    @Test func authorizedRootDescriptorRejectsNonDirectoryAndInvalidEventPath() throws {
        let root = try TestHostFSRoot()
        try root.write("payload", to: "file.txt")
        let fileFD = open(root.url.appendingPathComponent("file.txt").path, O_RDONLY | O_CLOEXEC)
        #expect(fileFD >= 0)
        guard fileFD >= 0 else { return }
        defer { Darwin.close(fileFD) }

        #expect(throws: HostFSError.invalidRoot(root.url.path)) {
            _ = try HostFS(
                rootDirectoryFileDescriptor: fileFD,
                eventRootPath: root.url.path
            )
        }
        #expect(throws: HostFSError.invalidRoot("relative/root")) {
            _ = try HostFS(
                rootDirectoryFileDescriptor: fileFD,
                eventRootPath: "relative/root"
            )
        }
    ...
    @Test func readdirplusReturnsSortedEntriesWithAttributes() throws {
        ...
        let entries = try allDirectoryEntries(in: fs, nodeID: HostFS.rootNodeID)
    ...
    @Test func hiddenNamesAreInvisibleToLookupReaddirAndNested() throws {
        ...
        #expect(try allDirectoryEntries(in: fs, nodeID: HostFS.rootNodeID).map(\.name) == ["project"])
        ...
        let project = try fs.lookup(parent: HostFS.rootNodeID, name: "project")
        #expect(try allDirectoryEntries(in: fs, nodeID: project.nodeID).map(\.name) == ["main.swift"])
    ...
    @Test func hiddenNamesCannotBeBypassedWithCaseVariants() throws {
        ...
        #expect(try allDirectoryEntries(in: fs, nodeID: HostFS.rootNodeID).isEmpty)
    ...
    @Test func movedParentSymlinkSwapCannotEscapeShareBeforeHostInvalidation() throws {
        ...
        let source = try fs.lookup(parent: HostFS.rootNodeID, name: "source.txt")
        _ = try allDirectoryEntries(in: fs, nodeID: known.nodeID)
...
/// Test-only full drain for HostFS mechanism assertions. Production directory responses consume the
/// same cursor incrementally and never build this array.
private func allDirectoryEntries(in hostFS: HostFS, nodeID: UInt64) throws -> [HostFSEntry] {
    let cursor = try hostFS.openDirectoryCursor(nodeID: nodeID)
    var entries: [HostFSEntry] = []
    while let name = try hostFS.nextDirectoryName(from: cursor) {
        do {
            if let entry = try hostFS.lookupIfExists(parent: nodeID, name: name) {
