...
struct FuseServerTests {
    @Test func syncfsFallsBackWithoutFailingConnectionAndDestroyAcknowledgesTeardown() throws {
        ...
        let sync = server.handle(request: request(
            unique: 8,
            opcode: .syncfs,
            nodeID: HostFS.rootNodeID,
            payload: [UInt8](repeating: 0, count: 8)
        ))
        let destroy = server.handle(request: request(
            unique: 9,
            opcode: .destroy,
            nodeID: 0
        ))
        ...
        #expect(
            try FuseProtocol.decodeOutHeader(sync).error
                == -FuseProtocol.linuxErrno(ENOSYS)
        )
        #expect(try FuseProtocol.decodeOutHeader(sync).unique == 8)
        #expect(try FuseProtocol.decodeOutHeader(destroy) == FuseOutHeader(
    ...
    @Test func getattrFileHandleSurvivesHostAtomicReplacement() throws {
        ...
        // to an ordinary path open.
        let firstPathGetattr = server.handle(request: pathGetattrRequest)
    ...
    @Test func getattrFileHandleRejectsMismatchedAndUnknownHandles() throws {
        let root = try TestFuseServerRoot()
        ...
        for invalidRequest in invalidRequests {
            let response = server.handle(request: invalidRequest)
    ...
    @Test func pinnedFallbackEnforcesLogicalAccessAndNodeIdentityOnEveryIOPath() throws {
        ...
        let writePayload = bytes(readHandle) + bytes(UInt64(0)) + bytes(UInt32(1))
            + bytes(UInt32(0)) + bytes(UInt64(0)) + bytes(UInt32(0)) + bytes(UInt32(0))
            + Array("X".utf8)
        let rejectedWrite = server.handle(request: request(
            unique: 333,
            opcode: .write,
            nodeID: oldNodeID,
        ...
        let readPayload = bytes(writeHandle) + bytes(UInt64(0)) + bytes(UInt32(32))
            + bytes(UInt32(0)) + bytes(UInt64(0)) + bytes(UInt32(0)) + bytes(UInt32(0))
        let rejectedRead = server.handle(request: request(
            unique: 337,
            opcode: .read,
            nodeID: oldNodeID,
            payload: readPayload
    ...
    @Test func writebackModeAllowsKernelReadOnLogicalWriteOnlyHandle() throws {
        ...
        #expect(try FuseProtocol.decodeOutHeader(read).error == 0)
        #expect(String(decoding: payload(from: read), as: UTF8.self) == "writeback")
        _ = server.handle(request: request(
            unique: 355,
            opcode: .release,
            nodeID: nodeID,
    ...
    @Test func releasedirCannotConsumeAFileHandleFromTheOtherNamespace() throws {
        ...
        let releasedDirectory = server.handle(request: releaseDirectoryRequest)
        #expect(try FuseProtocol.decodeOutHeader(releasedDirectory).error == 0)
    ...
    @Test func readReturnsRequestedSlice() throws {
        let root = try TestFuseServerRoot()
        ...
        let response = server.handle(request: req)

        #expect(try FuseProtocol.decodeOutHeader(response).error == 0)
        #expect(String(decoding: payload(from: response), as: UTF8.self) == "dory")
    }

    @Test func writeAndReleasePersistPayload() throws {
        let root = try TestFuseServerRoot()
        let server = try FuseServer(hostFS: HostFS(rootPath: root.url.path))
        let createIn = bytes(UInt32(0x40 | 0x2)) + bytes(UInt32(0o644))
            + bytes(UInt32(0)) + bytes(UInt32(0)) + Array("written.txt\0".utf8)
        let create = server.handle(request: request(unique: 101, opcode: .create, nodeID: HostFS.rootNodeID, payload: createIn))
        let createPayload = payload(from: create)
        let nodeID = createPayload.leUInt64(at: 0)
        let handle = createPayload.leUInt64(at: 128)
        let writeData = Array("hello dory".utf8)
        ...
        let written = server.handle(request: writeRequest)

        #expect(try FuseProtocol.decodeOutHeader(written).error == 0)
        #expect(payload(from: written).leUInt32(at: 0) == UInt32(writeData.count))

        let releaseIn = bytes(handle) + bytes(UInt32(0)) + bytes(UInt32(0)) + bytes(UInt64(0))
        let releaseRequest = request(unique: 103, opcode: .release, nodeID: nodeID, payload: releaseIn)
        let released = server.handle(request: releaseRequest)

        #expect(try FuseProtocol.decodeOutHeader(released).error == 0)
        #expect(try String(contentsOf: root.url.appendingPathComponent("written.txt"), encoding: .utf8) == "hello dory")
    }
    ...
    @Test func lookupMissHitAndGetxattrResponsesAreCanonical() throws {
        let root = try TestFuseServerRoot()
        ...
        let missing = server.handle(request: missingRequest)

        // Misses are always plain ENOENT. A negative dentry could hide a concurrently created path.
        #expect(missing.count == FuseOutHeader.byteCount)
        ...
        let hit = server.handle(request: hitRequest)

        #expect(hit.count == FuseOutHeader.byteCount + 128)
        #expect(try FuseProtocol.decodeOutHeader(hit).error == 0)

        // One successful response owns exactly one lookup reference. Once detached, one matching
        // FORGET retires the tombstone.
        let hitNodeID = payload(from: hit).leUInt64(at: 0)
        try hostFS.unlink(parent: HostFS.rootNodeID, name: "exists.txt")
        #expect(try hostFS.cachedAttributes(nodeID: hitNodeID).size == 6)
        hostFS.forgetLookup(nodeID: hitNodeID, count: 1)
        #expect(throws: HostFSError.notFound("node \(hitNodeID)")) {
            _ = try hostFS.cachedAttributes(nodeID: hitNodeID)
        }

        let getxattrRequest = request(unique: 203, opcode: .getxattr, nodeID: HostFS.rootNodeID)
        let getxattr = server.handle(request: getxattrRequest)

        #expect(getxattr.count == FuseOutHeader.byteCount)
        #expect(try FuseProtocol.decodeOutHeader(getxattr).error == -FuseProtocol.linuxErrno(ENOSYS))
    }

    @Test func createAndGetattrResponsesAreValid() throws {
        let root = try TestFuseServerRoot()
        let server = try FuseServer(hostFS: HostFS(rootPath: root.url.path))
        let createIn = bytes(UInt32(0x40 | 0x2)) + bytes(UInt32(0o644))
            + bytes(UInt32(0)) + bytes(UInt32(0)) + Array("created.txt\0".utf8)
        let createRequest = request(unique: 210, opcode: .create, nodeID: HostFS.rootNodeID, payload: createIn)
        let createResponse = server.handle(request: createRequest)

        #expect(createResponse.count == FuseOutHeader.byteCount + 144)
        #expect(try FuseProtocol.decodeOutHeader(createResponse).error == 0)
        let createdPayload = payload(from: createResponse)
        ...
        let getattrResponse = server.handle(request: getattrRequest)

        #expect(getattrResponse.count == FuseOutHeader.byteCount + 104)
        #expect(try FuseProtocol.decodeOutHeader(getattrResponse).error == 0)
    ...
    @Test func mkdirUnlinkAndRmdirResponsesAreValid() throws {
        let root = try TestFuseServerRoot()
        ...
        let mkdir = server.handle(request: mkdirRequest)

        #expect(mkdir.count == FuseOutHeader.byteCount + 128)
        #expect(try FuseProtocol.decodeOutHeader(mkdir).error == 0)
        let mkdirPayload = payload(from: mkdir)
        #expect(mkdirPayload.leUInt32(at: 108) == 1_000)
        #expect(mkdirPayload.leUInt32(at: 112) == 1_000)
        ...
        let unlink = server.handle(request: unlinkRequest)

        #expect(unlink.count == FuseOutHeader.byteCount)
        ...
        let rmdirRequest = request(unique: 222, opcode: .rmdir, nodeID: HostFS.rootNodeID, payload: Array("nested\0".utf8))
        let rmdir = server.handle(request: rmdirRequest)

        #expect(rmdir.count == FuseOutHeader.byteCount)
    ...
    @Test func readdirplusReturnsPackedDirectoryEntries() throws {
        ...
        let entries = parseDirentPlusRecords(data)

        #expect(header.error == 0)
        #expect(entries.count == 2)
        #expect(Set(entries.map(\.name)) == Set(["a.txt", "b.txt"]))
        #expect(entries.map(\.nextOffset) == [1, 2])
    }
    ...
    @Test func readdirplusOffsetRemainsStableWhenEarlierPageIsDeleted() throws {
        ...
        let firstEntries = parseDirentPlusRecords(first)
        let firstEntry = try #require(firstEntries.first)
        #expect(firstEntries.count == 1)
        #expect(Set(["a.txt", "b.txt", "c.txt"]).contains(firstEntry.name))
        #expect(firstEntry.nextOffset == 1)

        let removed = server.handle(request: request(
            unique: 2022,
            opcode: .unlink,
            nodeID: HostFS.rootNodeID,
            payload: Array("\(firstEntry.name)\0".utf8)
        ))
        ...
        let secondEntries = parseDirentPlusRecords(second)
        let secondEntry = try #require(secondEntries.first)
        #expect(secondEntries.count == 1)
        #expect(secondEntry.name != firstEntry.name)
        #expect(Set(["a.txt", "b.txt", "c.txt"]).contains(secondEntry.name))
        #expect(secondEntry.nextOffset == 2)
    }
    ...
    @Test func repeatedOffsetZeroDuringDeletionKeepsOriginalCookieSpace() throws {
        ...
        let original = parseDirentPlusRecords(readFromStart(unique: 2028))
        let removed = try #require(original.first)
        try FileManager.default.removeItem(at: root.url.appendingPathComponent(removed.name))
        let repeated = readFromStart(unique: 2029)
        let surviving = parseDirentPlusRecords(repeated)
        // Directory order is owned by the host filesystem. The removed name keeps an empty stable
        // cookie slot, and every survivor retains its original cookie regardless of that order.
        #expect(original.count == 3)
        #expect(surviving.count == 2)
        #expect(!surviving.map(\.name).contains(removed.name))
        let originalCookies = Dictionary(uniqueKeysWithValues: original.map { ($0.name, $0.nextOffset) })
        #expect(surviving.allSatisfy { originalCookies[$0.name] == $0.nextOffset })
    }
    ...
    @Test func cachePolicyDefaultsSafeAndActivatedValuesAreBounded() throws {
        ...
        #expect(payload(from: activatedLookup).leUInt64(at: 16) == FuseServer.maximumCoherentCacheValiditySeconds)
        #expect(payload(from: activatedLookup).leUInt64(at: 24) == FuseServer.maximumCoherentCacheValiditySeconds)
    }
            ...
            for _ in 0..<8 {
                group.addTask {
                    for _ in 0..<1_000 {
                        ...
                        guard validity == 0
                                || validity == FuseServer.maximumCoherentCacheValiditySeconds else {
                            return false
                        }
    ...
    @Test func flockOwnersConflictShareAndReleaseIndependently() throws {
        ...
        let secondRelease = server.handle(request: request(
            unique: 711,
            opcode: .release,
            nodeID: nodeID,
        ...
        let thirdRelease = server.handle(request: request(
            unique: 713,
            opcode: .release,
            nodeID: nodeID,
...
private struct ParsedDirentPlus: Equatable {
    let name: String
    let nextOffset: UInt64
}

private func parseDirentPlusRecords(_ data: [UInt8]) -> [ParsedDirentPlus] {
    var records: [ParsedDirentPlus] = []
    var offset = 0
    while offset + 128 + 24 <= data.count {
        let nameLength = Int(data.leUInt32(at: offset + 128 + 16))
        let recordLength = alignedDirentPlusLength(nameLength: nameLength)
        guard nameLength > 0, offset + recordLength <= data.count else { break }
        let nameStart = offset + 128 + 24
        records.append(ParsedDirentPlus(
            name: String(decoding: data[nameStart..<(nameStart + nameLength)], as: UTF8.self),
            nextOffset: data.leUInt64(at: offset + 128 + 8)
        ))
        offset += recordLength
    }
    return records
