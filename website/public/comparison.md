...
| **Platform** | Apple Silicon, macOS 14+ Sonoma. The raw `dory-hv` helper requires macOS 15+; a full Sonoma bundle routes to the included Virtualization.framework `dory-vmm` fallback. Intel is a later roadmap phase and is not part of the current public artifact or Homebrew contract. [[3]](#3) | macOS (Intel + Apple silicon), Windows, Linux. | macOS (Apple silicon; Intel support is limited). |
