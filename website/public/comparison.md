...
| **Idle memory posture (macOS)** | Runs all containers in **one shared VM** so idle memory stays flat as you add containers. Dory measures roughly **4.7× less idle memory than the one-VM-per-container model** for two containers (~122 MB vs ~574 MB). *Measured by Dory; the reproducible harness and publication rules are in `BENCHMARKS.md` and `scripts/benchmark-compare.sh`. Treat this as Dory's own figure unless a release artifact includes the raw run.* | Users have reported a persistent idle footprint in the **~3-4 GB** range, tied to a fixed VM memory block that macOS does not reclaim, plus elevated idle CPU/battery reports. [[6]](#6) [[7]](#7) [[8]](#8) | Also uses a single shared VM (one of the reasons its footprint is low). A head-to-head Dory-vs-OrbStack memory comparison must be measured on the same Mac before we publish a multiplier. |
...
- The idle-memory row deliberately uses the **corroborated ~3-4 GB** Docker Desktop baseline and
  labels Dory's ~4.7× figure as an internal measurement until a release artifact includes raw
  `benchmark-compare.sh` output. It does **not** claim any specific multiple against OrbStack; that
  comparison must be measured on the same Mac first.
