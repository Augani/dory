...
| Runtime mode settings | ✅ | `dory mode manual|auto-idle|always-on|battery-saver|show` persists the user's intended availability mode and last confirmed running/sleeping intent. Always On starts after daemon/host restart; the other modes restore whether the engine was awake or deliberately sleeping. `dory idle status` explains current sleep blockers such as running containers, published ports, pinned labels, and Kubernetes config. |
