...
| Network list / create / inspect / connect / disconnect / remove / prune | ✅ | Shipping Dory and Docker-backed fallback paths preserve native dockerd targets, filters, bodies, flags, statuses, and errors. Live qualification covers bridge driver/options, internal/attachable flags, explicit subnet/range/gateway/auxiliary IPAM, labels and filtered listing, duplicate-name and overlapping-subnet conflicts without mutation, static-IP/alias attachment, active-endpoint removal safety, explicit detach/delete, scoped DNS aliases, port liveness, prune safety, and restart persistence. Translated test runtimes retain their smaller compatibility model. |
| Volume list / create / inspect / remove / prune | ✅ | Shipping Dory and Docker-backed fallback paths preserve dockerd's exact filters, driver options, labels, delete flags, statuses, payloads, and errors. Live qualification covers fresh-root emptiness, filtered listing, same-name idempotency without metadata mutation, in-use rejection, exact copy bytes, restart persistence, active-volume prune safety, explicit removal, and deletion persistence. Mock/translated test runtimes retain their smaller compatibility model. |
