...
| P01-03, P01-05 | Exact plans bind media, firmware manifests, persistence, devices, capabilities, guest and overhead resources, ABI and schema identity. Backend arguments and runner envelopes preserve these commitments. | Contract complete; final entrypoint regression remains part of the phase exit. |
| P01-04 | Start/restart share preparation; compound operations borrow the caller's planning context. Mutable backing ownership is checked before quiescence, and fresh launch authority is acquired at process boundaries. Creation and snapshot publication reuse private verification handoffs within one context. | Final restore handoff and start call-graph checks underway. |
...
| P01-08 | Create, clone, restart, configuration, installer, desktop, snapshot and restore use typed durable roots with private recovery payloads, exact source/target authority and owned compensation. App/CLI/XPC preserve UUIDs. | Final ordinary-start planning/admission consolidation underway. |
| P01-09 | Reconnect authentication binds plan, operation, PID and process start time. Cancellation owns its control token, observes helper termination before guest rollback, and settles the same admission/journal leases. | Final compound-cancellation and authenticated fixture parity checks underway. |
...
| P01-12 | Public-operation campaigns cover supported/unsupported cells, real pinned media, missing components, malformed input, stale plans, altered artifacts, concurrent mutation, cancellation, replay and restart recovery. Rejected preflight preserves source state and authority. | Final combined regression and new start/restore handoff cases underway. |
| P01-13 | Release builds exclude diagnostic constructors, legacy launch construction and qualification bootstrap paths. Production entrypoints use daemon-owned planning; architecture tests inspect SwiftPM's resolved dependency graph. | Release build/symbol audit passed; final start entrypoint parity remains. |
...
The app transport campaign passes all 83 client tests, including create/clone/snapshot/restore UUID round trips. The earlier three projection tests cover generation ordering and readiness presentation. App tests run in a copied Debug host with a separate bundle identifier and ad-hoc signature; code and dependency identities are retained, and the installed app stays running. This verifies app code without claiming original-identity launch or release-signing qualification.
