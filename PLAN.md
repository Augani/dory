...
- **Progress (2026-09-09):** Long-mode 32/64-bit memory `CMPXCHG`, including the accepted `LOCK` form, implicit-lock 32/64-bit memory `XCHG`, 32/64-bit memory `XADD` including its `LOCK` form, 32/64-bit `LOCK ADD/SUB/AND/OR/XOR`, and 32/64-bit `LOCK INC/DEC/NOT/NEG` now resolve through the per-vCPU write TLB into C runtime host-address atomics. Register and immediate binary sources are admitted. Naturally aligned RAM uses compiler-lowered sequentially consistent C11 compare-exchange/exchange/fetch and RMW operations; page faults return to the precise interpreter boundary, while unaligned, cross-page, MMIO, and tracked page-table operands decline without effects to the existing serialized fallback. The interpreter and native fallback share one C-owned split-lock mutex, preserving single-copy behavior during incremental conversion. The binary and unary RMW operation/width matrices match the interpreter's result and flags in both tiers, including INC/DEC carry preservation and NOT flag preservation; immediate-source and unaligned-decline precision also pass alongside success/mismatch, swap, returned-old-value, cached-translation reuse, fault rollback, and cross-vCPU serialization coverage. The complete DBT/PC gate passes (1,351 tests, 167 suites). `LOCK ADC/SBB`, locked bit RMW, `CMPXCHG8B/16B`, and the complete litmus matrix remain before closure.
