...
**Progress, 2026-09-04:** P02 remains in progress. The [correctness receipt through run 51](docs/virtualization/evidence/p02-correctness-2026-09-04/review-through-run-51.json) binds 723 tracked inputs to `c34e979ff`, plus six identified generated FFI inputs. Run 51 passes 960 Swift Testing cases and 10 XCTest cases, with one optional boot-fixture skip and one disabled physical-reference test. Its [independent validation](docs/virtualization/evidence/p02-correctness-2026-09-04/evidence-validation-through-run-51.json) checks exact source, artifacts, schema, individual counts and eight pending floating-point fault methods. These cover waiting boundaries and selected environment-status transitions; arithmetic exception generation, legacy `CR0.NE=0` signaling and contradictory FXRSTOR state remain open. The [source-50 receipt](docs/virtualization/evidence/p02-correctness-2026-09-04/review-through-run-50.json) retains the optimized runner and the widened VirtIO filter; source 51 adds no guest or optimized-build outcome. The [through-49 receipt](docs/virtualization/evidence/p02-correctness-2026-09-04/review-through-run-49.json) preserves run 48's fixture failures and unchanged product source on retry. Overlapping runs are not summed.
