...
- **Progress (2026-09-09):** Long-mode 32/64-bit memory `CMPXCHG`, including the accepted `LOCK` form, now resolves through the per-vCPU write TLB into a C runtime host-address atomic. Naturally aligned RAM uses the compiler's sequentially consistent C11 atomic compare-exchange (lowered to the available ARM64 atomic sequence); page faults return to the precise interpreter boundary, while unaligned, cross-page, MMIO, and tracked page-table operands use the existing scalar fallback. The interpreter and native fallback now share one C-owned split-lock mutex, preserving single-copy behavior during incremental conversion. Success/mismatch parity, cached-translation reuse, unaligned fallback, fault rollback, and cross-vCPU serialization pass; the complete DBT/PC gate passes (1,345 tests, 167 suites). `XCHG`, general locked RMW, `CMPXCHG8B/16B`, and the complete litmus matrix remain before closure.
