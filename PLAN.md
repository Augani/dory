...
- **Progress (2026-09-09):** Long-mode 32/64-bit memory `CMPXCHG`, including the accepted `LOCK` form, and implicit-lock 32/64-bit memory `XCHG` now resolve through the per-vCPU write TLB into C runtime host-address atomics. Naturally aligned RAM uses compiler-lowered sequentially consistent C11 compare-exchange/exchange operations; page faults return to the precise interpreter boundary, while unaligned, cross-page, MMIO, and tracked page-table operands use the existing serialized fallback. The interpreter and native fallback share one C-owned split-lock mutex, preserving single-copy behavior during incremental conversion. Success/mismatch and swap parity, cached-translation reuse, unaligned fallback, fault rollback, flags preservation, and cross-vCPU serialization pass; the complete DBT/PC gate passes (1,346 tests, 167 suites). General locked RMW, `CMPXCHG8B/16B`, and the complete litmus matrix remain before closure.
