...
**Reviewed:** 2026-09-03; implementation verification and delivery status updated 2026-09-05.
...
| macOS ARM64 | A development VM completes installation, reaches the desktop, saves/restores through a new qualifier process and passes a 1,048,576-element guest Metal compute check. The user reports better responsiveness at 4 CPUs / 8 GiB. | Production MachineManager lifecycle, effective device policy, sustained graphics and release qualification remain P08. Qualifier results do not establish the complete product lifecycle. |
...
The native Mac lifecycle now passes a [production MachineManager composition check](docs/virtualization/evidence/p07-macos-2026-09-05/production-lifecycle-composition.json): start, suspend, mutable-disk provenance renewal and resume use a real Apple-prepared bundle with an authenticated substituted helper. The fixes align capability admission, reconnect descriptor forwarding, canonical definition authority and prepared-bundle reload. The composition test, 41 capability checks, 24 lifecycle/journal checks and one provenance regression pass. This fixture executes no VZ VM and uses synthetic saved memory; F10 and physical production-helper continuation remain open. Apple save/restore and private saved-state file validation are implemented. A subsequent [actual helper campaign](docs/virtualization/evidence/p07-macos-2026-09-05/actual-helper-saved-state-progress.json) authenticates a running VM, saves 1,509,953,536 bytes of Apple state and exits cleanly; a new helper fails restore with Apple permission-denied error12. The same failure occurs after waiting 120 seconds before save and when preserving the original saved-state filename. Apple’s Virtualization service logs identify `SecKeyCreateDecryptedDataWithParameters` failing with `NSOSStatusErrorDomain -25308` and an inability to compute the shared secret. Both the same-bundle qualifier and a fresh installation also fail new-process restore. A subsequent [read-only host observation](docs/virtualization/evidence/p07-macos-2026-09-05/host-lock-restore-observation.json) reports the console screen locked. Repeat restore after normal user unlock before attributing the denial to helper identity; the lock observation alone does not prove causation. Neither startup delay nor relaxed file permissions is an established fix. Resolve that denial before claiming successful managed continuation or full daemon lifecycle. The [disabled-camera runtime check](docs/virtualization/evidence/p07-macos-2026-09-05/disabled-camera-runtime.json) now confirms that a signed native runtime constructs no camera bridge when disabled. Managed launches default the camera off and reject unsupported enable requests. Thirteen focused XCTest cases and three Swift Testing cases pass; this is device-policy evidence without starting a guest.

The signed embedded-metadata daemon candidate now starts an isolated GPU engine and renderer worker. The real Linux guest identifies Virtio-GPU Venus (Apple M2 Pro), creates a Vulkan device, submits work and signals a fence ([guest readiness evidence](docs/virtualization/evidence/p06-container-2026-09-05/guest-vulkan-readiness.json)). This advances the earlier carrier probe to actual guest execution. A real ARM64 container also passes a Vulkan shader dispatch with all 65,536 outputs matching a CPU reference ([compute evidence](docs/virtualization/evidence/p06-container-2026-09-05/container-shader-compute.json)). The refreshed daemon also passes the same shader workload through standard Docker `--gpus all`, with inspected translation to the exact render node and `rw` permissions ([standard request evidence](docs/virtualization/evidence/p06-container-2026-09-05/container-standard-gpu-shader.json)). The earlier API failure came from a stale linked FFI archive and was resolved by the existing artifact producer. Application Settings flow and sustained workloads remain open. After fixing reuse of a one-shot renderer bootstrap, a disposable endpoint passes one automatic engine restart following an idle renderer kill: the volume sentinel survives and standard `--gpus all` compute returns 65,536 correct outputs again ([recovery evidence](docs/virtualization/evidence/p06-container-2026-09-05/container-gpu-recovery.json)). That recovery campaign explicitly sets driver environment variables. A subsequent supported-image build installs the tuple-bound ICD in the standard Vulkan search directory; an independent 60-second run without driver environment overrides completes 2,345 device/create/dispatch/teardown cycles and verifies 153,681,920 outputs with zero mismatches ([repeated compute evidence](docs/virtualization/evidence/p06-container-2026-09-05/container-repeated-no-env-compute.json)). The repository now provides `guest/initfs/export-container-image.sh` to verify and export the managed root-owned image, with explicit ARM64 import and source labels ([producer evidence](docs/virtualization/evidence/p06-container-2026-09-05/container-supported-image-producer.json)). A [pending-fence recovery campaign](docs/virtualization/evidence/p06-container-2026-09-05/renderer-pending-fence-failure-recovery.json) observes VK_NOT_READY after submission before killing the disposable renderer, then verifies renewed bootstrap authority, a preserved volume sentinel and successful fresh no-env compute. It does not establish fence status at the exact signal instant. The interrupted container reappears as Created with exit=0. Retained metadata contains a temporary running-state checkpoint and a committed never-started record, consistent with Docker 29.6.1 starting the task before committing its checkpoint. A maintained Docker 29.6.1 source patch now checkpoints start intent before task execution and reconciles interrupted, running, paused and indeterminate task states. Its [12 focused ARM64 tests](docs/virtualization/evidence/p06-container-2026-09-05/docker-start-intent-patch.json) pass independently, including disk reload and checkpoint failure injection. The [pinned binary producer and explicit initfs override](docs/virtualization/evidence/p06-container-2026-09-05/docker-producer-source.json) are implemented: admission checks artifact digest, architecture, current producer inputs and successful version execution. A stale ARM64 receipt was correctly rejected; a subsequent rebuild against the final producer passes version execution and initfs override admission. Independent digest and ownership checks match the earlier ARM64 binary and archive. A [disposable patched-daemon restart](docs/virtualization/evidence/p06-container-2026-09-05/patched-docker-restart.json) runs Docker 29.6.1-dory1, restores an interrupted container as exited/255, preserves its volume sentinel, and successfully runs a fresh ARM64 container. The same work fixes existing-file `/tmp` versus `/private/tmp` disk-authority mismatch; 14 engine policy checks pass independently. The exact pre-ack checkpoint boundary and a repeated GPU renderer-crash campaign remain required before production integration; the ordinary running image still uses upstream Docker. Upstream AMD64 static dockerd also exits 255 under the current FEX endpoint. [Minimal static Go probes](docs/virtualization/evidence/p06-container-2026-09-05/static-go-fex-startup.json) reproduce pre-main faults with Go 1.25.9 on AMD64 while the same sources pass natively on ARM64; GOAMD64 v1–v3 do not resolve it. The failure is broader than the Moby patch, but the exact FEX/runtime defect remains under investigation before an AMD64 candidate can be admitted. Long-term resource/performance qualification remains open, and the original observation does not establish a Dory storage failure. Arbitrary third-party images still need compatible guest drivers. Killing an idle renderer with restart disabled stops the engine and removes Docker availability ([fail-stop evidence](docs/virtualization/evidence/p06-container-2026-09-05/renderer-fail-stop.json)). This proves bounded failure containment, not automatic recovery or outstanding-work handling.

The pinned x86_64 Mesa VirGL2 runtime now builds with a matching GLX loader, Gallium library and virtio-gpu DRI driver. Archive, source, dependency and ELF checks pass ([build evidence](docs/virtualization/evidence/p06-pc-2026-09-05/mesa-virgl2-build.json)). The kernel and rootfs producers also pass their artifact checks. The initial [GRUB diagnostic boot](docs/virtualization/evidence/p06-pc-2026-09-05/grub-linux-boot-progress.json) reached Linux HPET/interrupt initialization but exhausted its budget before userspace. A subsequent [deterministic-clock control](docs/virtualization/evidence/p06-pc-2026-09-05/deterministic-ioapic-fault.json) exposed a guest general-protection fault caused by rejecting a transient IOAPIC reserved-vector register write. The fix preserves register readback while withholding invalid interrupt delivery; 10 focused tests pass. The same 1.2-billion-instruction control now passes that fault and reaches early kernel `__text_poke`, with no panic or general-protection fault in serial output. A [longer diagnostic](docs/virtualization/evidence/p06-pc-2026-09-05/efi-runtime-boot-fault.json) advances beyond SMP alternatives, then stops at 1,207,840,665 instructions with an instruction-fetch page fault while Linux calls the EFI runtime variable service during `efi_enter_virtual_mode`. A subsequent [firmware-only ownership probe](docs/virtualization/evidence/p06-pc-2026-09-05/efi-runtime-pointer-ownership.json) identifies that address as DXE Core’s default unavailable-variable-service stub in BootServicesCode memory. Dory’s variable driver has not installed its services; its dispatch and initialization must be diagnosed. The probe also verifies that ResetSystem points to separately allocated runtime code. Promoting the stub’s memory to runtime would not fix the missing variable service. An [EFI-disabled isolation run](docs/virtualization/evidence/p06-pc-2026-09-05/efi-disabled-isolation.json) advances beyond that fault to 2.5 billion instructions, then is deliberately terminated after prolonged absence of new progress. Its empty stdout is not a valid terminal boot receipt; disabling EFI runtime is not a production fix. The next [bounded diagnostic](docs/virtualization/evidence/p06-pc-2026-09-05/xhci-split-dword-boot-fault.json) completes its 3-billion-instruction budget but records a Linux init panic in `xhci_mem_init`: the controller rejected Linux’s 32-bit low/high writes to a 64-bit address register. The fix accepts split writes for all four xHCI address registers and preserves register side effects; eight focused tests pass, including device setup through both write forms. A full guest rerun is still required. Userspace has not yet been reached. Deterministic clock results do not qualify the production host-monotonic clock. A [sample-guided JIT change](docs/virtualization/evidence/p06-pc-2026-09-05/jit-memory-callback-profile.json) also skips memory callback setup for blocks that require no callbacks; 103 focused tests pass, but whole-boot speedup is not established. A subsequent [CPU feature lookup optimization](docs/virtualization/evidence/p06-pc-2026-09-05/cpu-feature-membership-profile.json) resolves immutable feature dependencies once. It matches the old implementation across 10,000 generated profiles and passes 56 profile/paging unit checks after correcting a baseline-invalid stack descriptor fixture. Its isolated lookup improves; whole-boot speedup remains unmeasured. A separate PC paging debug test still hits a decoder stack guard in both baseline and candidate builds. Complete userspace boot, ordinary interrupt routing and a real guest GLX workload remain required before PC GPU admission can open.
