...
**Progress, 2026-09-04:** P02 remains in progress. The [correctness receipt through run 37](docs/virtualization/evidence/p02-correctness-2026-09-04/review-through-run-37.json) binds 698 tracked source inputs to commit `123b37c97`. Run 37 passes 789 Swift Testing cases and 10 XCTest cases, with one optional boot-fixture skip and one disabled physical-reference test. Run 36's failed symlink-test fixture and its correction remain retained; production validation was unchanged. An [independent validation](docs/virtualization/evidence/p02-correctness-2026-09-04/evidence-validation-through-run-37.json) checks the artifact hashes, source reconstruction, schemas, individual result counts, inventory and paired boot receipts. Overlapping runs are not summed.

[Interpreter probe 3](docs/virtualization/evidence/p02-correctness-2026-09-04/userspace-interpreter-probe-3.json) and [baseline-JIT probe 9](docs/virtualization/evidence/p02-correctness-2026-09-04/userspace-baseline-jit-probe-9.json) use the same source-32 runner, pinned 6.18.35 kernel, musl v2 initramfs, machine configuration and budgets. Their seven workload result objects agree after removing only each fresh run UUID, and both independently reach host-observed ACPI S5. This closes P02-24 for the diagnostic's observable results, without claiming identical final memory, ASLR, timer scheduling or instruction counts. The interpreter retires 750,792,741 instructions in 1,150.549 seconds; baseline JIT retires 750,370,506 in 386.565 seconds. These are engineering measurements on the experimental host. The second kernel's [glibc probe 2](docs/virtualization/evidence/p02-correctness-2026-09-04/userspace-glibc-kernel-b-probe-2.json) remains a retained ISA-level rejection/reset without an accepted receipt. The systemd fixture has 40 passing preparation tests and awaits guest execution.

The [verified support inventory](docs/virtualization/evidence/p02-correctness-2026-09-04/run-37-isa-inventory.json.gz) closes P02-08: 1,158 exact vectors, 1,081 static decodes, 77 rejections and zero expectation mismatches. Its small catalog binds three historically retired forms and one fault attempt to run-30 test/source evidence; it records zero current-invocation or physical-reference executions and leaves other forms/dimensions unmeasured. The system-group audit, range preflight, runner/required-fixture CI source, time audit and bounded fuzzing already close P02-04/06/07/17/21. Two real kernels have different internal symbol layouts but identical `PT_LOAD` envelopes. Run 37 also covers SIMD execution-control fault priority; full CPU-profile/floating-point/ISA qualification, physical x86 reference, glibc/systemd acceptance, sustained stress and hosted CI remain open. The [paging and CPU audit](docs/virtualization/p02-paging-audit-2026-09-04.md) maps each closure to its evidence. P03 and P04 follow P02 as requested.
