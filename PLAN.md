...
| PC GPU qualification packaging | `RendererBootstrapQualificationCommand` currently hardcodes the managed ARM producer-fence contract, Mesa digest and combined VirGL/Venus capabilities. The PC runtime requires a distinct x86 kernel/Mesa binding, PC producer-fence contract and VirGL2-only capability receipt. | Add explicit PC qualification inputs and execute the signed worker with those exact inputs. Verify architecture/producer bindings as well as hashes: the first signed build supplied an x86 kernel to the old ARM-only command, so its successful build/signature is not valid guest-profile qualification. Stage and sign a separate PC receipt; verify, seal and prune it throughout packaging. Copying the ARM receipt to the PC filename cannot authorize a PC launch. Prove profile mismatch rejection and then run physical guest acceleration/reset checks on the resulting signed artifact. |
