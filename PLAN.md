...
- **x86 VM performance:** reduce measured timer/interrupt-path interpreter fallbacks, preserve precise faults and debug behavior, then rerun ordinary UEFI boot through the production runner. [Native RDTSC](docs/virtualization/evidence/p06-pc-2026-09-05/native-rdtsc.json) now passes 100 focused tests; its latest guest run must be repeated because the temporary raw receipt is no longer available. [Bounded clock comparisons](docs/virtualization/evidence/p06-pc-2026-09-05/clock-source-comparison.json) do not establish a clock fix or usable boot performance. The [combined JIT runner](docs/virtualization/evidence/p06-pc-2026-09-05/combined-jit-post-cpu0.json) now advances beyond the prior CPU0 stall to PCI/HPET initialization and TSC clocksource selection; userspace completion and useful responsiveness remain unverified.
