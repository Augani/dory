...
**Change boundary of this review:** documentation consolidation plus removal of obsolete document-only checks and mechanical tooling-reference updates. No runtime implementation, runtime-code removal, guest installation, or release is performed in this review.
...
- [ ] **P00-02** Rebuild the Swift package test products from a clean task-specific build location; fix the missing/incompatible cached-test-product setup. Do not treat `--skip-build` discovery failure as a runtime failure or a test pass.
...
- [ ] **P00-09** Add behavior/structured-contract coverage for backend selection and qualification policy that the deleted documentation-only checks never proved. The obsolete dossier checker and public-document assertions were removed in this cleanup; retain existing runtime checks and close the behavioral coverage gaps explicitly.
...
- [ ] **P01-01** Write the three-cell backend table as executable policy. Host ARM64 + Linux ARM64 → native DoryARMVirt; host ARM64 + Linux x86_64 → DoryPC/DBT; host ARM64 + macOS ARM64 → VZMac. Reject macOS x86_64, Intel hosts and unknown combinations before component download or disk mutation.
- [ ] **P01-02** Distinguish requested architecture, detected media architecture, host architecture, CPU profile, machine ABI and execution tier. A template label or filename must not substitute for media inspection.
- [ ] **P01-03** Make `DoryResolvedMachinePlan` the immutable launch input containing verified media/artifact references, guest resources, device policy, effective capabilities, persistence roots and schema/ABI identities.
...
- [ ] **P01-05** Bind runner arguments/envelope to that exact plan. The runner verifies its received resources and implements it; it does not choose a different backend, kernel, GPU or network mode.
- [ ] **P01-06** Carry graphics preference as a requirement or an explicitly selected recovery mode. Requested acceleration failing to initialize is a visible error; recovery to software requires a separate explicit operation and cannot report accelerated support.
- [ ] **P01-07** Model installation, stopped, starting, running, stopping, suspended, failed and recovering states once, with explicit operation progress. Distinguish process alive, VM started, guest booted, tools connected, desktop visible and workload-ready.
...
- [ ] **P01-10** Reconcile host memory/CPU admission across multiple VMs and the container engine. Account for guest RAM, DBT caches, renderer resources, disk staging and worker overhead; give users actionable capacity errors.
- [ ] **P01-11** Define migration for old backend names, distro-led definitions, disk identities and saved-state schemas. Retain original records until the new composition is validated; reject unsafe in-place conversion without deleting workloads.
