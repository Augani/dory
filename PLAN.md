...
- **Progress (2026-09-10, single-access memory-target chaining measurement):** With A05.4 closed, raw direct and IBTC links now admit single-access memory targets whenever the active execution carries memory capabilities. One callback authority spans the complete native chain, and a forced target read fault proves that both baseline and tier-one recover the exact two-block prefix through one native dispatcher entry. Restartable multi-access targets and guarded baseline exits remain separated. The metadata suite passes 19 tests and the deterministic DBT/PC/runner gate passes 1,553 tests in 179 suites. A frozen release run passes all seven authenticated workloads and powers off after 723,050,284 instructions in 337.255 seconds. Direct bypasses rise from about 44.9 million to 105.7 million and native dispatcher entries fall from about 190.1 million to 125.0 million; normalized entries fall from 259,174 to 172,876 per million instructions, while the within-run reduction improves from 19.11% to 45.83%. This is a meaningful structural gain but still far below >95%, and the unmatched wall regression is not accepted as a performance improvement. A05.3 stays open for block-scoped restartable-read chaining and further dispatcher elimination. [Receipt](docs/virtualization/evidence/a05-tier1-2026-09-10/pvh-single-access-memory-target-chaining.json)
- **Progress correction (2026-09-10, all multi-access target widening rejected):** An append-only context word let each generated memory block select its own restartable-read policy after a raw link, and focused coverage proved a two-read target could decline its second access with an exact two-block prefix. The deterministic gate passed 1,554 tests in 179 suites, but the pinned production workload rejected the broader admission: `/init` soft-locked in `unmap_page_range` during `execve`, produced no authenticated receipt, and exhausted the full one-billion-instruction budget after 408.130 seconds. Failure-run bypass share reached only 51.44%, still far below >95%. The candidate is reverted in `6daaad647`; accepted single-access memory-target chaining remains. A05.3 stays open, and the next widening must separate read-only multi-access targets from read-modify/write and stack-memory targets. [Receipt](docs/virtualization/evidence/a05-tier1-2026-09-10/pvh-restartable-memory-target-chaining-rejection.json)
- **Progress (2026-09-10, read-only multi-access target chaining):** Compiled blocks now state whether generated execution may commit a guest-memory write, and every memory-bearing block publishes its own restartable-read policy before its first callback. Direct and IBTC links therefore admit multi-access targets only when they are read-only; load/store, RMW, stack-memory, and other writers retain the dispatcher boundary after the broad-widening soft lock. Baseline and tier-one regressions prove exact cross-block prefix recovery when the second read declines and separately prove that a multi-access writer receives two dispatcher entries with no raw bypass. The focused policy gate passes four tests and the deterministic aggregate passes 1,555 tests in 179 suites. A frozen release run passes all seven authenticated workloads and powers off after 721,880,626 instructions in 315.850 seconds. Against the accepted single-access slice, direct bypasses rise from 105.7 million to 116.9 million, normalized dispatcher entries fall 8.46% from 172,876 to 158,256 per million instructions, and within-run avoidance rises from 45.83% to 50.58%. This accepts the narrower correctness boundary but remains far below the strict >95% gate; the unmatched 6.35% wall reduction is directional only. A05.3 stays open. [Receipt](docs/virtualization/evidence/a05-tier1-2026-09-10/pvh-read-only-restartable-target-chaining.json)
- **Progress correction (2026-09-10, legacy-to-tier-one chaining rejected):** A one-way mixed-compiler experiment admitted legacy baseline/optimizing sources into tier-one targets while keeping tier-one-to-legacy transitions separated. A directed regression used a legacy-only memory bit-test/branch to feed carry into tier-one ADC with exact state and one fewer warm dispatcher entry, and the deterministic aggregate passed 1,556 tests in 179 suites. Production rejected the broader contract: the guest emitted corrupted/NUL-containing PCI ECAM and TSC strings, entered `/init`, then looped through `unmap_page_range`, page-table locking, and rescheduling until the full one-billion-instruction budget expired after 338.803 seconds without an authenticated receipt. The failed run reached 200.0 million generated bypasses and 65.10% within-run avoidance, still below >95%. The likely ABI hole is that callback-free legacy sources do not establish the complete x0-x5 callback argument set that a tier-one target must preserve for later chained memory blocks, but the exact corrupting edge is not proven. The candidate is reverted in `ee4079587`; mixed-compiler links remain forbidden pending an edge-local ABI bridge. A05.3 stays open. [Receipt](docs/virtualization/evidence/a05-tier1-2026-09-10/pvh-legacy-to-tier1-chaining-rejection.json)
- **Progress (2026-09-10, chain-target rejection ranking):** Every dispatcher-observed resident edge considered for a direct patch or IBTC fill is now assigned to one bounded accepted/rejected counter, and the PC aggregate, UEFI smoke record, and PVH diagnostics expose the same histogram. A repeated multi-access writer regression proves attempts and the exact rejection bucket advance without a false accept; the focused diagnostics gate passes six tests and the deterministic aggregate passes 1,555 tests in 179 suites. The behavior-neutral frozen runner passes all seven workloads and powers off after 722,929,984 instructions in 320.254 seconds. All 107,147,294 attempts reconcile exactly: compiler-ABI mismatch is 56,066,297 (52.33%), restartable writers 31,746,152 (29.63%), guarded legacy targets 14,639,944 (13.66%), target/time boundaries 3,969,144 (3.70%), and accepts 725,757 (0.68%); source-shape, missing-memory, and publication failures are zero. This ranks an explicit mixed-compiler edge bridge first, but the preceding unbridged production failure forbids a simple admission change. A05.3 remains at 50.26% within-run avoidance and stays open. [Receipt](docs/virtualization/evidence/a05-tier1-2026-09-10/pvh-chain-target-rejection-ranking.json)
- **Progress correction (2026-09-10, directional ABI telemetry rejection):** A diagnostics-only refinement split compiler-ABI rejections by source/target compiler direction without changing chain admission. The deterministic 1,555-test/179-suite aggregate passed, but two runs of the identical release binary both terminated before emitting terminal output or a diagnostic receipt. The candidate is fully reverted as a reproducible layout-sensitive host-side reliability failure; no directional count, elapsed time, instruction total, or exact exit status is inferred from the missing receipts. The production-safe coarse histogram remains authoritative, and any directional measurement must be out of process or otherwise binary-layout-neutral. A05.3 remains at 50.26% within-run avoidance and stays open. [Receipt](docs/virtualization/evidence/a05-tier1-2026-09-10/pvh-chain-abi-direction-telemetry-rejection.json)
- **Progress correction (2026-09-10, normalized legacy-to-tier-one bridge rejection):** A second one-way bridge snapshotted the active memory callback bank into append-only context words, reloaded it at both compiler entries, and continued to reject tier-one-to-legacy links. Ten focused tests and the deterministic 1,558-test/179-suite aggregate passed, including poisoned-callback and exact-state mixed-compiler cases. Production nevertheless rejected the bridge twice: the same committed release runner crashed before a receipt with SIGBUS at `0x1c6`, then SIGSEGV at `0x64`; both crash reports place the failure in generated execution while the callback bank is live. The candidate is fully reverted in `2128478b9`. The low program counters prove corrupted host control flow but do not isolate a specific edge, so no performance counters are inferred and the production-safe 50.26% measurement remains authoritative. A future bridge must keep host callback pointers out of mutable guest context and pass two frozen-fixture runs. A05.3 stays open. [Receipt](docs/virtualization/evidence/a05-tier1-2026-09-10/pvh-normalized-legacy-to-tier1-chain-rejection.json)
- **Progress correction (2026-09-10, legacy-memory-to-tier-one edge rejection):** A narrower experiment admitted only legacy memory sources whose existing chain epilogue restores the canonical `x0`...`x5` entry tuple; callback-free legacy sources and all tier-one-to-legacy edges remained separated, and emitted machine code was unchanged. Three focused tests and the deterministic 1,557-test/179-suite aggregate passed. The pinned production run still corrupted RCU, PCI ECAM, and TSC console strings before userspace; busybox then faulted during exec and the kernel looped through scheduling-while-atomic warnings plus page-table locking until the one-billion-instruction budget expired after 412.188 seconds without an authenticated receipt. Failure-run avoidance reached 65.01%, but correctness failed, so that counter is rejected rather than credited. The candidate is reverted in `7b81d8238`. Restoring the callback tuple alone is therefore insufficient; a future attempt needs edge-level architectural-state tracing before any admission change. A05.3 remains at the accepted 50.26% and stays open. [Receipt](docs/virtualization/evidence/a05-tier1-2026-09-10/pvh-legacy-memory-to-tier1-chain-rejection.json)
- **Progress correction (2026-09-10, single-read legacy-to-tier-one edge rejection):** The legacy-memory experiment was narrowed again to the smallest rollback-free source class: exactly one memory read, no write, no guarded exit, and the existing canonical `x0`...`x5` chain epilogue. A positive bit-test/branch-to-ADC regression removed one warm dispatcher entry, a negative CALL/stack-writer regression retained the compiler boundary, and the deterministic aggregate passed 1,557 tests in 179 suites. Production still rejected the exact committed release binary twice before either run could serialize a receipt: first SIGBUS with `PC = LR = 0x1d3`, then SIGSEGV with `PC = LR = 0`. The candidate is reverted in `dca0a175e`; the separate ABI documentation correction remains. Static source-shape narrowing cannot establish this mixed edge's safety, so the next attempt must trace the first production edge and its complete state rather than admit another class. No performance counters are inferred, the accepted 50.26% measurement remains authoritative, and A05.3 stays open. [Receipt](docs/virtualization/evidence/a05-tier1-2026-09-10/pvh-single-read-legacy-to-tier1-chain-rejection.json)
- **Progress correction (2026-09-10, Tier-one FP/LR-only hardening rejection):** A reliability experiment moved the Tier-one frame-pointer/link-register save pair from its generated stack frame into existing context words while leaving mixed-compiler chaining disabled. Three focused ABI/host-control tests and the deterministic 1,556-test/179-suite aggregate passed, but the exact release binary crashed before producing a receipt. The crash is more diagnostic than the preceding low-PC failures: generated execution returned to the correct C caller with callee-saved `x20` corrupted to the Swift memory-write callback thunk, so the caller's exit-code store attempted a write into read-only `__TEXT` and raised SIGBUS. The candidate is reverted in `ff659f230`; protecting FP/LR alone is insufficient, and the next containment must preserve and verify the complete callee-saved bank before another mixed edge is admitted. No performance counters are inferred, the accepted 50.26% measurement remains authoritative, and A05.3 stays open. [Receipt](docs/virtualization/evidence/a05-tier1-2026-09-10/pvh-tier1-host-control-hardening-rejection.json)
- **Progress correction (2026-09-10, complete callee-saved-bank hardening rejection):** Both memory-capable baseline entries and Tier-one entries were changed to preserve all Darwin callee-saved registers in append-only context words instead of generated stack memory. A real MAP_JIT regression deliberately overwrote every qword of the Tier-one scratch frame and returned cleanly; 28 focused ABI/helper/chain/recovery tests and the deterministic 1,556-test/179-suite aggregate passed. Production did not: although the host no longer crashed, early kernel strings contained repeated NUL insertion and duplicated fragments, `/init` never authenticated, and the guest exhausted one billion instructions after 443.186 seconds while cycling through page-table, spin-lock, and scheduler paths. The candidate is reverted in `0682012f7`. This contains the observed return-register symptom without repairing the correlated guest-state corruption, so the next investigation must capture the first generated write or architectural divergence with binary-layout-neutral tracing rather than add more in-process context state. The failure run's 46.22% avoidance is rejected, the accepted 50.26% remains authoritative, and A05.3 stays open. [Receipt](docs/virtualization/evidence/a05-tier1-2026-09-10/pvh-complete-host-bank-hardening-rejection.json)
- **Progress correction (2026-09-10, native-trace batch isolation rejection):** Recorded native-trace batch replay was made independently selectable and disabled only for the production PC machine; ordinary generated calls, direct chains, IBTC, shadow returns, Tier-one admission, and mixed-compiler rejection remained unchanged. The trace-off regression and original trace-on regression both passed, as did the 36-test focused gate and deterministic 1,556-test/179-suite aggregate. One exact-binary production run recorded zero trace attempts/replays, passed all seven workloads, and powered off after 722,282,618 instructions in 345.434 seconds. The second identical-binary run nevertheless crashed before a receipt with `PC = LR = FP = 0`. The candidate is reverted in `8da92379e`: one pass cannot override a null-control-flow crash, and native batch replay is not the sole defect. The passing run's 51.07% avoidance is rejected with the candidate, the accepted 50.26% remains authoritative, and A05.3 stays open for isolation of the ordinary generated call and raw chain/predictor paths. [Receipt](docs/virtualization/evidence/a05-tier1-2026-09-10/pvh-native-trace-off-rejection.json)
- **Progress (2026-09-10, raw-target prediction reliability isolation):** Raw host-address prediction is now independently selectable per executor. The production PC machine disables direct-chain initiation and exposes neither the generated IBTC nor shadow-return storage, while standalone executor use retains all three paths by default; ordinary JIT execution, Tier-one admission, the physical block cache, native trace replay, and precise recovery remain enabled. The predictor-off regression and five existing predictor-on tests pass, as does the deterministic 1,556-test/179-suite aggregate. Two runs of the exact same hashed release runner pass all seven authenticated workloads and power off after 732,494,380 instructions in 228.779 seconds and 733,728,470 instructions in 219.399 seconds. Both receipts record zero direct patches/bypasses, IBTC activity, and shadow-stack activity, while native traces replay 377,237 and 344,251 times. Together with the preceding trace-off null-control-flow crash, this accepts a fail-closed production reliability boundary and makes one or more raw-target predictors the leading suspect, but it does not isolate a specific predictor or prove sole causality. The isolation deliberately supplies no dispatcher-avoidance credit: the accepted 50.26% enabled-path measurement remains authoritative, raw predictors must be reintroduced one at a time with two-run frozen-fixture proof, and A05.3 stays open. [Receipt](docs/virtualization/evidence/a05-tier1-2026-09-10/pvh-raw-target-prediction-isolation.json)
- **Progress correction (2026-09-12, direct-chain-only reintroduction rejected):** The combined isolation switch is now a three-way option set for direct chains, the IBTC, and shadow returns, with a directed regression proving direct-only linking and zero indirect/shadow activity. The focused machine/predictor gate passes 36 tests in two suites and the deterministic aggregate passes 1,557 tests in 179 suites. Production then enabled direct patches alone. One exact-binary run passed all seven workloads and powered off after 722,575,944 instructions in 366.354 seconds with 115,259,273 direct bypasses and 115,247,358 dispatcher entries. The second qualifying run of the identical binary emitted NUL-corrupted kernel strings, passed six workload records, faulted busybox at an inaccessible dynamic-loader address during the final workload, soft-locked in `unmap_page_range`, and exhausted one billion instructions after 367.918 seconds without an authenticated terminal receipt. Its IBTC and shadow activity remained exactly zero, so direct chaining alone is sufficient to reproduce the intermittent guest-corruption family. The production option is reverted in `825c5188c`; granular controls remain for diagnosis. Neither candidate counter is accepted, the production-safe 50.26% enabled-path measurement remains authoritative, and A05.3 stays open until the first corrupting direct edge is identified and repaired or excluded. [Receipt](docs/virtualization/evidence/a05-tier1-2026-09-12/pvh-direct-chain-only-rejection.json)
- **Progress (2026-09-12, legacy direct-chain reliability acceptance):** Direct-chain initiation is now independently selectable for legacy baseline/optimizing and Tier-one compiler ABIs. Production enables only same-tier legacy-ABI direct links; Tier-one remains active as a compiler, but cannot initiate a raw direct edge and the existing compiler-ABI gate prevents it from accepting a legacy edge. Three predictor-boundary regressions plus the PC machine suite pass 37 tests in two suites, and the deterministic aggregate passes 1,558 tests in 179 suites. Two runs of the exact same release runner pass all seven authenticated workloads and power off after 722,291,229 instructions in 290.761 seconds and 722,484,614 instructions in 292.485 seconds. They record 31,603,538 and 31,640,796 direct bypasses, 198,331,718 and 198,716,381 native dispatcher entries, more than 110,000 Tier-one compiled blocks each, and exactly zero IBTC or shadow-return activity. This accepts legacy-source direct chaining as the production reliability boundary and narrows the preceding direct-only corruption to Tier-one-source edges or an interaction those edges enable. The measured within-run avoidance is only 13.74%, so it does not replace the accepted 50.26% broader enabled-path measurement or satisfy the >95% gate. A05.3 stays open; Tier-one direct initiation must now be isolated and repaired or excluded before the remaining predictors are reintroduced independently. [Receipt](docs/virtualization/evidence/a05-tier1-2026-09-12/pvh-legacy-direct-chain-acceptance.json)
- **Progress (2026-09-12, Tier-one direct-chain reliability acceptance):** Production next enabled only same-tier Tier-one direct links, leaving legacy direct sources, the generated IBTC, and shadow returns disabled. The same 37-test focused gate and deterministic 1,558-test/179-suite aggregate pass. Two runs of the exact same release runner pass all seven authenticated workloads and power off after 733,655,970 instructions in 231.763 seconds and 732,658,671 instructions in 230.611 seconds. They record 85,363,154 and 85,315,713 Tier-one direct bypasses, 148,692,844 and 148,878,287 native dispatcher entries, 36.47% and 36.43% within-run avoidance, and exactly zero IBTC or shadow-return activity. This accepts the isolated Tier-one source class and makes it the stronger production default, but does not exonerate the combined direct-source configuration: that exact configuration previously failed one of two runs, while each source class has now passed only its own bounded two-run check. Coexistence, binary layout, timing, or another intermittent correlated defect remains possible and unproven. The accepted 50.26% broader enabled-path measurement remains the high-water mark, the >95% gate still fails, and A05.3 remains open for combination diagnosis plus independent IBTC and shadow-return reintroduction. [Receipt](docs/virtualization/evidence/a05-tier1-2026-09-12/pvh-tier1-direct-chain-acceptance.json)
- **Progress (2026-09-12, generated IBTC reliability acceptance):** The generated IBTC was then enabled alone, with both direct-source classes and shadow returns disabled. Its runtime regression now explicitly constructs an IBTC-only executor and proves one warm generated bypass, invalidation/refill, zero direct patches, and zero shadow activity; the focused gate passes 38 tests in two suites and the deterministic aggregate passes 1,558 tests in 179 suites. Two exact-binary production runs pass all seven authenticated workloads and power off after 733,963,807 instructions in 208.970 seconds and 733,597,155 instructions in 215.568 seconds. They record 2,125,312 and 2,119,641 IBTC hits, 101,161 and 101,524 fills, 13.52% and 13.49% lookup hit rates, and exactly zero direct or shadow activity. This accepts the IBTC as an isolated reliability boundary, but its 0.91% and 0.90% within-run dispatcher avoidance is weaker than Tier-one direct chaining, so production returns to the accepted Tier-one-direct-only default in `e965c424c`. A05.3 remains open for shadow-return isolation, safe predictor combinations, and the >95% gate. [Receipt](docs/virtualization/evidence/a05-tier1-2026-09-12/pvh-ibtc-only-acceptance.json)
- **Progress (2026-09-12, shadow-return reliability acceptance):** An explicit IBTC-plus-shadow regression exposed that the runtime's chain-accounting gate still depended on direct-link selection: storage existed, RET misses counted, but CALL could not publish a shadow record when both direct classes were disabled. Commit `6d5dc540c` separates generated accounting eligibility from raw link installation; shadow-enabled direct-slot blocks may now publish CALL records while the existing direct predictor gate continues to forbid patches and pending links. The focused predictor/machine gate passes 39 tests in two suites and the deterministic aggregate passes 1,558 tests in 179 suites. With IBTC plus shadow enabled and both direct classes disabled, two exact-binary runs pass all seven authenticated workloads and power off after 734,072,477 instructions in 243.228 seconds and 732,488,385 instructions in 236.890 seconds. They record 14,977,628 and 14,941,149 pushes, 1,989,688 and 1,983,247 shadow hits, 13.39% and 13.38% shadow hit rates, nonzero IBTC fallback activity, and exactly zero direct patches. Total within-run avoidance remains 0.91% in both runs, almost unchanged from IBTC-only because shadow hits replace return-path IBTC hits rather than opening a large new edge class. The functional decoupling and isolated shadow boundary are accepted, but production returns to Tier-one-direct-only in `9cc9e866a`; A05.3 remains open for safe predictor combinations and the >95% gate. [Receipt](docs/virtualization/evidence/a05-tier1-2026-09-12/pvh-shadow-return-acceptance.json)
- **Progress correction (2026-09-12, Tier-one direct plus IBTC combination rejected):** The first additive combination enabled Tier-one direct links and the generated IBTC while leaving legacy direct links and shadow returns disabled. The 39-test focused gate and deterministic 1,558-test/179-suite aggregate passed, but the first committed release run crashed before producing terminal JSON or a diagnostic receipt. The macOS report records `EXC_BAD_ACCESS`/SIGSEGV at address zero with `PC = LR = FP = 0`; generated memory callback pointers were live in the callee-saved register bank. The candidate is reverted in `1db538a3b`, restoring Tier-one-direct-only production. No workload milestone, elapsed runner total, or predictor counter is inferred from the missing receipt. Individually passing predictors therefore cannot be assumed safe in combination; A05.3 remains at the accepted 50.26% high-water measurement and stays open for out-of-process or binary-layout-neutral first-divergence evidence before another combination is admitted. [Receipt](docs/virtualization/evidence/a05-tier1-2026-09-12/pvh-tier1-direct-ibtc-rejection.json)
...
- **Progress (2026-09-10, fault/latency acceptance evidence):** A dedicated restartability matrix now executes RMW read faults, RMW write faults, a cross-page RMW read, and an MMIO restartable-read decline through both baseline and optimizing executors. All eight combinations retire only the safe prefix, preserve the uncommitted faulting state and memory, and leave the exact retry RIP. Pending-work diagnostics now count native observations and retain the largest conservative in-entry retirement count; the executor, PC aggregate, UEFI smoke JSON, and PVH receipt all expose the same fields. The focused pending/serialization gate passes 10 tests in four suites, and the full DBT/PC/runner gate passes 1,545 tests in 179 suites. A release runner reaches authenticated userspace, passes all seven workloads, and powers off after 732,847,534 instructions in 234.878 seconds. Its one native pending-work exit retired zero guest instructions before returning to the dispatcher; the structural worst case remains one natural block capped at 4,096 instructions. This satisfies the named fault-matrix and bounded interrupt-observation evidence, but A05.4 remains open for production-safe precise Tier-one masks and inline-TLB prefix recovery; A05.3 also remains below its >95% dispatcher gate. [Receipt](docs/virtualization/evidence/a05-tier1-2026-09-10/pvh-fault-latency-acceptance.json)
- **Progress correction (2026-09-10, exact-mask reliability retry):** Exact Tier-one live-in/dirty side-table masks were retried with the complete usage analysis forced out of the already-large emitter body. The focused 80-test metadata/Tier-one gate passed and the 1,545-test/179-suite aggregate passed twice after one non-reproducing concurrent-suite issue. Production reliability did not pass: one identical pinned run crashed after about 208.8 seconds with an unaligned generated return (`PC = LR = 0x6e`) while a second run passed all seven workloads after 733,795,557 instructions in 283.316 seconds. Because these mask values do not alter emitted `machineWords`, the crash is evidence of a layout-sensitive native chain/trace/predictor or memory-prologue lifecycle defect rather than proof of bad dataflow values. A one-of-two result is unacceptable, so the candidate was reverted and masks remain conservative. A05.4 stays open, and the invalid generated-return path is now the prerequisite reliability investigation. [Receipt](docs/virtualization/evidence/a05-tier1-2026-09-10/pvh-exact-mask-reliability-failure.json)
- **Progress (2026-09-10, generated host-control-state hardening):** Memory-capable generated blocks no longer restore FP/LR from the same stack frame that holds generated memory temporaries. Two append-only context words now capture the incoming host control state at each entry and supply it at the epilogue; the existing context layout remains stable through `pendingWork` at word 71. An encoding regression excludes both old stack-pair forms, while real helper and fault-recovery coverage exercises the new frame. The focused ABI/emitter/metadata gate passes 19 tests in three suites and the full DBT/PC/runner gate passes 1,546 tests in 179 suites. Two independent release runs cross the prior ~208.8-second crash window and two code-cache wraps, pass all seven authenticated workloads, and power off cleanly after 733,524,732 instructions in 239.266 seconds and 733,625,999 instructions in 237.145 seconds. This accepts containment of the invalid generated-return path but does not prove the original overwrite source. Tier-one masks remain conservative until the exact-mask candidate separately passes the same production gate, so A05.4 remains open. [Receipt](docs/virtualization/evidence/a05-tier1-2026-09-10/pvh-host-control-state-hardening.json)
- **Progress (2026-09-10, hardened exact-mask acceptance):** Exact Tier-one live-in and cumulative-dirty GPR masks are re-enabled on top of the context-backed host-control frame; only opaque helpers retain the all-register sentinel. The focused metadata/Tier-one gate passes 80 tests. Two parallel aggregate attempts exposed different pre-existing timing tests—pending-work coordination and cross-view atomic contention—and each passed immediately in isolation; the deterministic non-parallel aggregate passes all 1,546 tests in 179 suites. Two independent exact-mask release runs cross the former ~208.8-second crash window and four aggregate code-cache wraps, pass all seven authenticated workloads, and power off cleanly after 732,226,271 instructions in 283.474 seconds and 733,949,993 instructions in 248.700 seconds. This overturns the prior one-of-two exact-mask rejection without claiming that the original overwrite mechanism is proven. Baseline inline-TLB faults still use whole-block rollback and interpreter replay, so production-safe precise-prefix recovery remains before A05.4 can close. [Receipt](docs/virtualization/evidence/a05-tier1-2026-09-10/pvh-hardened-exact-mask-acceptance.json)
- **Progress (2026-09-10, inline-TLB read-fault recovery acceptance):** Generated inline-TLB read misses now publish their resolver BLR return PC only after the architectural page-table walker returns a fault, allowing the existing side table to restore the completed instruction prefix and retry the exact load. Applying the same mechanism to write and atomic resolver faults failed the pinned production gate deterministically: init executed from an invalid user address, raised an NX fault, panicked, and reset after 634,049,219 instructions in 230.094 seconds. Those two fault classes therefore remain on whole-block rollback and interpreter replay, with a MAP_JIT regression proving they expose neither a recovery PC nor speculative context. The narrowed read-only build passes 21 focused tests and the deterministic 1,548-test/179-suite aggregate. Two independent release runs cross the rejected candidate's failure boundary and four aggregate code-cache wraps, pass all seven authenticated workloads, and power off cleanly after 732,842,779 instructions in 254.671 seconds and 733,290,719 instructions in 253.915 seconds. A05.4 remains open for an instruction-entry checkpoint or equivalent production-safe write/atomic prefix mechanism; A05.3 remains below its >95% dispatcher gate. [Receipt](docs/virtualization/evidence/a05-tier1-2026-09-10/pvh-inline-tlb-read-recovery.json)
- **Progress correction (2026-09-10, write-checkpoint rejection):** The proposed instruction-entry checkpoint for inline write/atomic recovery was implemented and rejected. Baseline write instructions saved the exact IR-derived GPR write set plus RFLAGS before any statement; a read-succeeds/write-faults RMW regression proved that local state and memory rolled back exactly, and the 1,549-test/179-suite aggregate passed. Production did not: the all-class candidate and a write-only isolation both reproduced the init NX panic and guest reset near 634.3 million instructions, while an atomic-only isolation terminated the host runner with signal exit 138 before writing a receipt. The candidate is reverted; only the independently proven read-fault prefix path remains enabled, while write and atomic resolver faults retain whole-block rollback. A05.4 stays open, and the next attempt must capture the exact production fault site and non-context side effects rather than widen the checkpoint speculatively. [Receipt](docs/virtualization/evidence/a05-tier1-2026-09-10/pvh-inline-tlb-write-checkpoint-rejection.json)
- **Progress (2026-09-10, terminator-safe write recovery acceptance):** Generated-fault tracing isolated the deterministic init NX panic to musl `_Fork` at image offset `0x5c650`: a direct `CALL` decremented RSP before its return-address push faulted, but the rejected statement-only checkpoint did not cover the terminator, so interpreter replay decremented RSP again and eventually returned through an invalid executable address. Baseline instruction-entry checkpoints now cover exact IR-derived register writes plus RFLAGS for write-bearing statements and pre-push RSP/RFLAGS for direct and indirect `CALL` terminators; successful writes clear the active checkpoint, while a write-resolver page fault restores it, publishes the completed prefix, and immediately interpreter-deoptimizes only the faulting instruction with separate native/interpreter retirement accounting. Exact `clear_page_orig`, musl loader, RMW, and `_Fork` CALL-shape regressions pass in the 59-test focused gate, and the deterministic aggregate passes 1,552 tests in 179 suites. Two clean runs of the committed release runner cross the former reset boundary, pass all seven authenticated workloads, and power off after 733,277,497 instructions in 243.169 seconds and 733,142,657 instructions in 236.863 seconds. This accepts inline write-fault prefix recovery. Atomic resolver faults remain conservatively on whole-block rollback pending their own isolated diagnosis and two-run production proof, so A05.4 stays open; A05.3 also remains below its >95% dispatcher gate. [Receipt](docs/virtualization/evidence/a05-tier1-2026-09-10/pvh-inline-tlb-write-terminator-checkpoint-acceptance.json)
- **Progress (2026-09-10, compare-exchange recovery acceptance):** The generated direct compare-exchange helper now publishes its BLR continuation PC only for its typed architectural page-fault result; fallback and error results remain unmarked. A real MAP_JIT absent-page regression binds that PC to the exact atomic instruction and proves the active checkpoint preserves entry RFLAGS and RAX. The 59-test focused gate and deterministic 1,552-test/179-suite aggregate pass. Two independent runs of the same hashed release runner cross the earlier atomic-isolation host-signal and userspace-reset boundaries, pass all seven authenticated workloads, and power off after 733,131,164 instructions in 245.315 seconds and 733,590,737 instructions in 231.414 seconds. This accepts precise recovery for generated compare-exchange page faults, but not yet exchange, fetch-add, general locked RMW/bit-test, or pair compare-exchange helpers; those retain conservative exits pending equivalent typed-status coverage and production proof, so A05.4 stays open. [Receipt](docs/virtualization/evidence/a05-tier1-2026-09-10/pvh-inline-tlb-compare-exchange-recovery.json)
- **Progress (2026-09-10, complete typed atomic recovery and A05.4 closure):** A shared generated status decoder now extends page-fault-only recovery-PC publication to exchange, fetch-add, general locked binary and unary RMW, bit-test RMW, and pair compare-exchange; successful resolution continues in generated code, while fallback and error results fail closed to interpreter execution without recovery metadata. Together with the separately accepted direct compare-exchange path, the real MAP_JIT matrix covers every generated atomic family plus ordinary writes, and a cross-page fallback regression proves that a non-page-fault outcome exposes neither a recovery PC nor context. The focused gate passes 60 tests and the deterministic aggregate passes 1,553 tests in 179 suites. Two independent runs of the same hashed release runner pass all seven authenticated workloads and power off after 732,389,204 instructions in 255.904 seconds and 733,541,273 instructions in 239.730 seconds. Combined with the previously accepted exact masks, read/write prefix recovery, restartability matrix, and bounded pending-work observation, this satisfies the A05.4 action and check. A05.4 is closed; A05.3's >95% dispatcher-entry gate and A05.5 remain open. [Receipt](docs/virtualization/evidence/a05-tier1-2026-09-10/pvh-inline-tlb-atomic-recovery-acceptance.json)
