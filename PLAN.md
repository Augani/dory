...
- **Progress (2026-09-09):** Long-mode 32/64-bit memory `CMPXCHG`, including the accepted `LOCK` form, implicit-lock 32/64-bit memory `XCHG`, and 32/64-bit memory `XADD` including its `LOCK` form now resolve through the per-vCPU write TLB into C runtime host-address atomics. Naturally aligned RAM uses compiler-lowered sequentially consistent C11 compare-exchange/exchange/fetch-add operations; page faults return to the precise interpreter boundary, while unaligned, cross-page, MMIO, and tracked page-table operands decline without effects to the existing serialized fallback. The interpreter and native fallback share one C-owned split-lock mutex, preserving single-copy behavior during incremental conversion. Success/mismatch, swap, returned-old-value, and flags parity; cached-translation reuse; unaligned fallback; fault rollback; flags preservation; and cross-vCPU serialization pass. The complete DBT/PC gate passes (1,348 tests, 167 suites). Remaining locked arithmetic/logical/unary RMW forms, `CMPXCHG8B/16B`, and the complete litmus matrix remain before closure.
