...
This was a source and integration review, not a fresh physical guest qualification campaign. File locations and line numbers describe the reviewed working tree and will move as implementation changes. Full paths in this document are relative to the Dory repository root. Repeated module names use these abbreviations: `DoryDBTX86/`, `DoryOperations/`, `DorydKit/`, `DoryVZMacCore/`, `DoryVMMKit/` and the other core Swift modules resolve under `dory-core-swift/Sources/`; `DoryHV/`, `dory-hv/` and renderer/filesystem worker modules resolve under `Packages/ContainerizationEngine/Sources/`. App paths begin with `Dory/`.

### 2.1 Keep this investment

| Area | Existing implementation | What that establishes | What remains unproven or incomplete |
|---|---|---|---|
| Native Linux runtime | `Packages/ContainerizationEngine/Sources/DoryHV/Machine.swift`, `Sources/dory-hv/EngineMode.swift`, `DesktopMode.swift` | A substantial actual raw-HV Linux runtime, per-vCPU host execution, devices, desktop and host integration | General distro install coverage, consolidation with new execution contracts, complete release performance and recovery matrix |
| New CPU contracts | `dory-core-swift/Sources/DoryExecutionContracts/`, `DoryNativeHVArm64/` | Typed CPU, memory, state and execution interfaces; a native implementation exercised by probes | The new native engine is not the production Linux machine owner; deadline handling explicitly throws; dirty-page tracking is conservative |
| x86 execution | `dory-core-swift/Sources/DoryDBTX86/` | Large decoder/interpreter, paging, architectural state, baseline JIT, IR and differential/replay foundations | Complete semantics, credible CPU feature contract, JIT hot-path coverage, memory-order correctness and actual parallel vCPU execution |
| PC machine | `dory-core-swift/Sources/DoryMachinePC/`, `DoryFirmware/`, `Packages/ContainerizationEngine/Sources/dory-hv/DoryPCMode.swift` | PC memory map, ACPI/APIC/timers/PCI, virtio transports, USB, direct-kernel and UEFI scaffolding/runtime | Production Linux install-to-desktop acceptance, precise clocks/interrupts, fully connected accelerated GPU |
| ARM machine and firmware | `DoryMachineARMVirt/`, `Firmware/DoryARMVirt/`, `DoryHV/Machine.swift`, `dory-hv/ARMVirtUEFIRuntimeAuthority.swift` | ARM machine ABI, device-tree generation, EDK II build inputs and boot/runtime integration | A tested general installer contract across exact stock media and resource classes; reproducible signed firmware qualification |
| Linux graphics | `DoryHV/VirtioGPU.swift`, renderer worker modules, `Config/DoryRendererProductionTuple.json`, `guest/mesa/`, kernel patches | Real ARM raw-HV dual VirGL2/ANGLE/Metal and Venus/MoltenVK/Metal implementation; isolated worker and shared-resource contracts | x86 transport integration, stock-kernel compatibility, guest x86 packages, full guest/application evidence and release qualification |
| Portable virtio | `dory-core-swift/Sources/DoryVirtio/` | Reusable block, network, entropy, input, sound and partial GPU cores | Semantic parity with older raw-HV devices, complete GPU blob/fence/cursor support, one implementation per device |
| macOS ARM64 | `DoryVZMacCore/`, `DoryVMMKit/DoryVZMacAdapter.swift`, `DoryVZMacDesktopApplication.swift`, daemon/AppStore creation flow | A real product path through prepare, IPSW install, run and resume; Apple graphics view and platform persistence | End-to-end lifecycle correctness, saved-state path agreement, policy propagation, physical install/update/recovery/Metal evidence |
| Control plane | `DoryOperations/`, `DorydKit/MachineManager.swift`, production plan resolver/activation modules | Typed definitions, capabilities, launch plans, leases, journals, component verification, admission and diagnostics | One authoritative runtime plan consumed by every launch path; fewer duplicated representations and monolithic coordinators |
| Sharing and network | `DoryHV/VirtioFS.swift`, `DoryFSWorkerServiceCore/`, daemon networking, Rust dataplane/agent | Useful isolated file service, path authority, guest communication and network integration | Coherence and protocol completeness across both guest ISAs; measured copy/batching cost; consolidation of legacy network assumptions |
| Delivery and tests | `scripts/`, `.github/workflows/`, package and app tests, signed-component machinery | Significant packaging, safety checks, source/artifact gates and test infrastructure | Fewer source-text assertions; reproducible behavior tests and candidate-bound physical results |

### 2.2 Findings that must lead implementation

| ID | Finding and source evidence | Consequence | Required disposition |
|---|---|---|---|
| F01 | New `NativeHVArm64Engine.run(...until:)` rejects non-nil deadlines (`NativeHVArm64Engine.swift:259–261`); the implementation is used by smoke/calibration products rather than the production Linux runner | A new abstraction layer exists without replacing the established execution path | Adapt the actual runtime incrementally; keep one production execution owner; implement or explicitly narrow the deadline contract |
| F02 | `DoryPCDirectKernelMachine.run` serializes execution under a machine lock, with short round-robin slices (`:673–790`, `:927+`) | Multiple guest CPUs are currently a correctness model, not concurrent translated throughput | Qualify UP first; design real vCPU concurrency, shared memory and interrupt ordering before claiming SMP performance |
| F03 | Dirty PVH work adds broad Xen identity/hypercall behavior (`DoryX86CPUProfile.swift:155–176`, `DoryX86Interpreter.swift:4096+`) including successful unknown calls and a fixed 16 GiB response | Guest behavior can depend on falsely advertised facilities and incorrect memory topology | Replace workarounds with a narrowly defined boot/platform ABI; remove blanket success and hardcoded machine facts |
| F04 | Dirty ELF loading masks physical addresses and loads allocated sections outside program segments (`DoryPCPVHKernel.swift`) | A particular kernel progresses, but this is not a general safe ELF/PVH loader | Validate the actual boot ABI and kernel layout; remove unexplained relocation constants and section-loading assumptions |
| F05 | Kernel-specific addresses, trace output and workarounds appear in `DoryPCDirectKernelMachine.swift`; the real-kernel test relies on a local `/tmp` image and does not assert successful userspace boot | Diagnostic activity can look like broad compatibility while being unreproducible | Isolate diagnostics; pin test fixtures; assert checkpoints, timeouts and failures |
| F06 | `DoryPCMode.swift:297–302` rejects accelerated graphics | x86 Linux cannot currently inherit ARM GPU support | Wire signed worker authority, true GPU fences, retirement and presentation for VirGL2; require blob BAR/mappings and compatible x86 Venus runtime before opening Vulkan |
| F07 | `DoryVirtioGPU.swift` lacks the complete blob lifecycle; fence responses echo headers without a demonstrated GPU-completion contract; cursor commands do not publish a host cursor | The reusable GPU is not equivalent to the mature raw-HV GPU | Port proven behavior into one core; preserve asynchronous completion and reset generations |
| F08 | Guest Mesa/desktop build inputs currently target ARM64; production bootstrap expects one exact dual-capset tuple and a patched managed-kernel fence contract | Stock ISO and x86 acceleration are not automatically supported | Separate host renderer identity, guest architecture, capability profile and synchronization proof |
| F09 | Successful macOS install publishes `.stopped` (`DoryVZMacAdapter.swift:127`); the desktop observer calls `finish()` on `.stopped` (`DoryVZMacDesktopApplication.swift:360`) before the install task's intended `start()` (`:308`) | The install-to-first-boot flow can stop the app run loop prematurely | Separate install completion from terminal VM shutdown; test the real event ordering |
| F10 | Daemon save paths are under `saved-state-v1/state.tmp-*` (`DoryMachineSavedState.swift:116–122,273–274`); the Mac runner accepts only a parent equal to `--state-dir` (`DoryVZMacDesktopApplication.swift:559–565`) | Managed macOS suspend is rejected by incompatible path contracts | Use one descriptor/path authority and saved-state root throughout daemon and runner |
| F11 | Mac managed arguments chiefly pass identity/bundle/IPSW, while VZ configuration installs audio, clipboard, NAT and other defaults | Requested device/privacy policy can diverge from actual configuration | Carry validated device/network/share policy into the final VZ configuration and effective capabilities |
| F12 | Raw-HV host-share metadata caching is deliberately disabled (`VirtioFS.swift:116,342–346`); the PC share is non-DAX and lacks the notification extension | Old descriptions of coherent caching or universal sharing performance overstate current behavior | Preserve the correct zero-TTL baseline; implement missing coherence before optimizing |
| F13 | At audit start, readiness tests required the phrase “packaged QEMU TCG backend” in documentation and the Phase 0A checker required fixed headings; those document-only checks are now removed | Tests encode an obsolete plan rather than current runtime behavior | Cleanup completed for document-only checks; add structured support/launch/qualification coverage and retire remaining obsolete source-text policy checks |
| F14 | Monolithic machine/desktop/manager files coexist with additional planning/authority layers and duplicate device implementations | Changes require too many representations to stay synchronized | Delete redundant paths after parity tests; split by concrete ownership, not generic framework layers |

F03–F05 are work in progress already present before this review. Preserve the developer's changes now; review and replace them deliberately in the implementation phases. A suspicious implementation is not permission to discard unrelated uncommitted work.

### 2.3 What the previous evidence does and does not show

- The retained ARM HV throughput receipt `docs/virtualization/phase-0a-hv-throughput-engineering-evidence-schema2-2026-08-30.json` reports approximately **95.99%** of host-native throughput and **99.61%** of a minimal HV harness for its small integer loop on one M2 Pro. That is useful local engineering evidence, not a Linux/macOS whole-system result.
- The JIT probe receipt reports 100 successful runs on one host, while explicitly leaving notarization and the host matrix open. It does not prove translator correctness or product performance.
- The retained VZMac restore-image receipt says `SIGNED_METADATA_DISCOVERY_PASSED_INSTALL_AND_GUEST_QUALIFICATION_PENDING`; it did not download or install the IPSW. Do not count it as a macOS boot result.
- The deleted narrative ledger reported a Linux x86 PVH run reaching `start_kernel()` and early CPU initialization output after more than 50 million instructions. Preserve that milestone as **early kernel bring-up**, not completed boot, userspace, SMP, installer or desktop support.
- The old graphics narrative described a roughly 15-minute ARM GNOME run, a short VirGL workload and brief Venus/Zed use on one development tuple. This review did not independently verify the underlying physical result artifacts. Treat the report as historical engineering context until reproduced on the exact candidate.
- `Firmware/DoryARMVirt/qualification/*.json` includes scenario definitions with expected console interactions. These are test inputs, not proof that a campaign completed.
- Decoder coverage over binaries is useful gap discovery. It does not establish instruction semantics, feature-mask correctness, executed-path coverage, or ability to run those applications. Correct the old classification of SHLX/SHRX/SARX as BMI1: they belong to BMI2.

### 2.4 Review verification boundary

The review ran bounded existing offline checks. Before document cleanup, the no-QEMU gate tests, readiness tests and container-performance gate tests passed; the Phase 0A documentation test failed because its ADR-015 heading no longer matched its own parser. The consolidation removes the obsolete fixed-heading checker and its programme records; it does not change runtime behavior. After cleanup, the existing readiness (5), container-performance (3), CI umbrella (2), and release-orchestrator (4) tests passed. The relocated ABI files were verified byte-for-byte; no fresh Swift runtime test pass is inferred from that check.

A focused `swift test --package-path dory-core-swift --skip-build --filter DoryPCACPI` attempt could not execute tests because the cached test products referenced missing/incompatible executables. No Swift runtime test pass, fresh app build, physical guest boot, GPU benchmark, or notarized release qualification is claimed by this review. Establish a clean reproducible baseline in P00.
...
These are **planned source changes**, not deletions made during this review. For each removal, first identify call sites, persistent formats and replacement behavior; then remove the old path in the same integration change or an immediately following one. Do not keep indefinite fallback implementations.
...
| Early / R11 | Unused `DoryPCVirGLRendererAuthority` composition | Wire it correctly or replace it with the shared authority; do not leave a second unused renderer facade | PC acceleration gate passes with actual GPU completion semantics |
...
- [ ] **P01-04** Trace the current planners and authorities. Merge duplicate validation of the same immutable facts; retain revalidation only at a real trust boundary or mutable-resource handoff. Delete wrappers that merely rename the same value.
...
- [ ] **P01-08** Consolidate journals: one durable operation ID, idempotency key, expected state/version, acquired resources, stage and compensation record. Keep typed operation payloads; avoid generic dictionaries and duplicate lifecycle stores.
- [ ] **P01-09** Make cancellation and daemon/runner death release the same leases exactly once. Reconnect after daemon restart by matching process identity and machine generation, not PID alone.
...
- [ ] **P01-12** Add public-operation tests for every supported/unsupported combination, component absence, malformed media, stale plan, altered artifact, concurrent start, cancellation and restart. Assert no filesystem/network mutation on rejected preflight.
- [ ] **P01-13** Remove duplicate launch construction after every app, CLI, restore, clone and recovery entrypoint consumes the unified plan. Add an architecture test on allowed module dependencies, not on arbitrary source-code wording.
...
**Review evidence — 2026-09-04:** P01 remains open. The current engineering campaign uses commit `bdb49cf8fadcd60b90e906740abc5ab1d83b5db7` plus the preserved working changes. The [machine-readable receipt](docs/virtualization/p01-control-plane-review-2026-09-03.json) retains commands, source identities, results and exclusions. Passing control-plane tests do not qualify physical guest boot, graphics or release performance.

| Item | Current implementation and evidence | Remaining completion gate |
|---|---|---|
| P01-01 | The executable three-cell table rejects unsupported hosts/guests before acquisition or workspace mutation. The lower capability evaluator also rejects historical Windows/QEMU configurations; its obsolete firmware/driver selection code is removed. | Keep the same policy at every new public entrypoint. |
| P01-02 | Schema-6 plans separate requested/detected/host architecture, CPU profile, machine ABI and execution tier. Current-schema decoding cannot manufacture missing authority. Real ARM/x86 ISO and installed-disk inspection inputs are now exercised. | Physical runtime qualification remains in later phases. |
| P01-03, P01-05 | Plans bind verified media, selected firmware manifests, persistence roots, exact guest/overhead resources, devices, capabilities and ABI/schema identities. Linux runtime envelopes and Mac managed arguments preserve these commitments. Mutation tests reject altered bytes, replaced directories, missing authority and resource drift before spawn. | Complete original-identity app launch and the remaining public-entrypoint campaign before closing the phase exit. |
| P01-04 | Shared product preflight, runtime state type and package-dependency checks replace redundant representations. Start and restart share exact-plan preparation. Restart and installer transitions validate a live source before stopping and obtain fresh authority for replacement launch; installer transitions also share configuration preparation with ordinary updates. Launch validation reads workspace, artifact, runtime-identity and admission authority without repairing it. Public create/start/update/restore/clone route through production planning; normal daemon fallback cannot launch a legacy workspace without planning. | Finish the retained-compatibility call-site/removal inventory and the compound-operation call graph. |
| P01-06 | Required acceleration remains strict. A lower graphics level needs an explicit recovery request and durable authorization. Renderer failure remains visible. | The later recovery UI must consume this contract. |
| P01-07 | One state vocabulary and operation phase project to daemon/app/CLI. Process, VM, guest boot, tools, desktop and workload readiness remain distinct. Raw Linux pause now parks guest execution while keeping helper control responsive. Adoption clears old tools/desktop/workload readiness; a fresh compatible agent handshake restores tools readiness, and resume triggers this check for paused guests. | Closed by the shared state contract and 86 isolated app projection/client tests, including response-generation races. Physical observation qualification belongs to later runtime phases. |
| P01-08 | Start preparation no longer creates a second journal. Planning/install stages use the caller's durable UUID. Restart has one typed durable operation, source launch identity, unchanged exact plan and retained admission across stop/relaunch/readiness. Its preflight token cannot authorize spawn; launch obtains a new token after stop. App/CLI/XPC carry the caller UUID; replay cannot launch again. SIGKILL tests cover both source power states at three handoff boundaries. Regular configuration updates bind the caller UUID, exact private recovery input, stop, desired-state publication and replacement plan in one root operation. Production installer attach/eject now extends that root through stopped firmware capture, replacement planning, first boot and durable compensation. | Finish desktop-update composition, direct compatibility callers, cancellation, and real process-death coverage for configuration/installer updates. |
| P01-09 | Helpers receive a private inherited reconnect descriptor. Challenge authentication binds the secret, plan revision/hash, operation, exact PID/start-time generation and current runtime state. A SIGKILL campaign now kills the owning manager process, adopts its surviving authenticated helper from the durable plan repository, preserves running/paused state, resumes/stops without respawning, and removes its reconnect record. Fresh agent metadata and network discovery are verified with an injected agent; incompatible protocol leaves tools readiness false. | Finish cooperative cancellation for compound operations and prove the same leases settle exactly once. Physical guest channels and native/DBT workload-pause qualification remain later-phase gates. |
| P01-10 | Guest RAM, DBT cache, renderer buffers/overhead, filesystem workers, disk staging and the container engine ceiling are admitted together. The exact budget reaches the relevant runners and launch envelopes. Admission preflight preserves ledger bytes, and staging cleanup preserves the lock inode used for cross-process exclusion. | Retain runtime enforcement/pressure evidence when later worker and performance changes land. |
| P01-11 | Old backend/distro/disk identity and saved-state formats are validated before migration; schema-5 plans require replanning and retain original bytes. Start/restore/clone planning failures preserve originals or remove only the newly created clone. | Closed by 105 migration/saved-state tests, including upgraded-account public operations. Later physical-guest qualification is separate. |
| P01-12 | Current campaigns cover real pinned ARM/x86 media, exact arguments, no-spawn rejection, mutation, concurrent starts, cancellation, authenticated handoff and lifecycle crash recovery. Full package, app and Rust results are retained in the receipt, with app test-host identity isolated explicitly. | Finish restore/create/clone/snapshot interruption and compound cancellation tests on the final combined source. Original-identity app launch and physical cells remain later qualification work; the optional x86 PVH fixture is explicitly absent. |
| P01-13 | Normal per-workspace start requires production planning. App/CLI create, start, update, snapshot restore and clone reach the same daemon authority; the architecture test reads SwiftPM's resolved dependency graph. | Remove the remaining compatibility construction after the compound-operation and migration campaigns prove parity. |

Focused parallel review has additionally verified 86 app state/client tests, 105 migration and saved-state tests, 98 artifact streaming tests, and 70 planning preflight/replacement tests on their individually recorded frozen snapshots. The memory regressions fail against the original unbounded loops and pass after per-chunk release. These counts overlap earlier campaigns and are not a combined-current-tree total. P01-07 and P01-11 meet their listed contracts; desktop recovery, restore/create/clone/snapshot roots, cancellation and production compatibility removal remain active work. Logs and source manifests are retained in the receipt’s `focusedReviewCampaigns` entry.

The host remains arm64 macOS 27.0 (26A5425a), with Xcode 26.6 (17F109), SDK 26.5 and Swift 6.3.3. This is a beta-host engineering cell, separate from supported-host release qualification. Real-media tests use the ARM/x86 Ubuntu 24.04.4 server ISOs and an ARM64 Noble cloud disk with root device `/dev/vda1`; hashes and reproduction variables are retained in the receipt.

The broader plan campaign also repaired the missing Rust UniFFI generator entrypoint and a formatting failure. P02's former hardcoded local-kernel test is replaced with explicit kernel/initrd paths and SHA-256 identities, bounded execution, a guest JSON readiness/workload receipt and clean poweroff assertions. `DORY_REQUIRE_X86_PVH_BOOT=1` makes missing fixtures fail; optional local absence is an explicit skip. Provisioning and booting pinned fixtures is still required before P02-07/P02-22 can close.

`MachineManager.restart` now journals one `.restarting` operation across the stop/start window. Its contract binds the source helper's launch operation, exact source/target plan, caller UUID and admission lease. Recovery distinguishes an authenticated source which survived before stop, a stopped source with no replacement, and a ready replacement; it respectively retains the source, releases execution capacity and permits retry, or completes the same operation. Six process-level SIGKILL cases exercise these outcomes for running and paused sources with the production admission ledger and sparse fixture disks. The helpers implement the control protocol; this evidence does not qualify physical guests. Runtime-directory validation also no longer depends on Foundation changing `/private/tmp` spelling once the directory exists.

Restart preflight now rejects missing/stale plans, changed metadata or artifacts, missing final authorization and plan substitution while preserving a running or paused source. Twenty-four parameterized rejection cases compare PID, power/readiness state, operation count and regular-file bytes, identities and modification times. Two late-projection cases prove start/restart validation cannot recreate removed workspace authority. A stop-boundary media mutation rejects the replacement without spawning or creating a second operation. Production trust tests distinguish running admission for restart preflight from starting admission for launch, and exercise both restart power states through the activated production graph with protocol fixtures.

Read-only admission checks no longer persist expiry recovery or clean staging files. The campaign also found that staging cleanup matched `.resource-admissions.lock` and unlinked it while held; cleanup now excludes that file, with a stable-inode regression test. Restart takes the existing workspace mutation lock without rewriting its owner hint or permissions, and still excludes competing owners. Artifact resolution cannot create missing authority roots/locks, and launch validation cannot repair a partial runtime-identity publication. Explicit recovery retains those responsibilities.

Regular configuration updates from App/CLI/XPC now keep one caller UUID through the stop, desired-state publication and replacement planning. Their private content-addressed recovery input binds the requested settings, exact source/target metadata, native definition revision and source runtime identity. The root operation completes only after the replacement plan matches the published target. Created, stopped, running and paused sources are covered; replay neither republishes a plan nor stops another helper, and UUID reuse with different settings is rejected. Missing planning infrastructure and malformed UUIDs fail before stopping a helper. Stopping a never-started source also retires its unused starting reservation before replacement admission.

Ten injected interruption cases cover before stop, after confirmed stop, after machine metadata, after workspace publication and before the final journal write, for both resource changes and native-definition-only changes. Fresh production activation finishes partial native publication and replacement planning from the same journal. If only the stop committed, it retains the original settings and reports an interrupted operation without claiming that the original power state was restored. Terminal planning journals are validated as history during startup rather than replayed against newer desired state. Resource rejection remains recoverable; a durable unbound abort allows another update. Digest/tamper and exact-lock tests protect the private recovery input. These are control-plane/helper fixture tests, not new physical-guest or SIGKILL results.

Installer attach/eject preflight now validates configuration, firmware and exact live-plan authority before changing power state. Eject no longer opens a separate resume operation for a paused source. The transition keeps its workspace lock through publication, planning and rollback; production preflight only opens an existing lock, while an accepted first compatibility mutation may initialize one. Eighteen running/paused rejection cases cover missing planning infrastructure, firmware, plan, workspace or installer media, corrupt plan bytes, pending firmware promotion and a competing workspace lock. They compare the source PID, state, readiness, runtime identity, operation IDs and file metadata; authority files are fully hashed and sparse guest disks are sampled at both ends. Two successful eject cases and the existing first-boot/rollback tests retain behavior. These are production-composition helper fixtures, not physical EFI guest results.

The latest package suites pass 3,382 tests with one explicit optional PVH skip; the full isolated app suite passes 958 tests in 110 suites, for 4,340 passing tests overall. The original app build and deep/strict signature check also pass. These latest results bind the same recorded source snapshot; the receipt retains earlier snapshots and intermediate failures separately. The full app campaign also replaced two fragile build tests with parsed project settings and actual helper-pruning behavior in a temporary directory.

The app campaign uses a copied Debug test host with a separate bundle identifier so the installed app can remain running. The DoryTests bundle and embedded dependencies are checked byte-for-byte against the built app; the host executable's code/data payload is compared separately from its changed bundle identity and signature metadata. This isolates app code tests from original-identity launch and release-signing claims. A reproduced Swift Testing optional-number assertion failure now uses a typed, unwrapped comparison and checks the XPC value too. App disk-size conversion also uses checked multiplication: negative and overflowing explicit sizes remain invalid for daemon validation instead of being omitted or crashing the app. EFI and macOS cases cover absence, valid sizes, zero, negative input and overflow boundaries.

Production `transitionInstallerMedia` now owns one durable caller operation through stop, firmware capture, media publication, replacement planning and first boot. App/CLI/XPC carry the same UUID; completed replay preserves the helper and plan, and conflicting intent cannot reuse it. The journal records exact source/target settings, the source runtime generation, a content-addressed stopped firmware checkpoint and each replacement plan. A future target requirement expresses the desired configuration and ABI but cannot authorize a launch; the ordinary exact-plan reservation, pre-spawn grant and runner verification remain mandatory.

Rollback restores the original firmware and metadata, publishes a new native definition revision, and restores the original running/paused state under the same operation. Recovery checks the journal-owned helper and plan before stopping or accepting it. Fresh production activation reconstructs operation progress from the locked journal, and replacement launch occurs only after the verified trust floor activates. Two recovery defects found by the tests—missing parent launch ID and missing in-memory operation projection—are fixed without relaxing the spawn reservation guard.

The installer campaign covers three successful attachment power states, two successful ejections, ten forward-publication interruptions, two pre-stop source-preservation cases, two live first-boot completion replays, and twelve rollback cases. Rollback interruptions include both partial metadata publication and completed source publication; assertions check firmware, exact source metadata, native revision, media, power, operation count and admission. Fresh factory recovery runs after the source helper has stopped; live completion replay uses the same manager. These injected-interruption/helper fixtures are not new daemon SIGKILL or physical EFI boot evidence. An interruption after stop but before target publication retains the source settings and reports the stopped outcome without claiming restoration of the former live power state.

Desktop update now validates its native source before resolving or staging component artifacts. Eighteen production preflight cases cover created, stopped and running sources against missing plan/workspace/kernel authority, a competing workspace lock and a zero operation UUID; valid-source probes reach the resolver without mutation. Stopped preflight checks prospective execution capacity without granting a lease or writing the ledger, and its single-use token cannot authorize spawn. Distribution identity comes from typed native settings, and artifact resolution uses the persisted guest architecture instead of a changed daemon default.

These cases exposed two native launch defects. Spawn reservation now compares the exact persisted source configuration while carrying the validated typed runtime projection separately into launch. Software graphics selection now recomputes renderer and worker overhead from the final device intent during create, update and snapshot restore; guest CPU, RAM, disk and staging capacity are retained. Native updates also preserve network mode, port forwards and camera intent. Two typed-settings round trips cover software/accelerated graphics, isolated networking, forwards and camera through update, clone and restore. Existing late-mutation tests continue to enforce source identity and final authorization.

Historical desktop recovery holds the workspace mutation fence and rereads its journal before acting. Both schema 2 and schema 3 validate the source snapshot hash, preserving immutable guest identity when reconstructing the source. Committed cleanup additionally verifies the kernel against its exact receipt. Eight source/kernel cases distinguish valid recovery from tampering; invalid, corrupt and unknown journals retain repair evidence and reject subsequent start/update attempts. Schema-1 cleanup semantics remain covered. These are helper and in-process recovery tests, not new physical guest or SIGKILL evidence.

`updateDesktop` still retains its separate payload transaction and transient-share update. The preflight and recovery fixes above do not complete its compound operation. Direct compatibility update callers also remain outside the production configuration transaction. P01-08 therefore stays open for these paths, cancellation and real process-death coverage. Restart recovery preserves paused leases and reconciles adopted helpers whose admission was still starting or awaiting expiry recovery; physical-channel and host-pressure campaigns remain required.

The retained launch boundary is explicit: `startImplementation` routes resolved work through `startResolvedMachine`, the exact-plan resolver and the backend registry. `prepareMachineStart` rechecks mutable workspace bytes, artifact paths and share authority before spawn; the single-use pre-spawn authorization and runner envelope enforce the next process boundary. `startLegacyMachine`, derived legacy boot preparation and compatibility projection remain behind explicit compatibility authority for migration/qualification callers. Their removal still needs the P01-11/P01-13 entrypoint parity campaign.
...
- [ ] **P02-01** Move the fixed-address kernel traces, unconditional register/hypercall prints and local image paths out of normal execution. Preserve useful diagnostics as a bounded opt-in trace with image build ID, symbol map, last exits, faults and guest console tail.
- [ ] **P02-02** Replace globally advertised Xen identity and success stubs. Prefer correct non-Xen PVH entry with a supplied versioned memory map; ordinary UEFI remains the product boot contract. If a supported hypercall is retained, implement its exact semantics, capability discovery, argument validation, failure and guest fault behavior.
- [ ] **P02-03** Return defined failure for unsupported calls instead of success. Derive guest memory/topology answers from the actual machine configuration. Remove `try?` where it hides guest-memory faults or partial protocol writes.
- [ ] **P02-04** Audit every register encoding in `0F 01` and related system groups. Decode exact VMCALL/VMMCALL/VMX/SVM/invalidation forms and enforce mode, feature and privilege rules. Add all ModRM variants as bounded vectors; unsupported operations must raise the proper architectural exception.
- [ ] **P02-05** Rebuild ELF/PVH loading around checked offsets, lengths, program segments, BSS zero-fill, entry ranges and boot-note parsing. Remove arbitrary `p_paddr` masking; support only specified relocations. Do not generally load sections outside `PT_LOAD` to compensate for one malformed interpretation.
- [ ] **P02-06** Validate reserved low memory, command line, initramfs, memory map, ACPI tables, firmware apertures and kernel segments for overlap and overflow. Test multiple real kernels with different layouts, and malformed ELF/notes/headers that must reject without touching guest state.
- [ ] **P02-07** Generalize the existing Linux boot runner to explicit fixture/configuration arguments and bounded budgets. Required CI must obtain a pinned fixture or fail; optional local absence must be reported as skipped. Replace unconditional dumps with assertions of userspace readiness and clean poweroff.
...
- [ ] **P02-08** Create an ISA support inventory generated from or checked against decoded forms: prefixes, mode, operand/address size, decoder support, interpreter semantics, JIT support, flags, faults and reference vectors. Count executed forms separately from static binary decoding.
- [ ] **P02-09** Freeze a conservative versioned CPU profile. Audit all CPUID leaves/subleaves, topology, cache information, address widths, MSRs, feature dependencies and extended state. Never advertise a feature because its opcode decodes. Specifically audit currently omitted PAE/PSE/PGE against each selected Linux kernel: x86_64 requires PAE, while other requirements depend on kernel configuration. Implement and qualify the semantics before setting those bits; do not bypass checks using Xen identity.
- [ ] **P02-10** Define the first supported Linux CPU baseline explicitly. Qualify x86-64-v2 or v3 only when all required features work; keep AVX-512 and unrelated extensions outside the critical path unless a selected guest requires them. Preserve graceful `#UD` for unadvertised extensions.
...
- [ ] **P02-17** Audit architectural versus emulated time: TSC frequency/monotonicity, CPUID timing leaves, RDTSCP if advertised, pause/idle/reset semantics and scheduler interaction. Do not use raw host wall-clock discontinuities as guest time.
- [ ] **P02-18** Correct memory backing initialization: throwing allocation instead of process traps, validated size/alignment, holes/ROM/MMIO permissions, accounting, teardown and consistent errors. Share byte-array and mmap conformance cases rather than maintaining divergent semantics.
- [ ] **P02-19** Repair differential testing to give interpreter and JIT independent copies of initial memory/device state. Compare GPRs, RIP, relevant flags, SIMD/x87/XSTATE, segment/control state, written memory and fault details; compare permitted nondeterminism explicitly.
...
- [ ] **P02-21** Fuzz decode, instruction execution and page walking with fuel/memory limits. Minimize every guest-discovered bug into a regression case. Preserve crash inputs without exposing private guest memory in ordinary logs.

### P02.C Boot ladder and acceptance

- [ ] **P02-22** Boot a pinned minimal kernel/initramfs to a unique userspace marker. Exercise syscalls, process creation, exec, memory allocation, signals, timers, filesystem reads/writes and shutdown. Require a machine-readable result from the guest.
- [ ] **P02-23** Repeat with at least two different kernels and both musl and glibc userspace, then a systemd environment. Eliminate fixed load addresses and distribution-name workarounds from the VMM.
- [ ] **P02-24** Run the same diagnostic boot with interpreter and baseline JIT, preserving equivalent observable state and device behavior. Tier-switching may change speed; it must not change guest capabilities or machine ABI.
- [ ] **P02-25** Add one-vCPU stability stress: allocate/free/mmap/mprotect, fork/exec, package unpack, compression, checksums, clock tests, disk flush and sustained network I/O. Reduce failures before adding more CPU extensions.
- [ ] **P02-26** Record cold boot duration, guest instructions, fallback reasons and top host costs, but label them engineering measurements until P14.
...
- [ ] **P03-06** Complete the required PSCI functions: version/features and accurately supported CPU_ON/OFF/affinity/reset/poweroff behavior. Add suspend/hotplug only if part of the supported product, with explicit capability reporting.
...
- [ ] **P05-09** ARM: boot a stock installer through DoryARMVirt UEFI, install to a new disk, shut down, eject media, and boot the installed disk. Repeat on two independent general-purpose distro families before widening coverage.
...
- [ ] **P05-13** Qualify guest kernel and Mesa package updates, bootloader changes and initramfs regeneration on installed systems. Keep rollback/recovery media available without pinning every stock guest to Dory's managed kernel.
...
## P06. Complete accelerated Linux graphics on both architectures
...
- [ ] **P08-01** Correct install completion versus terminal `.stopped`: the desktop must remain alive after successful restore and perform first boot exactly once. Keep installer progress separate from VM lifecycle and publish one managed readiness handoff.
...
- [ ] **P08-06** Make suspend atomic: reserve capacity, quiesce, save, validate, publish, then acknowledge. Failure must leave a known running/paused/stopped state and a recoverable artifact, with no false “saved” event.
...
- [ ] **P08-16** Make camera/microphone access opt-in with host permission handling, revocation and visible use. Optional camera extension activation must not prevent the base Mac VM from installing or booting.
...
- [ ] **P08-26** Add guest-side OS/build/Metal device/feature inventory and a real Metal render/compute check with validated output. A visible `VZVirtualMachineView` is not proof of application GPU execution.
- [ ] **P08-27** Test Setup Assistant, login, repeated boot/shutdown, suspend/restore, host sleep/wake, display resizing, keyboard shortcuts, trackpad gestures, audio, clipboard, shared folders and ordinary browser/productivity use.
...
- [ ] **P10-07** Ship verified stopped/cold snapshots first for all three cells. Atomically include every required disk/identity/firmware artifact; expose their crash-consistent or application-consistent guarantee accurately.
...
- [ ] **P14-C06** Keep the existing harness responsibilities: `benchmark-user-workflows.sh`, `benchmark-developer-workflows.sh`, `benchmark-registry-npm.sh`, `benchmark-external-network.sh`, `benchmark-campaign.sh`, and `qualify-container-engine-performance.sh`. The latter now includes this plan as its governing document.
...
These are the first useful team issues, in dependency order within each workstream. They can begin together after the baseline is recorded.

| Team | First assignment | Next assignment | First reviewable milestone |
|---|---|---|---|
| CPU/DBT | Remove broad system-decode/Xen success shortcuts; fix required CPUID and differential state isolation | Correct ELF/PVH loader and assert userspace boot | Pinned kernel reaches userspace under interpreter/JIT without false platform services |
| PC/devices | Correct PM1/PM timer and interrupt behavior; shared queue vectors | PCI device parity and real UEFI installer | Installed x86 CLI guest boots its own disk and shuts down |
| ARM VMM/firmware | Prove current direct/UEFI baseline and one engine ownership strategy | Extract native execution with GIC/barrier parity | Same existing workloads run through the selected production boundary |
| Graphics/guest drivers | Inventory raw/portable GPU semantics; define truthful profiles and fence completion | Wire PC VirGL, then PC blob BAR/Venus and x86 guest artifacts | Real x86 OpenGL frame and sustained desktop; Vulkan follows its own gate |
| macOS | Fix install→first boot, saved-state authority and signal shutdown | Carry policy to builder; installed-disk transition | Product installs, stops, restarts and suspends a real Mac safely |
| Control/storage | Make one resolved launch input and operation truth | Wire Mac recovery/clone and all-cell cold snapshots | Same app/CLI plan and verified recoverable artifact operations |
| Desktop/tools | Separate guest readiness from helper liveness | General Mac agent, architecture-keyed Linux tools, actual capability UI | Clear install/runtime status and correct disabled integrations |
| QA/security/release | Repair reproducible builds/fixtures and identify prose-only checks | Physical matrix, failure injection and candidate collection | One repeatable qualification result per real guest cell |

Do not begin with a wholesale rewrite, broad language migration, Windows devices, Intel Mac emulation, a new renderer backend, or another generic planning framework.
...
- [ ] **DONE-04** Both Linux ISAs have qualified guest OpenGL and Vulkan acceleration; Mac has qualified guest Metal. Effective application/renderer evidence rules out accidental software rendering.
...
The old Phase 0A programme manifest and its fixed-heading document verifier/tests were removed. Readiness tests no longer require deleted documentation or enforce obsolete QEMU prose in documentation; remaining source-text policy checks are tracked for replacement. The container-performance script now includes `PLAN.md` in its evidence bundle. Runtime source is unchanged by this review; these small tooling edits make the requested document removal coherent.
