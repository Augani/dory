...
| PC GPU fence completion | `DoryVirtioGPU.process` scatters its response synchronously and echoes the FENCE flag without waiting for a renderer fence. The worker lane distinguishes receipt admission from actual fence signaling. In the PC transport, reset can also race between deferred completion's generation check and guest-memory writes because reset does not hold the queue processing lock. | Publish a fenced used-ring response only after the matching renderer signal. Preserve guest fence/context/ring identity independently of host fence IDs, handle failures and revoke pending work on reset. Serialize reset against the complete response publication transaction; test both a late callback and reset racing with publication. Do not substitute receipt admission for signal completion or discard requested fence semantics. |
