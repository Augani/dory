...
| Docker GPU | A signed daemon starts the isolated renderer; an ARM64 container using standard `--gpus all` completes a Vulkan shader with all 65,536 outputs correct. Exact render-node authorization is inspected. | Automatic crash recovery, application Settings flow, sustained workloads and broader isolation qualification remain open. Resolve the 16 KiB GPU / 4 KiB FEX conflict before claiming combined support. |
