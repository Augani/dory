...
**Producer progress (2026-09-08):** The shared VirGL2 Mesa producer now supports ARM64 (`arm-virgl2`) alongside x86_64 (`pc-virgl2`), with architecture-specific compiler, dependencies, artifact names, fingerprints, manifest checks, and ELF validation. Four architecture-selection tests pass, including rejection of substituted architecture/profile stamps. The real ARM64 build now passes the archive, manifest, architecture, dependency and GNU-libc checks; its [producer receipt](docs/virtualization/evidence/wave0-2026-09-08/arm64-virgl2-producer/receipt.json) retains the build log and stamp. Graphics qualification remains outstanding. The [x86_64 rebuild](docs/virtualization/evidence/wave0-2026-09-08/x86-virgl2-rebuild/receipt.json) also completed, and all three Mesa profiles now pass their producer verifiers. The inventory now requires the classic ARM64 kernel and Mesa pack as well as the existing graphics inputs. A freshly sealed app is still required.
...
**EFI control (2026-09-08):** With only `efi=debug` added and KASLR still enabled, the retained single-variable diagnostic reached root mount at 502.732 seconds and `/sbin/init` at 586.708 seconds. The strict 600-second deadline stopped it normally at 600.000127 seconds; agent readiness and RPC were not observed. This run avoided the earlier EFI panic without `nokaslr`, but does not establish its cause or a performance improvement. The separate `nokaslr`-only control reproduced the EFI instruction-fetch fault at `0x3fdc3743` and triple-faulted after 172.805 seconds, before root mount. Its process exit zero is not success: the [full stop and serial evidence](docs/virtualization/evidence/wave0-2026-09-08/pc-boot-timeline/nokaslr-only/receipt.json) records the failed boot. [Raw logs and identity receipt](docs/virtualization/evidence/wave0-2026-09-08/pc-boot-timeline/efi-debug-only/receipt.json).

**Unmodified control (2026-09-08):** The original installer command line on the same strict-deadline runner also faulted at `0x3fdc3743` and triple-faulted at 176.100 seconds. The debug memory map places that address in boot-service code. Linux preserves those mappings under `efi=debug`, so the next investigation is the firmware runtime reference and its owning symbol, rather than a speculative clock change. [Control evidence and source reference](docs/virtualization/evidence/wave0-2026-09-08/pc-boot-timeline/unmodified-strict-control/receipt.json).
