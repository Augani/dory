...
**Progress, 2026-09-04:** P02 remains in progress. The [correctness receipt through run 43](docs/virtualization/evidence/p02-correctness-2026-09-04/review-through-run-43.json) binds 706 tracked inputs to `3337bd442`, plus six identified generated FFI inputs. Run 43 passes 834 Swift Testing cases and 10 XCTest cases, with one optional boot-fixture skip and one disabled physical-reference test. Run 41 retains five failing binary80 tests with 121 issues; the unchanged tests pass after the run-42 correction. The [independent validation](docs/virtualization/evidence/p02-correctness-2026-09-04/evidence-validation-through-run-43.json) checks all four source snapshots, artifact hashes, schemas, individual counts, release-runner identity and I/O fixture provenance. Overlapping runs are not summed.

[Interpreter probe 3](docs/virtualization/evidence/p02-correctness-2026-09-04/userspace-interpreter-probe-3.json) and [baseline-JIT probe 9](docs/virtualization/evidence/p02-correctness-2026-09-04/userspace-baseline-jit-probe-9.json) still establish P02-24: the same source-32 runner and musl fixture produce seven equal workload result objects after excluding only each fresh UUID, and both independently reach ACPI S5. The second kernel's [glibc probe 3](docs/virtualization/evidence/p02-correctness-2026-09-04/userspace-glibc-kernel-b-probe-3.json) now also passes all seven glibc workloads and powers off, using source 39 and the explicitly selected `dory.x86_64.intel-compatible-v1` profile. The [reduced loader proof](docs/virtualization/evidence/p02-correctness-2026-09-04/glibc-cpu-replay-review.json) explains the earlier vendor-dispatch failure; the original Dory-v1 identity and feature masks remain unchanged. P02-23 stays open: the [first systemd probe](docs/virtualization/evidence/p02-correctness-2026-09-04/systemd-kernel-b-probe-1-host-failure-appendix.json) terminates its source-39 host runner with SIGTRAP in the CDQE accumulator conversion, with an empty log and no diagnostic JSON or accepted guest receipt. The selected host stack and exact source/binary disassembly are retained separately; this receipt does not validate the later correction.
