...
**Progress, 2026-09-04:** P02 remains in progress. The [correctness receipt through run 49](docs/virtualization/evidence/p02-correctness-2026-09-04/review-through-run-49.json) binds 719 tracked inputs to `8a907374a`, plus six identified generated FFI inputs. Run 49 passes 906 Swift Testing cases and 10 XCTest cases, with one optional boot-fixture skip and one disabled physical-reference test. The [independent validation](docs/virtualization/evidence/p02-correctness-2026-09-04/evidence-validation-through-run-49.json) checks both exact source snapshots, artifact hashes, schema, individual counts and 18 new regression methods. Run 48 retains four fixture failures with 206 issues: five constructors omitted the RIP matching their code at address zero. Run 49 adds only those five RIP arguments; product source and semantic expectations are unchanged. Three floating-point host-boundary, eight MMX state-effect and seven scalar-shift methods then pass. The [through-47 receipt](docs/virtualization/evidence/p02-correctness-2026-09-04/review-through-run-47.json) retains prior floating-point reset and IRET gates. Earlier failures remain preserved; overlapping runs are not summed.
