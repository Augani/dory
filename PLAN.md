...
The [verified support inventory](docs/virtualization/evidence/p02-correctness-2026-09-04/run-37-isa-inventory.json.gz) closes P02-08: 1,158 exact vectors, 1,081 static decodes, 77 rejections and zero expectation mismatches. Its small catalog binds three historically retired forms and one fault attempt to run-30 evidence; current-invocation and physical-reference executions remain zero. The system-group audit, range preflight, runner/required-fixture CI source, conservative versioned CPU-profile audit, time audit and bounded fuzzing close P02-04/06/07/09/17/21. The two real kernels have different internal symbol layouts but identical `PT_LOAD` envelopes. Linux CPU-baseline, floating-point and broader ISA qualification, physical reference, broader distribution coverage and hosted CI remain open. The [paging and CPU audit](docs/virtualization/p02-paging-audit-2026-09-04.md) maps the closures and remaining scope. P03 and P04 follow P02 as requested.
