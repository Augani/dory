...
The pinned x86_64 Mesa VirGL2 runtime now builds with a matching GLX loader, Gallium library and virtio-gpu DRI driver. Archive, source, dependency and ELF checks pass ([build evidence](docs/virtualization/evidence/p06-pc-2026-09-05/mesa-virgl2-build.json)). The kernel and rootfs producers also pass their artifact checks. A [GRUB diagnostic boot](docs/virtualization/evidence/p06-pc-2026-09-05/grub-linux-boot-progress.json) now reaches Linux HPET/interrupt initialization using explicit noapic boot arguments, then exhausts 1.2 billion instructions before userspace. Independent symbolization places the terminal stack in IRQ handling; the cause still needs diagnosis. Ordinary interrupt routing, complete userspace boot and a real guest GLX workload remain required before PC GPU admission can open.
