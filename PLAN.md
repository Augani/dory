...
| Docker GPU | GPU configuration/kernel selection and Docker device-request normalization exist, but `EngineMode.run` rejects GPU and constructs no worker-backed GPU device. | Add daemon renderer bootstrap, engine `VirtioGPU` attachment, guest DRM/ICD readiness and observed Docker device authorization. Resolve the 16 KiB GPU / 4 KiB FEX conflict before claiming combined support. |
