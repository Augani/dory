...
**Progress, 2026-09-04:** P02 remains in progress. The [correctness receipt through run 46](docs/virtualization/evidence/p02-correctness-2026-09-04/review-through-run-46.json) binds 714 tracked inputs to `73072d019`, plus six identified generated FFI inputs. Runs 44/45/46 pass 848/853/875 Swift Testing cases respectively; each also passes 10 XCTest cases, with one optional boot-fixture skip and one disabled physical-reference test. The [independent validation](docs/virtualization/evidence/p02-correctness-2026-09-04/evidence-validation-through-run-46.json) checks all three exact source snapshots, artifact hashes, schemas, individual counts, regression declarations and the distinct source-45 optimized runner. The [through-43 receipt](docs/virtualization/evidence/p02-correctness-2026-09-04/review-through-run-43.json) preserves run 41's five failing binary80 tests and 121 issues; the unchanged tests pass after its correction. Overlapping runs are not summed.
...
Runs 40–43 add feature-policy checks for 131 admitted encodings, fix five binary80 boundary failures, and correct the modern VirtIO network header. The [I/O fixture preparation](docs/virtualization/evidence/p02-correctness-2026-09-04/io-stress-v1-fixture-preparation-review.json) verifies 53 passing script tests, identical local cross-build outputs and exact pinned modules. Twelve host-helper tests plus two runner-configuration tests cover the fresh disk and isolated Ethernet peer. P02-25 remains open until a guest completes the two I/O workloads with reconciled frame counts, an actual flush, reopened block bytes and host-observed poweroff. Full CPU, floating-point and guest-device qualification are not inferred from these source gates. Runs 44–46 additionally validate noncanonical system-descriptor rejection, selected one-GiB-page support through warm translations and native fallback, full LDT limits, legacy floating-point admission, and translated interrupt tables/stacks with precise frame preflight. These are bounded corrections; their broader CPU, paging, fault and floating-point checkpoints stay open.
