...
**Progress, 2026-09-04:** P02 remains in progress. The [correctness receipt through run 50](docs/virtualization/evidence/p02-correctness-2026-09-04/review-through-run-50.json) binds 722 tracked inputs to `862383b91`, plus six identified generated FFI inputs. Run 50 passes 952 Swift Testing cases and 10 XCTest cases, with one optional boot-fixture skip and one disabled physical-reference test. The filter now includes all VirtIO suites, so the increased total includes existing tests as well as seven new real-mode IRET and eight available-event methods. The [independent validation](docs/virtualization/evidence/p02-correctness-2026-09-04/evidence-validation-through-run-50.json) checks the exact snapshot, artifacts, schemas, individual counts, regression declarations and optimized source-50 runner. The [through-49 receipt](docs/virtualization/evidence/p02-correctness-2026-09-04/review-through-run-49.json) preserves run 48's four fixture failures and its five-RIP-argument correction, with unchanged product source and expectations. Overlapping runs are not summed.
