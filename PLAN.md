...
- **x86 VM performance:** reduce measured timer/interrupt-path interpreter fallbacks, preserve precise faults and debug behavior, then rerun ordinary UEFI boot through the production runner. [Native RDTSC](docs/virtualization/evidence/p06-pc-2026-09-05/native-rdtsc.json) now passes 100 focused tests; its latest guest run must be repeated because the temporary raw receipt is no longer available. [Bounded clock comparisons](docs/virtualization/evidence/p06-pc-2026-09-05/clock-source-comparison.json) do not establish a clock fix or usable boot performance.
- **x86 containers:** repair precise asynchronous signal delivery in FEX; concurrent Go regexp workloads and dockerd still fail private candidate tests. Preserve passing ARM64 GPU compute while resolving the combined 4 KiB FEX / GPU guest configuration.
...
| PC GPU fence completion | `DoryVirtioGPU.process` scatters its response synchronously and echoes the FENCE flag without waiting for a renderer fence. The worker lane distinguishes receipt admission from actual fence signaling. In the PC transport, reset can also race between deferred completion's generation check and guest-memory writes because reset does not hold the queue processing lock. | Publish a fenced used-ring response only after the matching renderer signal. Preserve guest fence/context/ring identity independently of host fence IDs, handle failures and revoke pending work on reset. Follow the [VirtIO GPU control-header contract](https://docs.oasis-open.org/virtio/virtio/v1.3/csd01/virtio-v1.3-csd01.html): context ring indices are 0–63, and a completed context-timeline fence also retires outstanding sequence numbers at or below it on that timeline. Serialize reset against the complete response publication transaction; test both a late callback and reset racing with publication. Do not substitute receipt admission for signal completion or discard requested fence semantics. |
