...
| PC GPU partial readback | `DoryPCVirGLRendererAuthority.transfer3D` copies the entire staging backing into guest memory after a partial renderer readback. Bytes outside the transfer can still contain their attach-time values and overwrite later guest writes. | Preserve bytes outside the renderer's written ranges, including scatter backing, row padding and array/mip layouts. Derive range semantics from the pinned renderer; do not narrow the advertised format support to make the fix pass. Validate bounds before submitting work and avoid guest-sized span allocations. A whole-backing refresh alone requires proof that concurrent guest CPU and device writes cannot be lost. Verify observable guest bytes with a partial-readback regression. |
