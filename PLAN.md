...
- **x86 VM performance:** reduce measured timer/interrupt-path interpreter fallbacks, preserve precise faults and debug behavior, then rerun ordinary UEFI boot through the production runner. [Native RDTSC](docs/virtualization/evidence/p06-pc-2026-09-05/native-rdtsc.json) now passes 100 focused tests; its latest guest run must be repeated because the temporary raw receipt is no longer available. [Bounded clock comparisons](docs/virtualization/evidence/p06-pc-2026-09-05/clock-source-comparison.json) do not establish a clock fix or usable boot performance. The [combined JIT runner](docs/virtualization/evidence/p06-pc-2026-09-05/combined-jit-post-cpu0.json) now advances beyond the prior CPU0 stall to PCI/HPET initialization and TSC clocksource selection; userspace completion and useful responsiveness remain unverified. [Native memory fences](docs/virtualization/evidence/p06-pc-2026-09-05/native-memory-fences.json) now preserve synchronization and fault boundaries in 130 focused tests; the current live runner predates this change.
- **x86 containers:** repair precise asynchronous signal delivery in FEX. A [private signal-mask lifecycle correction](docs/virtualization/evidence/p06-container-2026-09-05/signal-mask-lifecycle-candidate.json) passes 220 edited push/pop probes and 100 controls, but changed RIP/RSP return probes still crash; the candidate is not promotable, and default Go regexp/dockerd qualification remains open. Preserve passing ARM64 GPU compute while resolving the combined 4 KiB FEX / GPU guest configuration.
...
| FEX asynchronous signals | The committed signal patch overwrites `ContextBackup.GuestState` after SRA spill, although upstream restores that field alongside the original host continuation. This conflates the original split-state backup with the guest-visible edit baseline. Prior RIP/stack/register failures also require coherent delivery boundaries. | First preserve the original continuation backup and separately capture the published x64 signal-frame baseline; verify unchanged continuation, changed GPR/RSP with unchanged RIP, flags/vector edits and nested signals. Separately prove prompt replay for direct-linked loops: existing JIT interrupt checks are conditional, so dispatcher deferral alone is insufficient. Do not promote new artifacts until existing default-preemption workloads pass. |
...
Current delivery wave: the x86 worker must establish guest-agent bind/handshake readiness on the new source-frozen runner before command and tier comparisons. The [rpcdiag timing audit](docs/virtualization/evidence/p06-pc-2026-09-05/rpcdiag-readiness-timing-audit.json) confirms serial command execution but finds that probes ran from a pre-exec marker; it does not establish a persistent transport defect. The GPU worker must run the real signed runner through MachineManager-owned renderer descriptors using the private test-root harness, then collect actual render/readback evidence; this does not close production catalog admission. The FEX worker must resolve the remaining Go workload failures before promoting its signal-context/host-altstack candidate. Preserve active build caches and immutable inputs; remove disposable disks only after terminal-process checks and external evidence capture.
