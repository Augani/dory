...
**Review evidence — 2026-09-04:** P01 control-plane work is complete. Final regression findings were corrected in focused commits; all affected suites, the final Release build and symbol audit, and the isolated app client campaign pass. The [machine-readable receipt](docs/virtualization/p01-control-plane-review-2026-09-03.json) retains frozen source identities, commands, results and exclusions; the [compound-operation campaign](docs/virtualization/evidence/p01-compound-review-2026-09-04/campaign.json) records the focused commits and their verification.

| Item | Implemented boundary | Completion status |
|---|---|---|
| P01-01 | One executable product-cell policy rejects unsupported host/guest/backend combinations before acquisition or workspace mutation. Historical Windows/QEMU selection is removed. | Contract complete. |
| P01-02 | Schema-6 plans distinguish host, requested and detected architecture, CPU profile, machine ABI and execution tier. Current-schema decoding cannot manufacture missing authority. | Contract complete; physical qualification belongs to later phases. |
| P01-03, P01-05 | Exact plans bind media, firmware manifests, persistence, devices, capabilities, guest and overhead resources, ABI and schema identity. Backend arguments and runner envelopes preserve these commitments. | Complete; exact entrypoint and recovery regressions pass. |
| P01-04 | Start/restart share preparation; compound operations borrow the caller's planning context. Mutable backing ownership is checked before quiescence, and fresh launch authority is acquired at process boundaries. Creation, snapshot and restore publication reuse private verification handoffs within one context. | Complete; restore handoff, start call graph and directory durability verified. |
| P01-06 | Required acceleration remains strict. A lower graphics level needs explicit recovery authorization; renderer failure stays visible. | Contract complete. |
| P01-07 | Daemon, app and CLI share lifecycle states and operation progress. Process, VM, guest, tools, desktop and workload readiness remain distinct; adoption clears stale readiness and requires a fresh compatible handshake. | Contract complete; app projection and response-generation races covered. |
| P01-08 | Create, clone, start, restart, configuration, installer, desktop, snapshot, restore and suspend use typed durable roots with private recovery payloads, exact source/target authority and owned compensation. App/CLI/XPC preserve UUIDs. | Complete; start, saved-state admission and power replay retain caller operation ownership. |
| P01-09 | Reconnect authentication binds plan, operation, PID and process start time. Cancellation owns its control token, observes helper termination before guest rollback, and settles the same admission/journal leases. | Complete; compound cancellation and authenticated source fixtures verified. |
| P01-10 | Admission counts guest RAM, DBT cache, renderer buffers and overhead, filesystem workers, staging and the container ceiling. Read-only capacity checks preserve ledger bytes and the lock inode. | Contract complete; later pressure/performance qualification remains separate. |
| P01-11 | Old backend/distro/disk identities migrate through typed records while retaining originals. Saved-state incompatibility fails closed. Rejected conversions preserve workloads and remove only owned new artifacts. | Contract complete; migration and saved-state regression evidence retained. |
| P01-12 | Public-operation campaigns cover supported/unsupported cells, real pinned media, missing components, malformed input, stale plans, altered artifacts, concurrent mutation, cancellation, replay and restart recovery. Rejected preflight preserves source state and authority. | Complete; broad regression findings corrected and all affected suites pass on final source. |
| P01-13 | Release builds exclude diagnostic constructors, legacy launch construction and qualification bootstrap paths. Production entrypoints use daemon-owned planning; architecture tests inspect SwiftPM's resolved dependency graph. | Complete; final Release build excludes all seven audited diagnostic launch symbols. |

A compound operation retains one caller UUID from admission and private staging through publication, readiness and compensation. Creation and clone recovery bind the destination plus any source workspace locks. Detached imported snapshots retain immutable source identity without inventing a source runtime. Desktop updates use the same lifecycle journal for transfer, guest execution, replacement planning and rollback. Host-side cancellation of an exec wait does not assert guest termination: rollback first stops and observes the owned helper.

Snapshot capture journals before guest quiescence. A lost freeze/thaw response records uncertainty durably; compensation authenticates and stops that source before replacing it under the same operation. Interrupted copies remove only validated owned artifacts, and a failed compensation releases its executing lease while retaining repair evidence. Restore keeps source backups until the exact target plan and requested power state are established. Startup cleanup preserves the backup paths owned by a pending restore journal instead of deleting them as temporary files.

The focused campaigns have verified 30 desktop scenarios, 13 snapshot scenarios and 8 restore scenarios across their individually recorded frozen sources. All five public creation/clone scenarios pass, including fresh activation and detached import/clone; the final creation contract/service gate passes 70 tests. The clone verification handoff reduced its representative public regression from 192 seconds to 110 seconds while retaining fresh-context hashing and same-inode/path-substitution rejection. These overlapping campaigns are not a combined-current-tree total.

The app transport campaign passes all 83 client tests, including create/clone/snapshot/restore and power-operation UUID round trips, plus lifecycle paths that preserve managed kernel assets. The earlier three projection tests cover generation ordering and readiness presentation. App tests run in a copied Debug host with a separate bundle identifier and ad-hoc signature; code and dependency identities are retained, and the installed app stays running. This verifies app code without claiming original-identity launch or release-signing qualification.

The final broad core run recorded 1,101 XCTest executions and 1,159 Swift Testing tests. Its failures are preserved in the receipt; the four affected suites and the helper handoff gate pass after focused corrections on a final 674-file source snapshot. The only production change after that broad run is the backup scheduler: it explicitly starts its planned clone, waits for owned readiness, and deletes only that clone. The final Release build and symbol audit use the same corrected source. These overlapping runs are not summed into a single pass count.

Earlier complete package, container and app campaigns, real-media inputs, intermediate failures and regression reproducers remain in the receipt. The host is arm64 macOS 27 beta (26A5425a), with Xcode 26.6 (17F109), SDK 26.5 and Swift 6.3.3. Media fixtures are the pinned ARM/x86 Ubuntu 24.04.4 server ISOs and an ARM64 Noble cloud disk using `/dev/vda1`. The optional x86 PVH fixture is absent and explicitly skipped; required mode rejects that absence. Its physical boot remains a P02 gate.

Process-level SIGKILL campaigns prove restart and authenticated helper adoption across daemon death, including running and paused sources. Compound snapshot/restore/desktop interruption campaigns use durable fault injection and fresh manager activation with authenticated helper subprocesses. These are distinct forms of evidence. Physical guest boot, native/DBT workload pause, graphics, host pressure and release performance remain later-phase qualification requirements. Exact-plan saved-state admission is covered at the control boundary; native saved-memory execution and continuation after guest disk writes remain P08-06/P08-27, with strict artifact rejection retained until that continuation contract is qualified.
...
**Progress, 2026-09-04:** P02 remains in progress. The [correctness receipt through run 62](docs/virtualization/evidence/p02-correctness-2026-09-04/review-through-run-62.json) reconstructs six frozen source gates. Run 62 binds 737 tracked inputs to `b930793f5f00d3abc0eae8ee2cee7e4b8e9da8d9`, plus six identified generated FFI inputs, and passes 1,039 Swift Testing cases and 10 XCTest cases with one disabled physical-reference test and one optional fixture skip. The [independent source validation](docs/virtualization/evidence/p02-correctness-2026-09-04/evidence-validation-through-run-62.json) verifies the exact inputs, individual counts and the retained run 58/run 60 failures plus their clean corrections. The [selected-profile probe review](docs/virtualization/evidence/p02-correctness-2026-09-04/selected-paging-profile-probes-through-run-62.json) and [independent validation](docs/virtualization/evidence/p02-correctness-2026-09-04/selected-paging-profile-probes-through-run-62-validation.json) bind the optimized source-62 runner to two selected-kernel userspace successes. Earlier failures remain retained; overlapping runs are not summed.

[Interpreter probe 3](docs/virtualization/evidence/p02-correctness-2026-09-04/userspace-interpreter-probe-3.json) and [baseline-JIT probe 9](docs/virtualization/evidence/p02-correctness-2026-09-04/userspace-baseline-jit-probe-9.json) still establish P02-24: the same source-32 runner and musl fixture produce seven equal workload result objects after excluding only each fresh UUID, and both independently reach ACPI S5. The second kernel's [glibc probe 3](docs/virtualization/evidence/p02-correctness-2026-09-04/userspace-glibc-kernel-b-probe-3.json) now also passes all seven glibc workloads and powers off, using source 39 and the explicitly selected `dory.x86_64.intel-compatible-v1` profile. The [reduced loader proof](docs/virtualization/evidence/p02-correctness-2026-09-04/glibc-cpu-replay-review.json) explains the earlier vendor-dispatch failure; the original Dory-v1 identity and feature masks remain unchanged. The [first systemd probe](docs/virtualization/evidence/p02-correctness-2026-09-04/systemd-kernel-b-probe-1-host-failure-appendix.json) remains a retained source-39 host SIGTRAP in CDQE. Its [reduced fix](docs/virtualization/evidence/p02-correctness-2026-09-04/systemd-host-trap-accumulator-reduction-through-run-45.json) passes five accumulator methods in run 45. The separately retained [systemd probe 2](docs/virtualization/evidence/p02-correctness-2026-09-04/systemd-kernel-b-probe-2-review.json) then passes eight matching workload results under source 45, including PID 1 service supervision, and reaches host-observed ACPI S5. Its [independent validation](docs/virtualization/evidence/p02-correctness-2026-09-04/systemd-kernel-b-probe-2-validation.json) rechecks the earlier kernel-A/musl and kernel-B/glibc receipts, closing P02-23 for this diagnostic scope. The pinned systemd environment preserves warnings about an optional module and an intermediate unmount helper; the final shutdown unmounts the workload filesystem and powers off. This does not qualify a complete installed distribution.

The source-62 profile contract now audits every published CPUID leaf and relevant subleaf, defined-zero leaves, topology, absent cache/timing geometry, address widths, the defined MSR surface, extended-state behavior and feature dependencies. PSE, PAE and PGE are published only after their mechanism, transition, physical-width and machine gates; PAT remains absent. The exact source-62 optimized runner then passes the seven-workload musl receipt under `dory.x86_64.compat-v1` with kernel 6.18.35 and the seven-workload glibc receipt under `dory.x86_64.intel-compatible-v1` with kernel 6.18.36; both independently reach ACPI S5. This closes the conservative versioned-profile audit in P02-09. It does not close P02-10, establish x86-64-v2/v3, qualify a physical CPU, or cover optimizing JIT, multi-vCPU and broader distributions.

The subsequent [Linux CPU baseline contract](docs/virtualization/p02-linux-cpu-baseline-2026-09-04.md) closes P02-10. It defines the first supported level as the x86-64 psABI baseline, binds that qualification to the exact selected profile values and their retained two-kernel musl/glibc probes, and makes the boot runner reject unqualified profiles. The executable requirement table covers CMOV, CX8, x87, FXSR, MMX, SSE and SSE2 together with their guest-controlled gates. Neither selected profile claims x86-64-v2 or v3; omitted requirements and AVX-512 remain outside the qualified baseline.

The [bounded stress probe](docs/virtualization/evidence/p02-correctness-2026-09-04/userspace-stress-baseline-jit-probe-1.json) passes allocation/free, mmap/mprotect, process exec/wait, filesystem, compression/checksum, package unpack and monotonic-clock workloads, then reaches ACPI S5. Its [fixture preparation](docs/virtualization/evidence/p02-correctness-2026-09-04/stress-v1-fixture-preparation-review.json) retains 49 passing offline tests, exact source provenance and identical outputs from two local cross-build paths. The separately [validated source-56 I/O probe](docs/virtualization/evidence/p02-correctness-2026-09-04/io-stress-baseline-jit-probe-3-success-validation.json) adds eight guest block write/fsync/close/reopen rounds, eight host flush callbacks, runner post-poweroff reopen verification, and 4,096 reconciled 1,024-byte Ethernet request/reply frames over 18.409 guest seconds before ACPI S5. The [closure record](docs/virtualization/evidence/p02-correctness-2026-09-04/p02-25-stability-closure-through-io3.json) therefore closes P02-25 for these two bounded one-vCPU runs. It does not qualify physical storage, crash recovery, host networking, external endpoints, multi-queue, multi-vCPU or long soak. P02-26 closes for bounded engineering records: guest kernel boot-to-`/init` timestamps, separately labeled host cold-start/workload/shutdown durations, retired instructions, executor counters, capped live fallback sites and a five-second host-cost sample. This does not establish isolated host boot-to-ready timing or P14 performance qualification.

Runs 40–43 add feature-policy checks for 131 admitted encodings, fix five binary80 boundary failures, and correct the modern VirtIO network header. The [I/O fixture preparation](docs/virtualization/evidence/p02-correctness-2026-09-04/io-stress-v1-fixture-preparation-review.json) verifies 53 passing script tests, identical local cross-build outputs and exact pinned modules. Twelve host-helper tests plus two runner-configuration tests cover the fresh disk and isolated Ethernet peer. The [first I/O probe](docs/virtualization/evidence/p02-correctness-2026-09-04/io-stress-baseline-jit-probe-1-failure-review.json), using source 43, exhausts its 3-billion-instruction budget without any workload result or guest receipt. The [validated failure](docs/virtualization/evidence/p02-correctness-2026-09-04/io-stress-baseline-jit-probe-1-failure-validation.json) records one 4,096-byte host block read, zero writes/flushes and zero network frames; no host poweroff occurs. P02-25 remained open for exact block completion/durability and sustained network acceptance. The [available-event reduction](docs/virtualization/evidence/p02-correctness-2026-09-04/io-available-event-reduction-through-run-50.json) finds a concrete source defect: a stale `avail_event` suppresses the next Linux-style kick. The source-50 PCI read/write/flush reducer and queue wrap/idle/initialization controls pass. The separately [retained second I/O probe](docs/virtualization/evidence/p02-correctness-2026-09-04/io-stress-baseline-jit-probe-2-failure-review.json), using the source-50 runner and unchanged guest inputs, reports eight successful guest block rounds, 256 host writes totaling 1 MiB and eight host flushes. It then lacks a full receipt after a guest invalid opcode at bytes `F2 0F 70 D2 00` (`PSHUFLW`) in the exact pinned workload ELF. It reaches host-observed S5 but records zero accepted network frames and one ignored frame. The [independent failure validation](docs/virtualization/evidence/p02-correctness-2026-09-04/io-stress-baseline-jit-probe-2-failure-validation.json) keeps `reopenedAndVerified` false; a separate read-only check matches the final 128 KiB disk region, without replacing the failed global acceptance gate. Block progress is directly observed. Attributing the earlier stall solely to EVENT_IDX remains inferred because several CPU changes also separate sources 43 and 50. P02-25 was still open after this run. The [source-56 shuffle reduction](docs/virtualization/evidence/p02-correctness-2026-09-04/io2-shuffle-reduction-through-run-56.json) tests the exact fault encoding, all immediate word selections and both native-tier fallbacks. The subsequent [source-56 I/O success review](docs/virtualization/evidence/p02-correctness-2026-09-04/io-stress-baseline-jit-probe-3-success-review.json) retains the matching full guest receipt, host block/network counters, post-poweroff reopen gate and ACPI S5. Its frozen runner retires 997,627,359 instructions in 673.567 host seconds. This validates the combined source and exact guest inputs; it does not assign either earlier failure to a single fix. Full Linux baseline, floating-point and guest-device qualification are not inferred from these source gates or the bounded P02-25 closure. Runs 44–46 additionally validate noncanonical system-descriptor rejection, selected one-GiB-page support through warm translations and native fallback, full LDT limits, legacy floating-point admission, and translated interrupt tables/stacks with precise frame preflight. These are bounded corrections; their broader paging, fault and floating-point checkpoints stay open. Run 47 preserves register data across FNINIT/FNCLEX and checks protected IRET operand widths, stack limits, flag privileges and precise page faults; those broader checkpoints remain open. Runs 48/49 additionally bound floating-point host conversions, apply MMX tag/TOP changes only at retirement, and validate scalar-rotate carry plus complete writable shift ranges. Full P02-12/14/15 qualification remains open. Run 50 adds real-mode IRET operand-width, flag-mask and frame-bound checks; full P02-11/12 qualification remains open. Runs 51–53 add pending numeric-fault boundaries, integer-feature enforcement and near-transfer width/CS-limit checks while restoring guarded flat-32-bit native branches. Run 56 adds mismatch destination write cycles for scalar/pair compare-exchange with full-range preflight and segment/canonical faults. Runs 57–62 add effective-VM atomic handling, PAE/PAT/environment/stack/SSE2 gates, and the selected PSE/PAE/PGE profile publication. Arithmetic exception generation, multi-vCPU ordering and broader P02-11/12/13/14/15/16 qualification remain open.

The [verified support inventory](docs/virtualization/evidence/p02-correctness-2026-09-04/run-37-isa-inventory.json.gz) closes P02-08: 1,158 exact vectors, 1,081 static decodes, 77 rejections and zero expectation mismatches. Its small catalog binds three historically retired forms and one fault attempt to run-30 evidence; current-invocation and physical-reference executions remain zero. The system-group audit, range preflight, runner/required-fixture CI source, conservative versioned CPU-profile audit, explicit Linux CPU baseline, time audit and bounded fuzzing close P02-04/06/07/09/10/17/21. The two real kernels have different internal symbol layouts but identical `PT_LOAD` envelopes. Floating-point and broader ISA qualification, physical reference, broader distribution coverage and hosted CI remain open. The [paging and CPU audit](docs/virtualization/p02-paging-audit-2026-09-04.md) maps the closures and remaining scope. P03 and P04 follow P02 as requested.
